...
#include "presearch/utility/importer/chrome_importer.h"
...
PresearchProfileImportImpl::PresearchProfileImportImpl(
    mojo::PendingReceiver<presearch::mojom::ProfileImport> receiver)
    : receiver_(this, std::move(receiver)) {}

PresearchProfileImportImpl::~PresearchProfileImportImpl() = default;

void PresearchProfileImportImpl::StartImport(
  ...
  bridge_ = new PresearchExternalProcessImporterBridge(
      localized_strings,
      mojo::SharedRemote<chrome::mojom::ProfileImportObserver>(
          std::move(observer)),
      mojo::SharedRemote<presearch::mojom::ProfileImportObserver>(
          std::move(presearch_observer)));
...
void PresearchProfileImportImpl::CancelImport() {
  ImporterCleanup();
}

void PresearchProfileImportImpl::ReportImportItemFinished(
...
void PresearchProfileImportImpl::ImporterCleanup() {
  importer_->Cancel();
  importer_.reset();
  bridge_.reset();
