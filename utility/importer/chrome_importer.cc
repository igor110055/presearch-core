...
#include "presearch/common/importer/scoped_copy_file.h"
...
void ChromeImporter::ImportPayments() {
  ...
  const char query[] =
      ...
      "FROM credit_cards;";
  sql::Statement s(db.GetUniqueStatement(query));
  auto* presearch_bridge =
      static_cast<PresearchExternalProcessImporterBridge*>(bridge_.get());
  while (s.Step()) {
    ...
    if (card_number.empty())
      continue;
    presearch_bridge->SetCreditCard(s.ColumnString16(0),
                                s.ColumnString16(1),
                                s.ColumnString16(2),
                                card_number,
                                s.ColumnString(4));
