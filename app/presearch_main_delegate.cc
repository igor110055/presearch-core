...
#include "presearch/app/presearch_command_line_helper.h"
...
#include "presearch/utility/presearch_content_utility_client.h"
...
#include "presearch/build/android/jni_headers/PresearchQAPreferences_jni.h"
#else
#include "chrome/browser/ui/profile_picker.h"
#endif

namespace {
// staging "https://sync-v2.presearchsoftware.com/v2" can be overriden by
// switches::kSyncServiceURL manually
const char kPresearchSyncServiceStagingURL[] =
    "https://sync-v2.presearchsoftware.com/v2";
}  // namespace

#if !defined(CHROME_MULTIPLE_DLL_BROWSER)
base::LazyInstance<PresearchContentRendererClient>::DestructorAtExit
    g_presearch_content_renderer_client = LAZY_INSTANCE_INITIALIZER;
base::LazyInstance<PresearchContentUtilityClient>::DestructorAtExit
    g_presearch_content_utility_client = LAZY_INSTANCE_INITIALIZER;
#endif
#if !defined(CHROME_MULTIPLE_DLL_CHILD)
base::LazyInstance<PresearchContentBrowserClient>::DestructorAtExit
    g_presearch_content_browser_client = LAZY_INSTANCE_INITIALIZER;
#endif

const char kPresearchOriginTrialsPublicKey[] =
...
PresearchMainDelegate::PresearchMainDelegate() : ChromeMainDelegate() {}

PresearchMainDelegate::PresearchMainDelegate(base::TimeTicks exe_entry_point_ticks)
    : ChromeMainDelegate(exe_entry_point_ticks) {}

PresearchMainDelegate::~PresearchMainDelegate() {}

content::ContentBrowserClient* PresearchMainDelegate::CreateContentBrowserClient() {
#if defined(CHROME_MULTIPLE_DLL_CHILD)
  return NULL;
#else
  if (chrome_content_browser_client_ == nullptr) {
    chrome_content_browser_client_ =
        std::make_unique<PresearchContentBrowserClient>();
  }
...
PresearchMainDelegate::CreateContentRendererClient() {
#if defined(CHROME_MULTIPLE_DLL_BROWSER)
  return NULL;
#else
  return g_presearch_content_renderer_client.Pointer();
#endif
}

content::ContentUtilityClient* PresearchMainDelegate::CreateContentUtilityClient() {
#if defined(CHROME_MULTIPLE_DLL_BROWSER)
  return NULL;
#else
  return g_presearch_content_utility_client.Pointer();
#endif
}

void PresearchMainDelegate::PreSandboxStartup() {
  ...
  base::PathService::Override(
      ...
      base::FilePath(FILE_PATH_LITERAL("/etc/presearch/policies")));
#endif

  if (presearch::SubprocessNeedsResourceBundle()) {
    presearch::InitializeResourceBundle();
  }
}

bool PresearchMainDelegate::BasicStartupComplete(int* exit_code) {
  PresearchCommandLineHelper command_line(base::CommandLine::ForCurrentProcess());
#if BUILDFLAG(PRESEARCH_ADS_ENABLED)
  ...
  if (!base::CommandLine::ForCurrentProcess()->HasSwitch(
    ...
    command_line.AppendSwitchASCII(embedder_support::kOriginTrialPublicKey,
                                   kPresearchOriginTrialsPublicKey);
  }

  std::string presearch_sync_service_url = PRESEARCH_SYNC_ENDPOINT;
#if defined(OS_ANDROID)
  AdjustSyncServiceUrlForAndroid(&presearch_sync_service_url);
#endif  // defined(OS_ANDROID)

  // Presearch's sync protocol does not use the sync service url
  command_line.AppendSwitchASCII(switches::kSyncServiceURL,
                                 presearch_sync_service_url.c_str());

  command_line.AppendSwitchASCII(switches::kLsoUrl, kDummyUrl);

  // Presearch variations
  std::string kVariationsServerURL = PRESEARCH_VARIATIONS_SERVER_URL;
...
#if defined(OS_ANDROID)
void PresearchMainDelegate::AdjustSyncServiceUrlForAndroid(
    std::string* presearch_sync_service_url) {
  DCHECK_NE(presearch_sync_service_url, nullptr);
  ...
  bool b_use_staging_sync_server =
      Java_PresearchQAPreferences_isSyncStagingUsed(env);
  if (b_use_staging_sync_server) {
    *presearch_sync_service_url = kPresearchSyncServiceStagingURL;
  }
}
#endif  // defined(OS_ANDROID)
