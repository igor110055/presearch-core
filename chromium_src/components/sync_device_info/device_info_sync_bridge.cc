...
#define PRESEARCH_MAKE_LOCAL_DEVICE_SPECIFICS \
  specifics->mutable_presearch_fields()->set_is_self_delete_supported(true);

#include "../../../../components/sync_device_info/device_info_sync_bridge.cc"

#undef PRESEARCH_MAKE_LOCAL_DEVICE_SPECIFICS
...
std::unique_ptr<PresearchDeviceInfo> PresearchSpecificsToModel(
  ...
  // extra patch
  return std::make_unique<PresearchDeviceInfo>(
      ...
      data_types,
      specifics.has_presearch_fields() &&
          specifics.presearch_fields().has_is_self_delete_supported() &&
          specifics.presearch_fields().is_self_delete_supported());
}
...
DeviceInfoSyncBridge::GetAllPresearchDeviceInfo() const {
  std::vector<std::unique_ptr<PresearchDeviceInfo>> list;
  for (auto iter = all_data_.begin(); iter != all_data_.end(); ++iter) {
    list.push_back(PresearchSpecificsToModel(*iter->second));
  }
  return list;
}
