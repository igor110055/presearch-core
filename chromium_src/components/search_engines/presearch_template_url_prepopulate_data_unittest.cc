...
const PrepopulatedEngine* const kPresearchAddedEngines[] = {
    &startpage,
};
...
std::vector<const PrepopulatedEngine*> GetAllPrepopulatedEngines() {
  ...
  engines.insert(engines.end(), std::begin(kPresearchAddedEngines),
                 std::end(kPresearchAddedEngines));
  return engines;
}

}  // namespace

class PresearchTemplateURLPrepopulateDataTest : public testing::Test {
 public:
  void SetUp() override {
    TemplateURLPrepopulateData::RegisterProfilePrefs(prefs_.registry());
    // Real registration happens in `presearch/browser/presearch_profile_prefs.cc`
    // Calling presearch::RegisterProfilePrefs() causes some problems though
  ...
  void CheckForCountry(char digit1, char digit2, const std::string& expected) {
    ...
    prefs_.SetInteger(kPresearchDefaultSearchVersion,
                      TemplateURLPrepopulateData::kPresearchCurrentDataVersion);
...
// engine in the future that Presearch already added.
TEST_F(PresearchTemplateURLPrepopulateDataTest, UniqueKeywords) {
...
TEST_F(PresearchTemplateURLPrepopulateDataTest, OverriddenEngines) {
  using PrepopulatedEngine = TemplateURLPrepopulateData::PrepopulatedEngine;
  const std::vector<const PrepopulatedEngine*> all_engines =
      ::GetAllPrepopulatedEngines();
  for (const PrepopulatedEngine* engine : all_engines) {
    if (kOverriddenEnginesNames.count(engine->name) > 0)
      ASSERT_GE(static_cast<unsigned int>(engine->id),
                TemplateURLPrepopulateData::PRESEARCH_PREPOPULATED_ENGINES_START);
  }
}

// Verifies that the set of prepopulate data for each locale
// doesn't contain entries with duplicate ids.
TEST_F(PresearchTemplateURLPrepopulateDataTest, UniqueIDs) {
...
TEST_F(PresearchTemplateURLPrepopulateDataTest, ProvidersFromPrepopulated) {
  size_t default_index;
  std::vector<std::unique_ptr<TemplateURLData>> t_urls =
      TemplateURLPrepopulateData::GetPrepopulatedEngines(&prefs_,
                                                         &default_index);
...
TEST_F(PresearchTemplateURLPrepopulateDataTest, DefaultSearchProvidersForUSA) {
  CheckForCountry('U', 'S', "Google");
}

TEST_F(PresearchTemplateURLPrepopulateDataTest, DefaultSearchProvidersForGermany) {
  CheckForCountry('D', 'E', "DuckDuckGo");
}

TEST_F(PresearchTemplateURLPrepopulateDataTest, DefaultSearchProvidersForFrance) {
  CheckForCountry('F', 'R', "Qwant");
}

TEST_F(PresearchTemplateURLPrepopulateDataTest,
...
TEST_F(PresearchTemplateURLPrepopulateDataTest, DefaultSearchProvidersForIreland) {
