...
#define PRESEARCH_KEY_STORAGE_KWALLET_GET_KEY_IMPL                               \
  true ? kwallet_dbus_->ReadPassword(handle_, GetFolderName(), GetKeyName(), \
                                     app_name_, &password)                   \
       :

#define PRESEARCH_KEY_STORAGE_KWALLET_INIT_FOLDER        \
  handle_, GetFolderName(), app_name_, &has_folder); \
  if (false) Dummy(

#include "../../../../components/os_crypt/key_storage_kwallet.cc"
#undef PRESEARCH_KEY_STORAGE_KWALLET_INIT_FOLDER
#undef PRESEARCH_KEY_STORAGE_KWALLET_GET_KEY_IMPL
