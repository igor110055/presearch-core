...
constexpr SkColor kPresearchBrandColor = SkColorSetRGB(0xff, 0x76, 0x54);

class PresearchTextButtonHighlightPathGenerator
    : public views::HighlightPathGenerator {
 public:
  PresearchTextButtonHighlightPathGenerator() = default;

  // HighlightPathGenerator
  SkPath GetHighlightPath(const views::View* view) override;

 private:
  DISALLOW_COPY_AND_ASSIGN(PresearchTextButtonHighlightPathGenerator);
};
...
MdTextButton::MdTextButton(PressedCallback callback,
  ...
  views::HighlightPathGenerator::Install(
      this, std::make_unique<PresearchTextButtonHighlightPathGenerator>());
}
...
void MdTextButton::OnPaintBackground(gfx::Canvas* canvas) {
  // Set presearch-style hover colors
...
std::unique_ptr<InkDrop> MdTextButton::CreateInkDrop() {
  ...
  // is handled by the OnPaintBackground and presearch-style doesn't
...
void MdTextButton::UpdateColors() {
  ...
  if (theme->GetPlatformHighContrastColorScheme() !=
    ...
    SetTextColor(ButtonState::STATE_HOVERED, kPresearchBrandColor);
    SetTextColor(ButtonState::STATE_PRESSED, kPresearchBrandColor);
  }
  ...
  if (GetState() == ButtonState::STATE_PRESSED
    ...
    // The only thing that differs for Presearch is the stroke color
    SkColor stroke_color = kPresearchBrandColor;
    SetBackground(
...
SkPath PresearchTextButtonHighlightPathGenerator::GetHighlightPath(
