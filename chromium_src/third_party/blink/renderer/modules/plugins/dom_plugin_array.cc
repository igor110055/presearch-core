...
void FarblePlugins(DOMPluginArray* owner,
  ...
  if (!frame || !frame->GetContentSettingsClient())
    return;
  switch (frame->GetContentSettingsClient()->GetPresearchFarblingLevel()) {
    ...
    case PresearchFarblingLevel::BALANCED: {
      std::mt19937_64 prng = PresearchSessionCache::From(*(frame->DomWindow()))
      ...
      for (unsigned index = 0; index < dom_plugins->size(); index++) {
        ...
        if ((name == "Chrome PDF Plugin") || (name == "Chrome PDF Viewer")) {
          ...
          plugin->SetFilename(
              PresearchSessionCache::From(*(frame->DomWindow()))
...
#define PRESEARCH_DOM_PLUGINS_UPDATE_PLUGIN_DATA__RESET_PLUGIN_DATA \
  if (PluginData* data = GetPluginData())                       \
    data->ResetPluginData();

#define PRESEARCH_DOM_PLUGINS_UPDATE_PLUGIN_DATA__FARBLE_PLUGIN_DATA \
  presearch::FarblePlugins(this, data, &dom_plugins_);

#include "../../../../../../../third_party/blink/renderer/modules/plugins/dom_plugin_array.cc"

#undef PRESEARCH_DOM_PLUGIN_ARRAY_GET_PLUGIN_DATA
