...
#define PRESEARCH_AUDIOBUFFER_GETCHANNELDATA                                       \
  ...
  if (ExecutionContext* context = ExecutionContext::From(script_state)) {      \
    if (WebContentSettingsClient* settings =                                   \
      ...
      if (len > 0) {                                                           \
        ...
        presearch::AudioFarblingCallback audio_farbling_callback =                 \
            presearch::PresearchSessionCache::From(*context).GetAudioFarblingCallback( \
                settings);                                                     \
...
#define PRESEARCH_AUDIOBUFFER_COPYFROMCHANNEL                                    \
  if (ExecutionContext* context = ExecutionContext::From(script_state)) {    \
    if (WebContentSettingsClient* settings =                                 \
            presearch::GetContentSettingsClientFor(context)) {                   \
      presearch::AudioFarblingCallback audio_farbling_callback =                 \
...
#undef PRESEARCH_AUDIOBUFFER_GETCHANNELDATA
#undef PRESEARCH_AUDIOBUFFER_COPYFROMCHANNEL
