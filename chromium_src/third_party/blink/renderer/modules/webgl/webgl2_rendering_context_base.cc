...
#include "presearch/components/content_settings/renderer/presearch_content_settings_agent_impl_helper.h"
...
ScriptValue FarbleGLIntParameter(WebGL2RenderingContextBase* owner,
  ...
  if (value > 0) {
    std::mt19937_64 prng =
        presearch::PresearchSessionCache::From(*ExecutionContext::From(script_state))
...
#define PRESEARCH_WEBGL2_RENDERING_CONTEXT_BASE                                 \
  if (canvas() && !AllowFingerprinting(canvas()->GetDocument().GetFrame())) \
    return ScriptValue::CreateNull(script_state->GetIsolate());

#define PRESEARCH_WEBGL2_RENDERING_CONTEXT_BASE_GETPARAMETER                      \
  if (ExecutionContext* context = ExecutionContext::From(script_state)) {     \
    if (WebContentSettingsClient* settings =                                  \
      ...
      if (settings->GetPresearchFarblingLevel() == PresearchFarblingLevel::MAXIMUM) { \
        switch (pname) {                                                      \
      ...
      } else if (settings->GetPresearchFarblingLevel() ==                         \
                 PresearchFarblingLevel::BALANCED) {                              \
        switch (pname) {                                                      \
          case GL_MAX_VERTEX_UNIFORM_COMPONENTS:                              \
...
#undef PRESEARCH_WEBGL2_RENDERING_CONTEXT_BASE
#undef PRESEARCH_WEBGL2_RENDERING_CONTEXT_BASE_GETPARAMETER
