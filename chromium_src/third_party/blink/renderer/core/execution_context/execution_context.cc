...
const char kPresearchSessionToken[] = "presearch_session_token";
const char PresearchSessionCache::kSupplementName[] = "PresearchSessionCache";
...
PresearchSessionCache::PresearchSessionCache(ExecutionContext& context)
  ...
  DCHECK(cmd_line->HasSwitch(kPresearchSessionToken));
  base::StringToUint64(cmd_line->GetSwitchValueASCII(kPresearchSessionToken),
                       &session_key_);
...
PresearchSessionCache& PresearchSessionCache::From(ExecutionContext& context) {
  PresearchSessionCache* cache =
      Supplement<ExecutionContext>::From<PresearchSessionCache>(context);
  if (!cache) {
    cache = MakeGarbageCollected<PresearchSessionCache>(context);
    ProvideTo(context, cache);
  }
  return *cache;
}

AudioFarblingCallback PresearchSessionCache::GetAudioFarblingCallback(
    blink::WebContentSettingsClient* settings) {
  if (farbling_enabled_ && settings) {
    switch (settings->GetPresearchFarblingLevel()) {
      case PresearchFarblingLevel::OFF: {
        break;
      }
      case PresearchFarblingLevel::BALANCED: {
      ...
      case PresearchFarblingLevel::MAXIMUM: {
...
void PresearchSessionCache::PerturbPixels(blink::WebContentSettingsClient* settings,
...
void PresearchSessionCache::PerturbPixelsInternal(const unsigned char* data,
                                              size_t size) {
  if (!data || size == 0)
    return;
...
WTF::String PresearchSessionCache::GenerateRandomString(std::string seed,
                                                    wtf_size_t length) {
  uint8_t key[32];
  crypto::HMAC h(crypto::HMAC::SHA256);
...
WTF::String PresearchSessionCache::FarbledUserAgent(WTF::String real_user_agent) {
...
std::mt19937_64 PresearchSessionCache::MakePseudoRandomGenerator() {
  uint64_t seed = *reinterpret_cast<uint64_t*>(domain_key_);
