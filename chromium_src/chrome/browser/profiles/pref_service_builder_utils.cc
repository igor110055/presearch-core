...
void RegisterProfilePrefs(bool is_signin_profile,
  ...
  presearch_ads::AdsService::RegisterProfilePrefs(registry);
  presearch_rewards::RewardsService::RegisterProfilePrefs(registry);
  ...
  registry->SetDefaultPrefValue(prefs::kSigninAllowedOnNextStartup,
                                base::Value(false));
#if defined(OS_LINUX)
  // Use presearch theme by default instead of gtk theme.
...
std::unique_ptr<sync_preferences::PrefServiceSyncable> CreatePrefService(
  ...
  if (presearch::IsSessionProfilePath(path)) {
    base::FilePath original_path = presearch::GetParentProfilePath(path);
