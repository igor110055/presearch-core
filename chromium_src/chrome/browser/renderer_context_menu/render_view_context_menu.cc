...
#include "presearch/browser/ipfs/import/ipfs_import_controller.h"
#include "presearch/browser/profiles/profile_util.h"
#include "presearch/browser/renderer_context_menu/presearch_spelling_options_submenu_observer.h"
...
GURL GetSelectionNavigationURL(Profile* profile, const base::string16& text) {
  ...
  AutocompleteClassifier classifier(
      ...
      std::make_unique<PresearchAutocompleteSchemeClassifier>(profile));
...
base::OnceCallback<void(PresearchRenderViewContextMenu*)>*
PresearchGetMenuShownCallback() {
...
void RenderViewContextMenu::RegisterMenuShownCallbackForTesting(
  ...
  *PresearchGetMenuShownCallback() = std::move(cb);
}

#define PRESEARCH_APPEND_SEARCH_PROVIDER \
...
#define SpellingOptionsSubMenuObserver PresearchSpellingOptionsSubMenuObserver
...
#undef RenderViewContextMenu
#undef PRESEARCH_APPEND_SEARCH_PROVIDER

PresearchRenderViewContextMenu::PresearchRenderViewContextMenu(
...
bool PresearchRenderViewContextMenu::IsCommandIdEnabled(int id) const {
  switch (id) {
#if BUILDFLAG(IPFS_ENABLED)
...
#if BUILDFLAG(ENABLE_TOR)
      if (presearch::IsTorDisabledForProfile(GetProfile()))
        return false;
...
#if BUILDFLAG(IPFS_ENABLED)
void PresearchRenderViewContextMenu::ExecuteIPFSCommand(int id, int event_flags) {
...
void PresearchRenderViewContextMenu::ExecuteCommand(int id, int event_flags) {
  switch (id) {
#if BUILDFLAG(IPFS_ENABLED)
    case IDC_CONTENT_CONTEXT_IMPORT_IPFS_PAGE:
    case IDC_CONTENT_CONTEXT_IMPORT_IMAGE_IPFS:
...
void PresearchRenderViewContextMenu::AddSpellCheckServiceItem(bool is_checked) {
  // Call our implementation, not the one in the base class.
  // Assumption:
  // Use of spelling service is disabled in Presearch profile preferences.
...
// static
void PresearchRenderViewContextMenu::AddSpellCheckServiceItem(
    ui::SimpleMenuModel* menu,
    bool is_checked) {
  // Suppress adding "Spellcheck->Ask Presearch for suggestions" item.
}

#if BUILDFLAG(IPFS_ENABLED)
bool PresearchRenderViewContextMenu::IsIPFSCommandIdEnabled(int command) const {
...
void PresearchRenderViewContextMenu::SeIpfsIconAt(int index) {
...
void PresearchRenderViewContextMenu::BuildIPFSMenu() {
  if (!ipfs::IsIpfsMenuEnabled(browser_context_))
    return;
  int index =
...
void PresearchRenderViewContextMenu::InitMenu() {
  RenderViewContextMenu_Chromium::InitMenu();

#if BUILDFLAG(ENABLE_TOR) || !BUILDFLAG(ENABLE_PRESEARCH_TRANSLATE_GO)
  int index = -1;
#endif
#if BUILDFLAG(ENABLE_TOR)
