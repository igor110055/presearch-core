...
const char kPresearchReferralCode[] = "presearch-referral-code";
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE prev_instance,
  ...
  if (!return_code) {
    ...
    if (cmd_line.HasSwitch(kPresearchReferralCode)) {
      const std::string referral_code =
          cmd_line.GetSwitchValueASCII(kPresearchReferralCode);
