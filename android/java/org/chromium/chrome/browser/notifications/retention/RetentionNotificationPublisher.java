...
public class RetentionNotificationPublisher extends BroadcastReceiver {
    private static final String NOTIFICATION_CHANNEL_NAME = "presearch";
        ...
        if (action != null && action.equals(RETENTION_NOTIFICATION_ACTION)) {
            if (presearchActivity != null) {
                ...
                case RetentionNotificationUtil.EVERY_SUNDAY:
                    presearchActivity.checkForPresearchStats();
                    break;
                ...
                case RetentionNotificationUtil.PRESEARCH_STATS_TIME:
                    if (presearchActivity.getActivityTab() != null 
                        && presearchActivity.getActivityTab().getUrlString() != null
                        && !UrlUtilities.isNTPUrl(presearchActivity.getActivityTab().getUrlString())) {
                        presearchActivity.getTabCreator(false).launchUrl(UrlConstants.NTP_URL, TabLaunchType.FROM_CHROME_UI);
                    }
                    break;
            ...
            case RetentionNotificationUtil.DEFAULT_BROWSER_3:
                if (!PresearchSetDefaultBrowserNotificationService.isPresearchSetAsDefaultBrowser(context)) {
                    createNotification(context, intent);
                }
                break;
            case RetentionNotificationUtil.DAY_10:
            ...
            case RetentionNotificationUtil.DAY_35:
                ...
                break;
            case RetentionNotificationUtil.EVERY_SUNDAY:
                if (OnboardingPrefManager.getInstance().isPresearchStatsNotificationEnabled()) {
                    createNotification(context, intent);
                }
                break;
            }
