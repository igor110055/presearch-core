...
public class PresearchNotificationPlatformBridge extends NotificationPlatformBridge {
    private static final int[] EMPTY_VIBRATION_PATTERN = new int[0];

    private @NotificationType int mNotificationType;

    @CalledByNative
    private static PresearchNotificationPlatformBridge create(long nativeNotificationPlatformBridge) {
        if (sInstance != null) {
            throw new IllegalStateException(
                "There must only be a single NotificationPlatformBridge.");
        }

        sInstance = new PresearchNotificationPlatformBridge(nativeNotificationPlatformBridge);
        return (PresearchNotificationPlatformBridge) sInstance;
    }

    private PresearchNotificationPlatformBridge(long nativeNotificationPlatformBridge) {
