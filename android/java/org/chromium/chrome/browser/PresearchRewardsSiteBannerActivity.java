...
public class PresearchRewardsSiteBannerActivity extends Activity implements
        PresearchRewardsHelper.LargeIconReadyCallback, PresearchRewardsObserver {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.presearch_rewards_site_banner);

        isAnonWallet = PresearchRewardsHelper.isAnonWallet();
        ...
        radio_tip_amount[0].setTextOff(PresearchRewardsHelper.ONE_BAT_TEXT);
        radio_tip_amount[0].setTextOn(PresearchRewardsHelper.ONE_BAT_TEXT);
        radio_tip_amount[0].setChecked(false);

        radio_tip_amount[1] = findViewById(R.id.five_bat_option);
        radio_tip_amount[1].setTextOff(PresearchRewardsHelper.FIVE_BAT_TEXT);
        radio_tip_amount[1].setTextOn(PresearchRewardsHelper.FIVE_BAT_TEXT);
        radio_tip_amount[1].setChecked(true);

        radio_tip_amount[2] = findViewById(R.id.ten_bat_option);
        radio_tip_amount[2].setTextOff(PresearchRewardsHelper.TEN_BAT_TEXT);
        radio_tip_amount[2].setTextOn(PresearchRewardsHelper.TEN_BAT_TEXT);
        ...
        String publisherName = mPresearchRewardsNativeWorker.GetPublisherName(currentTabId_);
        ...
        mIconFetcher = new PresearchRewardsHelper(currentActiveTab);
        mIconFetcher.retrieveLargeIcon(favicon_url, this);

        double balance = .0;
        PresearchRewardsBalance rewards_balance = mPresearchRewardsNativeWorker.GetWalletBalance();
        ...
        String walletAmount = df.format(balance) + " " + PresearchRewardsHelper.BAT_TEXT;

        ((TextView)findViewById(R.id.wallet_amount_text)).setText(walletAmount);

        double usdValue = mPresearchRewardsNativeWorker.GetWalletRate();
        ...
        View.OnClickListener send_tip_clicker = new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (true == enough_funds) {
                    ...
                    mPresearchRewardsNativeWorker.Donate(mPresearchRewardsNativeWorker.GetPublisherId(currentTabId_),
                        amount, monthly_bool);
                    Intent intent = new Intent(getApplicationContext(), PresearchRewardsDonationSentActivity.class);
                    intent.putExtra(PresearchRewardsSiteBannerActivity.TAB_ID_EXTRA, currentTabId_);
                    intent.putExtra(PresearchRewardsSiteBannerActivity.TIP_AMOUNT_EXTRA, amount);
                    intent.putExtra(PresearchRewardsSiteBannerActivity.TIP_MONTHLY_EXTRA, monthly_bool);
        ...
        String part1 = String.format(getResources().getString(R.string.presearch_ui_not_enough_tokens), isAnonWallet ? getResources().getString(R.string.point) : getResources().getString(R.string.token));
        String part2 = getResources().getString(R.string.presearch_ui_please);
        String part3 = getResources().getString(R.string.presearch_ui_add_funds);
        ...
        sb.append(part3);
        sb.append("</u>.");
        Spanned toInsert = PresearchRewardsHelper.spannedFromHtmlString(sb.toString());
        ...
        */
        ...
        @PublisherStatus int pubStatus =
                mPresearchRewardsNativeWorker.GetPublisherStatus(currentTabId_);
        ...
        if (mPresearchRewardsNativeWorker.IsCurrentPublisherInRecurrentDonations(mPresearchRewardsNativeWorker.GetPublisherId(currentTabId_))) {
            ...
            PresearchRewardsHelper.expandTouchArea (monthly_layout, monthly_chk, TOUCH_PADDING_MONTHLY);
        }
        ...
        vto.addOnGlobalLayoutListener (new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                int newY = top - PresearchRewardsHelper.dp2px( PUBLISHER_ICON_SIDE_LEN / 2 + ACTIVITY_HEADER_PADDING);
                int minTopPosition = PresearchRewardsHelper.dp2px(ACTIVITY_HEADER_CONTENT_SIDE_MARGIN);
    ...
    private void setPublisherNoteText(@PublisherStatus int pubStatus) {
        String note_part1 = "";
        if (pubStatus == PresearchRewardsPublisher.CONNECTED) {
            // show |presearch_ui_site_banner_connected_text| text if
            ...
            if (balance_obj != null) {
                double presearchFunds = ((balance_obj.mWallets.containsKey(PresearchRewardsBalance.WALLET_ANONYMOUS) && balance_obj.mWallets.get(PresearchRewardsBalance.WALLET_ANONYMOUS) != null) ? balance_obj.mWallets.get(PresearchRewardsBalance.WALLET_ANONYMOUS) : .0) +
                                    ((balance_obj.mWallets.containsKey(PresearchRewardsBalance.WALLET_BLINDED) && balance_obj.mWallets.get(PresearchRewardsBalance.WALLET_BLINDED) != null) ? balance_obj.mWallets.get(PresearchRewardsBalance.WALLET_BLINDED) : .0);
                if (presearchFunds <= 0) {
                    note_part1 = getResources().getString(R.string.presearch_ui_site_banner_connected_text);
                }
            }
        }
        else if (pubStatus == PresearchRewardsPublisher.NOT_VERIFIED) {
            note_part1 = getResources().getString(R.string.presearch_ui_site_banner_notice_text);
        }
        ...
        if (!TextUtils.isEmpty(note_part1)) {
            ...
            sb1.append("</font></br>");
            Spanned toInsert = PresearchRewardsHelper.spannedFromHtmlString(sb1.toString());
            ...
            not_verified_warning_text.setOnTouchListener(new View.OnTouchListener() {
                ...
                public boolean onTouch(View view, MotionEvent motionEvent) {
                    ...
                    if (motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
                        ...
                        if (PresearchRewardsHelper.subtextAtOffset(full_note_str, note_part2, offset) ){
                            event_consumed = true;
                            Intent intent = new Intent();
                            intent.putExtra(PresearchActivity.OPEN_URL, PresearchActivity.REWARDS_LEARN_MORE_URL);
    ...
    public void onMonthlyCheckboxClicked(View view) {
        ...
        int string_id = (checked)? R.string.presearch_ui_do_monthly : R.string.presearch_ui_send_tip;
    ...
    public void OnRewardsParameters(int errorCode) {
        if (errorCode == 0) {
            if (mPresearchRewardsNativeWorker != null) {
                double balance = .0;
                PresearchRewardsBalance rewardsBalance = mPresearchRewardsNativeWorker.GetWalletBalance();
