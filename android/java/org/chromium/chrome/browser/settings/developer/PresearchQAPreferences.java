 ...
 */
public class PresearchQAPreferences extends PresearchPreferenceFragment
    ...
    private PresearchDbUtil mDbUtil;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mDbUtil = PresearchDbUtil.getInstance();
                    ...
                    public void onClick(DialogInterface dialog, int button) {
                        if (button == AlertDialog.BUTTON_POSITIVE) {
                            ...
                            if (!restorePhrase.isEmpty()) {
                                PresearchRewardsNativeWorker.getInstance().RecoverWallet(
                                    restorePhrase);
                            }
                        }
                    }
    ...
    @Override
    public void onStart() {
        PresearchRewardsNativeWorker.getInstance().AddObserver(this);
        super.onStart();
    }

    @Override
    public void onStop() {
        PresearchRewardsNativeWorker.getInstance().RemoveObserver(this);
        super.onStop();
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (PREF_USE_REWARDS_STAGING_SERVER.equals(preference.getKey())) {
            PresearchRewardsNativeWorker.getInstance().ResetTheWholeState();
    ...
    private void checkQACode() {
        ...
        DialogInterface.OnClickListener onClickListener = new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int button) {
                if (button != AlertDialog.BUTTON_POSITIVE
                        || !input.getText().toString().equals(PresearchConfig.DEVELOPER_OPTIONS_CODE)) {
                    getActivity().finish();
                }
            }
        };
    ...
    private void enableMaximumAdsNumber(boolean enable) {
        if (enable) {
            // Save current values
            int adsPerHour = PresearchRewardsNativeWorker.getInstance().GetAdsPerHour();
            SharedPreferences sharedPreferences = ContextUtils.getAppSharedPreferences();
            SharedPreferences.Editor sharedPreferencesEditor = sharedPreferences.edit();
            sharedPreferencesEditor.putInt(QA_ADS_PER_HOUR, adsPerHour);
            sharedPreferencesEditor.apply();
            // Set max value
            PresearchRewardsNativeWorker.getInstance().SetAdsPerHour(MAX_ADS);
            return;
        }
        // Set saved values
        int adsPerHour = ContextUtils.getAppSharedPreferences().getInt(
                             QA_ADS_PER_HOUR, DEFAULT_ADS_PER_HOUR);
        PresearchRewardsNativeWorker.getInstance().SetAdsPerHour(adsPerHour);
    }
    ...
    public void OnResetTheWholeState(boolean success) {
        if (success) {
            SharedPreferences sharedPreferences = ContextUtils.getAppSharedPreferences();
            SharedPreferences.Editor sharedPreferencesEditor = sharedPreferences.edit();
            sharedPreferencesEditor.putBoolean(PresearchRewardsPanelPopup.PREF_GRANTS_NOTIFICATION_RECEIVED, false);
            sharedPreferencesEditor.putBoolean(PresearchRewardsPanelPopup.PREF_WAS_PRESEARCH_REWARDS_TURNED_ON, false);
            sharedPreferencesEditor.apply();

            PresearchPrefServiceBridge.getInstance().setSafetynetCheckFailed(false);
            PresearchPrefServiceBridge.getInstance().setUseRewardsStagingServer(mUseRewardsStagingServer);
            PresearchRewardsHelper.setRewardsEnvChange(true);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CHOOSE_FILE_FOR_IMPORT_REQUEST_CODE && resultCode == Activity.RESULT_OK
            ...
            try {
                ...
                Log.e(PresearchDbUtil.getTag(), "Error on preparing database file: " + e);
                return;
            }
            requestRestart(true);
        }
