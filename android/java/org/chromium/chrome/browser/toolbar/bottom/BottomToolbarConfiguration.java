...
public class BottomToolbarConfiguration {
    ...
    public static boolean isBottomToolbarEnabled() {
        ...
        if (sharedPreferences.getBoolean(
                    PresearchPreferenceKeys.PRESEARCH_BOTTOM_TOOLBAR_SET_KEY, false)) {
            return sharedPreferences.getBoolean(
                    PresearchPreferenceKeys.PRESEARCH_BOTTOM_TOOLBAR_ENABLED_KEY, true);
        } else {
            SharedPreferencesManager.getInstance().writeBoolean(
                    PresearchPreferenceKeys.PRESEARCH_BOTTOM_TOOLBAR_SET_KEY, true);
            boolean enable = true;
            if (isSmallScreen()) {
                enable = false;
            }
            SharedPreferencesManager.getInstance().writeBoolean(
                    PresearchPreferenceKeys.PRESEARCH_BOTTOM_TOOLBAR_ENABLED_KEY, enable);
