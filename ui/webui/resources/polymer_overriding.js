...
function addPresearchBehaviors(moduleName, component) {
...
function addPresearchProperties(moduleName, component) {
...
const allPresearchTemplateModificationsMap = {}

function addPresearchTemplateModifications(moduleName, component, modifyFn) {
...
const styleOverridePrefix = 'presearch-override-style-'

function addPresearchStyleOverride(moduleName, component) {
...
export function RegisterPolymerComponentProperties(propertiesMap) {
  ...
  for (const componentName in propertiesMap) {
    if (componentPropertyModifications[componentName]) {
        addPresearchProperties(componentName, componentPropertyModifications[componentName])
    }
  }
}
...
export function RegisterPolymerTemplateModifications(modificationsMap) {
  ...
  for (const componentName in modificationsMap) {
    ...
    addPresearchTemplateModifications(componentName, existingComponent, modifyFn)
  }
  Object.assign(allPresearchTemplateModificationsMap, awaitingComponentModifications)
}
...
export async function RegisterStyleOverride(componentName, styleTemplate) {
  ...
  const existingComponent = window.customElements.get(componentName)
  if (existingComponent) {
    addPresearchStyleOverride(componentName, existingComponent)
  } else {
...
export function OverrideIronIcons(iconSetName, overridingIconSetName, iconOverrides) {
  ...
  for (const chromiumIconName in iconOverrides) {
    ...
    if (!chromiumIcon) {
      console.error(`[presearch overrides] Could not find chromium icon '${chromiumIconName}' in iconset '${iconSetName}' for replacement!`)
      continue
    }
    const presearchIconName = iconOverrides[chromiumIconName]
    const presearchIcon = overrideIconSet.querySelector(`#${presearchIconName}`)
    if (!presearchIcon) {
      console.error(`[presearch overrides] Could not find presearch icon '${presearchIconName}' in iconset '${overridingIconSetName}' for replacement!`)
      continue
    }
    // replace
    ...
    while (presearchIcon.firstChild) {
      chromiumIcon.appendChild(presearchIcon.firstChild)
    }
  }
...
function PerformPresearchModifications(name, component) {
  if (debug) {
    console.debug(`Polymer component registering: ${name}`, component)
  }
  addPresearchBehaviors(name, component)
  addPresearchProperties(name, component)
  const templateModifyFn = allPresearchTemplateModificationsMap[name]
  if (templateModifyFn) {
    addPresearchTemplateModifications(name, component, templateModifyFn)
    delete allPresearchTemplateModificationsMap[name]
  }
  if (moduleNamesWithStyleOverrides.includes(name)) {
    addPresearchStyleOverride(name, component)
  }
}
...
Polymer.Class = function (info, mixin) {
  ...
  PerformPresearchModifications(name, info)
  return oldClass(info, mixin)
}
