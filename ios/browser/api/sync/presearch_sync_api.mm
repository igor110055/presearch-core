...
#import "presearch/ios/browser/api/sync/presearch_sync_api.h"
...
@interface PresearchSyncDeviceObserver : NSObject {
  std::unique_ptr<PresearchSyncDeviceTracker> _device_observer;
}
@end

@implementation PresearchSyncDeviceObserver

- (instancetype)initWithDeviceInfoTracker:(syncer::DeviceInfoTracker*)tracker
                                 callback:(void (^)())onDeviceInfoChanged {
  if ((self = [super init])) {
    _device_observer =
        std::make_unique<PresearchSyncDeviceTracker>(tracker, onDeviceInfoChanged);
  }
  return self;
}
@end

@interface PresearchSyncServiceObserver : NSObject {
  std::unique_ptr<PresearchSyncServiceTracker> _service_tracker;
}
@end

@implementation PresearchSyncServiceObserver

- (instancetype)
    initWithProfileSyncService:(syncer::ProfileSyncService*)profileSyncService
                      callback:(void (^)())onSyncServiceStateChanged {
  if ((self = [super init])) {
    _service_tracker = std::make_unique<PresearchSyncServiceTracker>(
...
@interface PresearchSyncAPI () {
  std::unique_ptr<PresearchSyncWorker> _worker;
  ChromeBrowserState* _chromeBrowserState;
}
@end

@implementation PresearchSyncAPI

+ (instancetype)sharedSyncAPI {
  static PresearchSyncAPI* instance = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    instance = [[PresearchSyncAPI alloc] init];
  });
  return instance;
}
...
- (instancetype)init {
  if ((self = [super init])) {
    ...
    _worker.reset(new PresearchSyncWorker(_chromeBrowserState));
  }
  return self;
}
...
- (id)createSyncDeviceObserver:(void (^)())onDeviceInfoChanged {
  ...
  return [[PresearchSyncDeviceObserver alloc]
...
- (id)createSyncServiceObserver:(void (^)())onSyncServiceStateChanged {
  auto* service =
      ProfileSyncServiceFactory::GetAsProfileSyncServiceForBrowserState(
          _chromeBrowserState);
  return [[PresearchSyncServiceObserver alloc]
