...
#include "presearch/ios/browser/api/bookmarks/presearch_bookmarks_observer.h"
...
@interface PresearchBookmarksAPI () {
  bookmarks::BookmarkModel* bookmark_model_;    // NOT OWNED
  BookmarkUndoService* bookmark_undo_service_;  // NOT OWNED
}
@end

@implementation PresearchBookmarksAPI
+ (instancetype)sharedBookmarksAPI {
  static PresearchBookmarksAPI* instance = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    instance = [[PresearchBookmarksAPI alloc] init];
  });
  return instance;
}
