...
#import "presearch/ios/app/presearch_core_main.h"
...
@interface PresearchCoreMain () {
  std::unique_ptr<PresearchWebClient> _webClient;
  std::unique_ptr<PresearchMainDelegate> _delegate;
  std::unique_ptr<web::WebMain> _webMain;
  ChromeBrowserState* _mainBrowserState;
}
@end

@implementation PresearchCoreMain
...
- (instancetype)initWithSyncServiceURL:(NSString*)syncServiceURL {
  if ((self = [super init])) {
    ...
    _webClient.reset(new PresearchWebClient());
    web::SetWebClient(_webClient.get());

    _delegate.reset(new PresearchMainDelegate());
