...
async function applyPatches() {
  ...
  const coreRepoPath = config.presearchCoreDir
...
const util = {
  ...
  updateBranding: () => {
    ...
    const presearchComponentsDir = path.join(config.presearchCoreDir, 'components')
    const chromeAppDir = path.join(config.srcDir, 'chrome', 'app')
    const presearchAppDir = path.join(config.presearchCoreDir, 'app')
    const chromeBrowserResourcesDir = path.join(config.srcDir, 'chrome', 'browser', 'resources')
    const presearchBrowserResourcesDir = path.join(config.presearchCoreDir, 'browser', 'resources')
    const presearchAppVectorIconsDir = path.join(config.presearchCoreDir, 'vector_icons', 'chrome', 'app')
    const chromeAndroidJavaStringsTranslationsDir = path.join(config.srcDir, 'chrome', 'browser', 'ui', 'android', 'strings', 'translations')
    const presearchAndroidJavaStringsTranslationsDir = path.join(config.presearchCoreDir, 'browser', 'ui', 'android', 'strings', 'translations')

    let fileMap = new Set();
    const autoGeneratedPresearchToChromiumMapping = Object.assign({}, l10nUtil.getAutoGeneratedPresearchToChromiumMapping())
    // The following 3 entries we map to the same name, not the chromium equivalent name for copying back
    autoGeneratedPresearchToChromiumMapping[path.join(presearchAppDir, 'presearch_strings.grd')] = path.join(chromeAppDir, 'presearch_strings.grd')
    autoGeneratedPresearchToChromiumMapping[path.join(presearchAppDir, 'settings_presearch_strings.grdp')] = path.join(chromeAppDir, 'settings_presearch_strings.grdp')
    autoGeneratedPresearchToChromiumMapping[path.join(presearchComponentsDir, 'components_presearch_strings.grd')] = path.join(chromeComponentsDir, 'components_presearch_strings.grd')

    Object.entries(autoGeneratedPresearchToChromiumMapping).forEach(mapping => fileMap.add(mapping))

    // Copy xtb files for:
    // presearch/app/resources/chromium_strings*.xtb
    // presearch/app/resources/generated_resoruces*.xtb
    // presearch/components/strings/components_chromium_strings*.xtb
    // presearch/browser/ui/android/strings/translations/android_chrome_strings*.xtb
    fileMap.add([path.join(presearchAppDir, 'resources'), path.join(chromeAppDir, 'resources')])
    fileMap.add([path.join(presearchComponentsDir, 'strings'), path.join(chromeComponentsDir, 'strings')])
    fileMap.add([presearchAndroidJavaStringsTranslationsDir, chromeAndroidJavaStringsTranslationsDir])
    // By overwriting, we don't need to modify some grd files.
    fileMap.add([path.join(presearchAppDir, 'theme', 'presearch'), path.join(chromeAppDir, 'theme', 'presearch')])
    fileMap.add([path.join(presearchAppDir, 'theme', 'presearch'), path.join(chromeAppDir, 'theme', 'chromium')])
    fileMap.add([path.join(presearchAppDir, 'theme', 'default_100_percent', 'presearch'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'presearch')])
    fileMap.add([path.join(presearchAppDir, 'theme', 'default_200_percent', 'presearch'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'presearch')])
    fileMap.add([path.join(presearchAppDir, 'theme', 'default_100_percent', 'presearch'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'chromium')])
    fileMap.add([path.join(presearchAppDir, 'theme', 'default_200_percent', 'presearch'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'chromium')])
    fileMap.add([path.join(presearchAppDir, 'theme', 'default_100_percent', 'common'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'common')])
    fileMap.add([path.join(presearchAppDir, 'theme', 'default_200_percent', 'common'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'common')])
    fileMap.add([path.join(presearchComponentsDir, 'resources', 'default_100_percent'), path.join(chromeComponentsDir, 'resources', 'default_100_percent')])
    fileMap.add([path.join(presearchComponentsDir, 'resources', 'default_100_percent', 'presearch'), path.join(chromeComponentsDir, 'resources', 'default_100_percent', 'chromium')])
    fileMap.add([path.join(presearchComponentsDir, 'resources', 'default_200_percent'), path.join(chromeComponentsDir, 'resources', 'default_200_percent')])
    fileMap.add([path.join(presearchComponentsDir, 'resources', 'default_200_percent', 'presearch'), path.join(chromeComponentsDir, 'resources', 'default_200_percent', 'chromium')])
    fileMap.add([path.join(presearchAppVectorIconsDir, 'vector_icons', 'presearch'), path.join(chromeAppDir, 'vector_icons', 'presearch')])
    // Copy chrome-logo-faded.png for replacing chrome logo of welcome page with presearch's on Win8.
    fileMap.add([path.join(presearchBrowserResourcesDir, 'chrome-logo-faded.png'), path.join(chromeBrowserResourcesDir, 'chrome-logo-faded.png')])
    fileMap.add([path.join(presearchBrowserResourcesDir, 'downloads', 'images', 'incognito_marker.svg'), path.join(chromeBrowserResourcesDir, 'downloads', 'images', 'incognito_marker.svg')])
    fileMap.add([path.join(presearchBrowserResourcesDir, 'settings', 'images'), path.join(chromeBrowserResourcesDir, 'settings', 'images')])
    fileMap.add([path.join(presearchBrowserResourcesDir, 'signin', 'profile_picker', 'images'), path.join(chromeBrowserResourcesDir, 'signin', 'profile_picker', 'images')])
    // Copy to make our ${branding_path_component}_behaviors.cc
    fileMap.add([path.join(config.presearchCoreDir, 'chromium_src', 'chrome', 'installer', 'setup', 'presearch_behaviors.cc'),
                 path.join(config.srcDir, 'chrome', 'installer', 'setup', 'presearch_behaviors.cc')])
    // Replace webui CSS to use our fonts.
    fileMap.add([path.join(config.presearchCoreDir, 'ui', 'webui', 'resources', 'css', 'text_defaults_md.css'),
    ...
    if (process.platform === 'darwin') {
      ...
      // Each channel's app icons are stored in presearch/app/theme/$channel/app.icns.
      // With this copying, we don't need to modify chrome/BUILD.gn for this.
      const iconSource = path.join(presearchAppDir, 'theme', 'presearch', 'mac', config.channel, 'app.icns')
      const iconDest = path.join(chromeAppDir, 'theme', 'presearch', 'mac', 'app.icns')
      ...
      const brandingSource = path.join(presearchAppDir, 'theme', 'presearch', branding_file_name)
      const brandingDest = path.join(chromeAppDir, 'theme', 'presearch', 'BRANDING')
    ...
    if (config.targetOS === 'android') {
      ...
      if (config.channel === 'development') {
        androidIconSet = 'res_presearch_default'
      } else if (config.channel === '') {
        androidIconSet = 'res_presearch'
      } else if (config.channel === 'beta') {
        androidIconSet = 'res_presearch_beta'
      } else if (config.channel === 'dev') {
        androidIconSet = 'res_presearch_dev'
      } else if (config.channel === 'nightly') {
        androidIconSet = 'res_presearch_nightly'
      }

      const androidIconSource = path.join(presearchAppDir, 'theme', 'presearch', 'android', androidIconSet)
      const androidIconDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res_chromium')
      const androidIconBaseSource = path.join(presearchAppDir, 'theme', 'presearch', 'android', androidIconSet + '_base')
      const androidIconBaseDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res_chromium_base')
      const androidResSource = path.join(config.presearchCoreDir, 'android', 'java', 'res')
      const androidResDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res')
      const androidResTemplateSource = path.join(config.presearchCoreDir, 'android', 'java', 'res_template')
      const androidResTemplateDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res_template')
      const androidContentPublicResSource = path.join(config.presearchCoreDir, 'content', 'public', 'android', 'java', 'res')
      const androidContentPublicResDest = path.join(config.srcDir, 'content', 'public', 'android', 'java', 'res')
      const androidTouchtoFillResSource = path.join(config.presearchCoreDir, 'browser', 'touch_to_fill', 'android', 'internal', 'java', 'res')
      const androidTouchtoFillResDest = path.join(config.srcDir, 'chrome', 'browser', 'touch_to_fill', 'android', 'internal', 'java', 'res')
      const androidToolbarResSource = path.join(config.presearchCoreDir, 'browser', 'ui', 'android', 'toolbar', 'java', 'res')
      const androidToolbarResDest = path.join(config.srcDir, 'chrome', 'browser', 'ui', 'android', 'toolbar', 'java', 'res')
      const androidComponentsResSource = path.join(config.presearchCoreDir, 'components', 'browser_ui', 'widget', 'android', 'java', 'res')
      const androidComponentsResDest = path.join(config.srcDir, 'components', 'browser_ui', 'widget', 'android', 'java', 'res')

      // Mapping for copying Presearch's Android resource into chromium folder.
  ...
  updateOmahaMidlFiles: () => {
    ...
    const srcDir = path.join(config.presearchCoreDir, 'win_build_output', 'midl', 'google_update')
  ...
  signWinBinaries: () => {
    ...
    fs.copySync(path.join(config.outputDir, 'presearch.exe'), path.join(dir, 'presearch.exe'));
    fs.copySync(path.join(config.outputDir, 'chrome.dll'), path.join(dir, 'chrome.dll'));

    util.run('python', [path.join(config.presearchCoreDir, 'script', 'sign_binaries.py'), '--build_dir=' + dir])
  },
  ...
  generateWidevineSigFiles: () => {
    ...
    util.run('python', [sig_generator, '--input_file=' + path.join(src_dir, 'presearch.exe'),
        '--flags=1',
        '--certificate=' + cert,
        '--private_key=' + key,
        '--output_file=' + path.join(config.outputDir, 'presearch.exe.sig'),
  ...
  buildRedirectCCTool: () => {
    ...
    const redirectCCExe = path.join(config.presearchCoreDir, 'buildtools', 'win', 'redirect-cc', 'bin', 'redirect-cc.exe')
    ...
    const msBuildArgs = [
      path.join(config.presearchCoreDir, 'buildtools', 'win', 'redirect-cc', 'redirect-cc.sln'),
  ...
  generateXcodeWorkspace: (options = config.defaultOptions) => {
    ...
    const args = util.buildArgsToString(config.buildArgs())
    const genScript = path.join(config.presearchCoreDir, 'vendor', 'gn-project-generators', 'xcode.py')
  ...
  lint: (options = {}) => {
    ...
    cmd_options.cwd = config.presearchCoreDir
    cmd_options = mergeWithDefault(cmd_options)
    util.run('vpython', [path.join(config.presearchCoreDir, 'build', 'commands', 'scripts', 'lint.py'),
  ...
  format: (options = {}) => {
    ...
    cmd_options.cwd = config.presearchCoreDir
    cmd_options = mergeWithDefault(cmd_options)
    util.run('vpython', [path.join(config.presearchCoreDir, 'build', 'commands', 'scripts', 'format.py'), options.full ? '--full' : ''], cmd_options)
  },
  ...
  gclientSync: (forceReset = false, cleanup = false, presearchCoreRef = null, options = {}) => {
    ...
    if (forceReset || presearchCoreRef) {
      if (!presearchCoreRef) {
        // Use current branch (sync will then pull latest) or current exact hash
        // if we're not in a branch.
        presearchCoreRef = util.runGit(config.presearchCoreDir, ['rev-parse', '--abbrev-ref', 'HEAD'])
        if (presearchCoreRef === 'HEAD') {
          // get the rev hash if we're not in a branch
          presearchCoreRef = util.runGit(config.presearchCoreDir, ['rev-parse', 'HEAD'])
        }
      }

      args = args.concat(['--revision', 'src/presearch@' + presearchCoreRef])
      reset = true
    }
    ...
    if (forceReset) {
      ...
      if (cleanup) {
        // temporarily ignored until we can figure out how not to delete src/presearch in the process
        // args = args.concat(['-D'])
      }
    }
