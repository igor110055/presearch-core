...
// this is a huge hack because the npm config doesn't get passed through from presearch-browser .npmrc/package.json
...
const Config = function () {
  ...
  this.presearchCoreDir = path.join(this.srcDir, 'presearch')
  this.presearchCoreRepo = getNPMConfig(['projects', 'presearch-core', 'repository', 'url'])
  this.buildToolsDir = path.join(this.srcDir, 'build')
  this.resourcesDir = path.join(this.rootDir, 'resources')
  this.depotToolsDir = path.join(this.presearchCoreDir, 'vendor', 'depot_tools')
  this.defaultGClientFile = path.join(this.rootDir, '.gclient')
  this.gClientFile = process.env.PRESEARCH_GCLIENT_FILE || this.defaultGClientFile
  ...
  this.presearchGoogleApiKey = getNPMConfig(['presearch_google_api_key']) || 'AIzaSyAQfxPJiounkhOjODEO5ZieffeBv6yft2Q'
  this.googleApiEndpoint = getNPMConfig(['presearch_google_api_endpoint']) || 'https://www.googleapis.com/geolocation/v1/geolocate?key='
  ...
  this.presearchServicesKey = getNPMConfig(['presearch_services_key']) || ''
  this.infuraProjectId = getNPMConfig(['presearch_infura_project_id']) || ''
  ...
  this.presearchSyncEndpoint = getNPMConfig(['presearch_sync_endpoint']) || ''
  ...
  this.webcompatReportApiEndpoint = getNPMConfig(['webcompat_report_api_endpoint']) || 'https://webcompat.presearch.org/1/webcompat'
  this.chromePgoPhase = 0
  // this.buildProjects()
  this.presearchVersion = getNPMConfig(['version']) || '0.0.0'
  this.androidOverrideVersionName = this.presearchVersion
  this.releaseTag = this.presearchVersion.split('+')[0]
  ...
  this.presearchStatsApiKey = getNPMConfig(['presearch_stats_api_key']) || ''
  this.presearchStatsUpdaterUrl = getNPMConfig(['presearch_stats_updater_url']) || ''
  ...
  this.extraGnArgs = {}
  this.extraNinjaOpts = []
  this.presearchSafetyNetApiKey = getNPMConfig(['presearch_safetynet_api_key']) || ''
  this.presearchAndroidDeveloperOptionsCode = getNPMConfig(['presearch_android_developer_options_code']) || ''
  this.presearchAndroidKeystorePath = getNPMConfig(['presearch_android_keystore_path'])
  this.presearchAndroidKeystoreName = getNPMConfig(['presearch_android_keystore_name'])
  this.presearchAndroidKeystorePassword = getNPMConfig(['presearch_android_keystore_password'])
  this.presearchAndroidKeyPassword = getNPMConfig(['presearch_android_key_password'])
  this.presearchVariationsServerUrl = getNPMConfig(['presearch_variations_server_url']) || ''
}
...
Config.prototype.buildArgs = function () {
  const version = this.presearchVersion
  ...
  let args = {
    ...
    presearch_services_key: this.presearchServicesKey,
    root_extra_deps: ["//presearch"],
    ...
    presearch_channel: this.channel,
    presearch_google_api_key: this.presearchGoogleApiKey,
    presearch_google_api_endpoint: this.googleApiEndpoint,
    google_default_client_id: this.googleDefaultClientId,
    google_default_client_secret: this.googleDefaultClientSecret,
    presearch_infura_project_id: this.infuraProjectId,
    ...
    presearch_product_name: getNPMConfig(['presearch_product_name']) || "presearch-core",
    presearch_project_name: getNPMConfig(['presearch_project_name']) || "presearch-core",
    presearch_version_major: version_parts[0],
    presearch_version_minor: version_parts[1],
    presearch_version_build: version_parts[2],
    chrome_version_string: this.chromeVersion,
    chrome_version_major: chrome_version_parts[0],
    presearch_sync_endpoint: this.presearchSyncEndpoint,
    safebrowsing_api_endpoint: this.safeBrowsingApiEndpoint,
    presearch_variations_server_url: this.presearchVariationsServerUrl,
    ...
    presearch_stats_api_key: this.presearchStatsApiKey,
    presearch_stats_updater_url: this.presearchStatsUpdaterUrl,
  ...
  if (this.shouldSign()) {
    if (process.platform === 'darwin') {
      ...
      args.presearch_android_keystore_path = this.presearchAndroidKeystorePath
      args.presearch_android_keystore_name = this.presearchAndroidKeystoreName
      args.presearch_android_keystore_password = this.presearchAndroidKeystorePassword
      args.presearch_android_key_password = this.presearchAndroidKeyPassword
    }
  }
  ...
  if (this.targetOS === 'android') {
    ...
    if (!this.isOfficialBuild()) {
      ...
      args.chrome_public_manifest_package = 'com.presearch_beta'
    } else if (this.channel === 'dev') {
      args.chrome_public_manifest_package = 'com.presearch_dev'
    } else if (this.channel === 'nightly') {
      args.android_channel = 'canary'
      args.chrome_public_manifest_package = 'com.presearch_nightly'
    ...
    args.presearch_android_developer_options_code = this.presearchAndroidDeveloperOptionsCode
    args.presearch_safetynet_api_key = this.presearchSafetyNetApiKey
  ...
  if (this.targetOS === 'ios') {
    ...
    delete args.enable_hangout_services_extension
    delete args.presearch_google_api_endpoint
    delete args.presearch_google_api_key
    delete args.presearch_stats_api_key
    delete args.presearch_stats_updater_url
    delete args.presearch_infura_project_id
    ...
    delete args.presearch_variations_server_url
  }

  if (process.platform === 'win32') {
    args.cc_wrapper = path.join(this.srcDir, 'presearch', 'buildtools', 'win', 'redirect-cc', 'bin', 'redirect-cc.exe')
  } else {
    args.cc_wrapper = path.join(this.srcDir, 'presearch', 'script', 'redirect-cc.py')
  }
  return args
}
...
Config.prototype.shouldSign = function () {
  ...
  if (this.targetOS === 'android') {
    return this.presearchAndroidKeystorePath !== undefined
  }
...
Config.prototype.update = function (options) {
  ...
  if (options.presearch_google_api_key) {
    this.presearchGoogleApiKey = options.presearch_google_api_key
  }

  if (options.presearch_safetynet_api_key) {
    this.presearchSafetyNetApiKey = options.presearch_safetynet_api_key
  }

  if (options.presearch_google_api_endpoint) {
    this.googleApiEndpoint = options.presearch_google_api_endpoint
  }

  if (options.presearch_infura_project_id) {
  ...
  if (options.presearch_stats_api_key) {
    this.presearchStatsApiKey = options.presearch_stats_api_key
  }

  if (options.presearch_stats_updater_url) {
    this.presearchStatsUpdaterUrl = options.presearch_stats_updater_url
  }
  ...
  if (options.xcode_gen) {
    ...
    if (options.xcode_gen === 'ios') {
      this.xcode_gen_target = '//presearch/vendor/presearch-ios:*'
    } else {
...
Object.defineProperty(Config.prototype, 'defaultOptions', {
  get: function () {
    ...
    env = this.addPythonPathToEnv(env, path.join(this.srcDir, 'presearch', 'chromium_src', 'python_modules'))
    env = this.addPythonPathToEnv(env, path.join(this.srcDir, 'presearch', 'script'))
    env = this.addPythonPathToEnv(env, path.join(this.srcDir, 'tools', 'grit', 'grit', 'extern'))
    env = this.addPythonPathToEnv(env, path.join(this.srcDir, 'presearch', 'vendor', 'requests'))
    ...
    env.TARGET_ARCH = this.gypTargetArch // for presearch scripts
    env.GYP_MSVS_VERSION = env.GYP_MSVS_VERSION || '2017' // enable 2017
    if (this.channel != "") {
      env.PRESEARCH_CHANNEL = this.channel
    }
