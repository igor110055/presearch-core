...
def _OverrideVersionKey(plist, presearch_version):
  """ `minor.build` version string is used for update.
  When we begin to use the Major version component, Presearch version string will
  ...
  version_values = presearch_version.split('.')
...
def Main(argv):
  ...
  parser.add_option('--presearch_channel', dest='presearch_channel', action='store',
      type='string', default=None, help='Channel (beta, dev, nightly)')
  parser.add_option('--presearch_product_dir_name', dest='presearch_product_dir_name',
      action='store', type='string', default=None,
      help='Product directory name')
  parser.add_option('--presearch_feed_url', dest='presearch_feed_url', action='store',
      type='string', default=None, help='Target url for update feed')
  parser.add_option('--presearch_eddsa_key', dest='presearch_eddsa_key', action='store',
      type='string', default=None, help='Public EdDSA key for update')
  parser.add_option('--presearch_version', dest='presearch_version', action='store',
      type='string', default=None, help='presearch version string')
  ...
  if options.skip_signing:
    plist['KSChannelID'] = options.presearch_channel
  elif 'KSChannelID' in plist:
    # 'KSChannelID' is set at _modify_plists() of modification.py.
    del plist['KSChannelID']

  plist['CrProductDirName'] = options.presearch_product_dir_name

  if options.presearch_feed_url:
    plist['SUFeedURL'] = options.presearch_feed_url

  if options.presearch_eddsa_key:
    plist['SUPublicEDKey'] = options.presearch_eddsa_key

  _OverrideVersionKey(plist, options.presearch_version)
