...
# src/out/Release/Presearch_Browser_CHANNEL_Packaging), the signing directory is
# relative to that
...
from signing_helper import GetPresearchSigningConfig
...
def create_config(config_args, development, mac_provisioning_profile=None):
    ...
    config_class = GetPresearchSigningConfig(config_class, mac_provisioning_profile)
    return config_class(*config_args)


def NotarizePresearchDmgPkg(paths, config, dmg, pkg, outdir, signed):
    """
    Notarize Presearch .dmg and .pkg files.
    """
    uuids_to_path_map = {}
    for dist in config.distributions:
        ...
        for result in notarize.wait_for_results(
            ...
            presearch_path = uuids_to_path_map[result]
            notarize.staple(presearch_path)
...
def main():
    ...
    rc = NotarizePresearchDmgPkg(paths, config, args.dmg, args.pkg, args.outdir, args.signed)
    return rc
