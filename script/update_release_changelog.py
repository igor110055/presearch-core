...
def main():
    ...
    """
    Download the presearch-browser/CHANGELOG.md file, parse it and
    ...
    # PRESEARCH_REPO is defined in lib/helpers.py
    repo = GitHub(get_env_var('GITHUB_TOKEN')).repos(PRESEARCH_REPO)
...
def parse_args():
    desc = "Parse Presearch Browser changelog and add markdown to release notes for tag" \
        "\n\nRequires the following ENVIRONMENT VARIABLES be set:" \
        "\n\nPRESEARCH_GITHUB_TOKEN: Github token to update draft release if not published yet. "
    ...
    parser.add_argument('-t', '--tag',
                        help='Presearch version tag (allowed format: "v1.5.45" or "refs/tags/v1.5.45") (required)',
                        required=True)
    parser.add_argument('-u', '--url', help='URL for Presearch Browser raw markdown file (required)', required=True)
