...
from lib.grd_string_replacements import (write_xml_file_from_tree,
                                         write_presearchified_grd_override,
                                         update_presearchified_grd_tree_override,
...
def generate_overrides_and_replace_strings(source_string_path):
    ...
    update_presearchified_grd_tree_override(original_xml_tree_with_branding_fixes, True)
    write_presearchified_grd_override(source_string_path)
    ...
    for part in parts:
        ...
        # Check for the special case of presearch_stings.grd:
        if (os.path.basename(source_string_path) == 'presearch_strings.grd'
                and override_file == 'settings_chromium_strings_override.grdp'):
            override_file = 'settings_presearch_strings_override.grdp'
...
def main():
    args = parse_args()
    # This file path is a string path inside presearch/ but just recently copied
    ...
    if basename == 'presearch_strings':
        ...
        elem1.text = 'Presearch Beta (mDNS-In)'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//message[@name="IDS_INBOUND_MDNS_RULE_NAME_CANARY"]')[0]
        elem1.text = 'Presearch Nightly (mDNS-In)'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//message[@name="IDS_INBOUND_MDNS_RULE_NAME_DEV"]')[0]
        elem1.text = 'Presearch Dev (mDNS-In)'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
        ...
        elem1.text = 'Inbound rule for Presearch Beta to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//message[@name="IDS_INBOUND_MDNS_RULE_DESCRIPTION_CANARY"]')[0]
        elem1.text = 'Inbound rule for Presearch Nightly to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//message[@name="IDS_INBOUND_MDNS_RULE_DESCRIPTION_DEV"]')[0]
        elem1.text = 'Inbound rule for Presearch Dev to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//part[@file="settings_chromium_strings.grdp"]')[0]
        elem1.set('file', 'settings_presearch_strings.grdp')
