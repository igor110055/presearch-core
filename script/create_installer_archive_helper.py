...
def SignAndCopyPreSignedBinaries(skip_signing, output_dir, staging_dir, current_version):
    if not skip_signing:
        ...
        """Copies already signed three binaries - presearch.exe and chrome.dll
        ...
        shutil.copy(os.path.join(src_dir, 'presearch.exe'), chrome_dir)
        shutil.copy(os.path.join(src_dir, 'chrome.dll'), version_dir)


def PresearchCopyAllFilesToStagingDir(config, staging_dir, g_archive_inputs):
    current_dir = os.path.realpath(os.path.dirname(os.path.realpath(__file__)))

    presearch_extension_locales_src_dir_path = os.path.realpath(
        os.path.join(current_dir, os.pardir, 'components',
                     'presearch_extension', 'extension', 'presearch_extension', '_locales'))
    CopyExtensionLocalization('presearch_extension', presearch_extension_locales_src_dir_path,
                              config, staging_dir, g_archive_inputs)

    presearch_rewards_locales_src_dir_path = os.path.realpath(
        os.path.join(current_dir, os.pardir, 'components',
                     'presearch_rewards', 'resources', 'extension', 'presearch_rewards', '_locales'))
    CopyExtensionLocalization('presearch_rewards', presearch_rewards_locales_src_dir_path,
                              config, staging_dir, g_archive_inputs)
...
def CopyExtensionLocalization(extension_name, locales_src_dir_path, config, staging_dir, g_archive_inputs):
    ...
    locales_dest_path = os.path.join(locales_dest_path, config.get('GENERAL', 'presearch_resources.pak'),
