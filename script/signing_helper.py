...
presearch_channel = os.environ.get('PRESEARCH_CHANNEL')
...
def GeneratePresearchWidevineSigFile(paths, config, part):
...
def AddPresearchPartsForSigning(parts, config):
    ...
    # Add libs
    presearch_dylibs = (
        'libpresearch_rust.dylib',
    )
    for library in presearch_dylibs:
...
def GetPresearchSigningConfig(config_class, mac_provisioning_profile=None):
    ...
    class ProvisioningProfileCodeSignConfig(config_class):
        ...
        def distributions(self):
            return [model.Distribution(channel=presearch_channel)]
