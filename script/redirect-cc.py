...
def main():
    args = sys.argv[1:]
    presearch_path = replace_cc_arg(args)
    ...
    if IS_WIN32 and cc_retcode == 0 and presearch_path:
...
def replace_cc_arg(args):
    ...
    # Get this `presearch/src/presearch/script/redirect-cc.py` script absolute location
    ...
    # Build possible presearch/chromium_src_path
    presearch_path = os.path.join(chromium_original_dir, 'presearch', 'chromium_src',
                              rel_path)
    if os.path.isfile(presearch_path):
        # Okay, we can replace
        args[index_path] = os.path.relpath(presearch_path, os.path.abspath('.'))
        return presearch_path
