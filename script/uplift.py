...
from lib.config import get_env_var, SOURCE_ROOT, PRESEARCH_CORE_ROOT, get_raw_version
...
class PrConfig:
    ...
    def initialize(self, args):
        try:
            ...
            if len(self.github_token) == 0:
                try:
                    ...
                    if sys.platform.startswith('win'):
                        npm_cmd = 'npm.cmd'
                    result = execute([npm_cmd, 'config', 'get', 'PRESEARCH_GITHUB_TOKEN']).strip()
                    if result == 'undefined':
                        raise Exception('`PRESEARCH_GITHUB_TOKEN` value not found!')
                    self.github_token = result
                except Exception as e:
                    print('[ERROR] no valid GitHub token was found either in npmrc or ' +
                          'via environment variables (PRESEARCH_GITHUB_TOKEN)')
                    return 1
            ...
            self.labels = parse_labels(self.github_token, PRESEARCH_CORE_REPO, args.labels, verbose=self.is_verbose)
            if self.is_verbose:
...
def get_remote_version(branch_to_compare):
    global config
    decoded_file = get_file_contents(config.github_token, PRESEARCH_REPO, 'package.json', branch_to_compare)
...
def parse_issues_fixed(body):
    try:
        regex = r'((Resolves|Fixes|Fix|Closes|Close|resolves|fixes|fix|closes|close) https:\/\/github\.com\/presearch\/presearch-browser\/issues\/(\d*))'  # nopep8
...
def main():
    ...
    result = fetch_origin_check_staged(PRESEARCH_CORE_ROOT)
    if result != 0:
        return result

    # get all channel branches (starting at master)
    presearch_browser_version = get_remote_version('master')
    remote_branches = get_remote_channel_branches(presearch_browser_version)
    ...
    if args.uplift_using_pr:
        try:
            ...
            repo = GitHub(config.github_token).repos(PRESEARCH_CORE_REPO)
        ...
        with scoped_cwd(PRESEARCH_CORE_ROOT):
    ...
    local_branch = get_local_branch_name(PRESEARCH_CORE_ROOT)
    if not config.title and not args.uplift_using_pr:
        config.title = get_title_from_first_commit(PRESEARCH_CORE_ROOT, top_level_base)
    ...
    push_branches_to_remote(PRESEARCH_CORE_ROOT, config.branches_to_push,
...
def get_milestone_for_branch(channel_branch):
    ...
    if not config.milestones:
        config.milestones = get_milestones(config.github_token, PRESEARCH_CORE_REPO)
...
def submit_pr(channel, top_level_base, remote_base, local_branch, issues_fixed):
    ...
    try:
        ...
        number = create_pull_request(config.github_token, PRESEARCH_CORE_REPO, pr_title, pr_body,
        ...
        add_reviewers_to_pull_request(config.github_token, PRESEARCH_CORE_REPO, number,
                                      team_reviewers=config.team_reviewers,
                                      verbose=config.is_verbose, dryrun=config.is_dryrun)
        set_issue_details(config.github_token, PRESEARCH_CORE_REPO, number, milestone_number,
