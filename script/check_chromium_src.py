...
PRESEARCH_SRC = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
PRESEARCH_CHROMIUM_SRC = os.path.join(PRESEARCH_SRC, 'chromium_src')
CHROMIUM_SRC = os.path.abspath(os.path.dirname(PRESEARCH_SRC))
...
EXCLUDES = [
    ...
    'third_party/blink/renderer/modules/storage/presearch_dom_window_storage.h',
    'chrome/installer/linux/common/presearch-browser/chromium-browser.appdata.xml',
    'chrome/installer/linux/common/presearch-browser/chromium-browser.info',
]
...
def do_check_includes(override_filepath):
    ...
    with open(override_filepath, mode='r', encoding='utf-8') as override_file:
        ...
        # +3 to get to chromium_src->presearch->src.
...
def filter_chromium_src_filepaths(include_regexp=None, exclude_regexp=None):
    """
    Return a list of paths pointing to the files in |PRESEARCH_CHROMIUM_SRC|
    ...
    for dir_path, _dirnames, filenames in os.walk(PRESEARCH_CHROMIUM_SRC):
        for filename in filenames:
            full_path = os.path.join(dir_path, filename)

            # Strip the PRESEARCH_CHROMIUM_SRC prefix to match against the
            # list of relative paths to be included and/or excluded
            relative_path = full_path.replace(PRESEARCH_CHROMIUM_SRC, '')[1:]
