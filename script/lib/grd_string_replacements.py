...
branding_replacements = [
    (r'The Chromium Authors. All rights reserved.', r'The Presearch Authors. All rights reserved.'),
    (r'Google LLC. All rights reserved.', r'The Presearch Authors. All rights reserved.'),
...
default_replacements = [
    ...
    (r'Automatically send usage statistics and crash reports to Presearch',
     r'Automatically send crash reports to Presearch'),
    (r'Automatically sends usage statistics and crash reports to Presearch',
     r'Automatically sends crash reports to Presearch'),
...
fixup_replacements = [
    ...
    (r'Sends URLs of some pages you visit to Presearch', r'Sends URLs of some pages you visit to Google'),
...
def presearchify_grd_text(text, is_main_text, branding_replacements_only):
    """Replaces text string to Presearch wording"""
...
def generate_presearchified_node(elem, is_comment, branding_replacements_only):
    """Replaces a node and attributes to Presearch wording"""
    if elem.text:
        elem.text = presearchify_grd_text(elem.text, not is_comment, branding_replacements_only)

    if elem.tail:
        elem.tail = presearchify_grd_text(elem.tail, not is_comment, branding_replacements_only)

    if 'desc' in elem.keys():
        elem.attrib['desc'] = presearchify_grd_text(elem.attrib['desc'], False, branding_replacements_only)
    for child in elem:
        generate_presearchified_node(child, is_comment, branding_replacements_only)
...
def update_presearchified_grd_tree_override(source_xml_tree, branding_replacements_only):
    """Takes in a grd(p) tree and replaces all messages and comments with Presearch wording"""
    for elem in source_xml_tree.xpath('//message'):
        generate_presearchified_node(elem, False, branding_replacements_only)
    for elem in source_xml_tree.xpath('//comment()'):
        generate_presearchified_node(elem, True, branding_replacements_only)


def write_presearchified_grd_override(source_string_path):
    """Takes in a grd file and replaces all messages and comments with Presearch wording"""
    source_xml_tree = etree.parse(source_string_path)
    update_presearchified_grd_tree_override(source_xml_tree, False)
    write_xml_file_from_tree(source_string_path, source_xml_tree)


def get_override_file_path(source_string_path):
    """Obtain src/presearch source string override path for local grd strings with replacements"""
