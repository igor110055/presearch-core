...
from lib.grd_string_replacements import (generate_presearchified_node,
...
transifex_project_name = 'presearch'
base_url = 'https://www.transifex.com/api/2/'
transifex_handled_slugs = [
    'android_presearch_strings',
    'presearch_generated_resources',
    'presearch_components_resources',
    'presearch_extension',
    'rewards_extension',
...
def transifex_name_from_greaselion_script_name(script_name):
    match = re.search(
        'presearch-site-specific-scripts/scripts/(.*)/_locales/en_US/messages.json$', script_name)
    if match:
...
def transifex_name_from_filename(source_file_path, filename):
    ...
    if 'presearch_components_strings' in source_file_path:
        return 'presearch_components_resources'
    elif ext == '.grd':
        return filename
    elif 'presearch-site-specific-scripts/scripts/' in source_file_path:
        return transifex_name_from_greaselion_script_name(source_file_path)
    elif 'presearch_extension' in source_file_path:
        return 'presearch_extension'
    elif 'presearch_rewards' in source_file_path:
...
def is_translateable_string(grd_file_path, message_tag):
    ...
    # to be translateable in Presearch. These can be found in the main function in
    # presearch/script/chromium-rebase-l10n.py
...
def get_original_grd(src_root, grd_file_path):
    """Obtains the Chromium GRD file for a specified Presearch GRD file."""
    # TODO: consider passing this mapping into the script from l10nUtil.js
    grd_file_name = os.path.basename(grd_file_path)
    if grd_file_name == 'components_presearch_strings.grd':
        return os.path.join(src_root, 'components',
                            'components_chromium_strings.grd')
    elif grd_file_name == 'presearch_strings.grd':
...
def check_for_chromium_upgrade_extra_langs(src_root, grd_file_path):
    """Checks the Presearch GRD file vs the Chromium GRD file for extra
    languages."""
    chromium_grd_file_path = get_original_grd(src_root, grd_file_path)
    if not chromium_grd_file_path:
        return
    presearch_langs = get_transifex_languages(grd_file_path)
    chromium_langs = get_transifex_languages(chromium_grd_file_path)
    x_presearch_extra_langs = presearch_langs - chromium_langs
    assert len(x_presearch_extra_langs) == 0, (
        'Presearch GRD %s has extra languages %s over Chromium GRD %s' % (
            grd_file_path, chromium_grd_file_path,
            list(x_presearch_extra_langs)))
    x_chromium_extra_langs = chromium_langs - presearch_langs
    assert len(x_chromium_extra_langs) == 0, (
        'Chromium GRD %s has extra languages %s over Presearch GRD %s' % (
...
def presearchify(string_value):
    """Replace Chromium branded strings with Presearch beranded strings."""
    return (string_value.replace('Chrome', 'Presearch')
            .replace('Chromium', 'Presearch')
            .replace('Google', 'Presearch')
            .replace('Presearch Docs', 'Google Docs')
            .replace('Presearch Drive', 'Google Drive')
            .replace('Presearch Play', 'Google Play')
            .replace('Presearch Safe', 'Google Safe')
            .replace('Sends URLs of some pages you visit to Presearch',
                     'Sends URLs of some pages you visit to Google')
            .replace('Presearch Account', 'Presearch sync chain'))
...
def upload_missing_translation_to_transifex(source_string_path, lang_code,
    ...
    url = base_url + url_part
    translated_value = presearchify(translated_value)
    payload = {
...
def get_acceptable_json_lang_codes(langs_dir_path):
    ...
    # Source language for Presearch locales
...
def get_chromium_grd_src_with_fallback(grd_file_path, presearch_source_root):
    source_root = os.path.dirname(presearch_source_root)
    chromium_grd_file_path = get_original_grd(source_root, grd_file_path)
    if not chromium_grd_file_path:
        filename = os.path.basename(grd_file_path)
        rel_path = os.path.relpath(grd_file_path, presearch_source_root)
...
def pull_xtb_without_transifex(grd_file_path, presearch_source_root):
    xtb_files = get_xtb_files(grd_file_path)
    chromium_grd_file_path = get_chromium_grd_src_with_fallback(grd_file_path,
                                                                presearch_source_root)
    ...
    for idx, xtb_file in enumerate(xtb_file_paths):
        ...
        for node in xml_tree.xpath('//translation'):
            generate_presearchified_node(node, False, True)
