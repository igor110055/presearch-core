...
def main():
    ...
    if os.environ.get('PRESEARCH_CORE_DIR'):
        presearch_core_dir = os.environ.get('PRESEARCH_CORE_DIR')
    else:
        logging.error("Error: Required environment variable \'PRESEARCH_CORE_DIR\' not set! Exiting...")
        exit(1)

    if args.debug:
        logging.basicConfig(stream=sys.stderr, level=logging.DEBUG)
        logging.debug('presearch_version: {}'.format(get_raw_version()))
        ...
        logging.debug('presearch_core_dir: {}'.format(presearch_core_dir))
    ...
    for item in ['upload_to_aptly', 'upload_to_rpm_repo']:
        ...
        if re.match(r'.*rpm.*', item):
            bucket = 'presearch-browser-rpm-staging-'
        else:
            bucket = 'presearch-browser-apt-staging-'

        upload_script = os.path.join(presearch_core_dir, 'script', item)
...
def download_linux_pkgs_from_github(args, logging):
    ...
    # PRESEARCH_REPO defined in helpers.py
    repo = GitHub(args.github_token).repos(PRESEARCH_REPO)
    tag_name = args.tag
    release = {}
...
def parse_args():
    ...
    parser.add_argument('-c', '--channel', help='The Presearch channel, i.e. \'nightly\', \'dev\', \'beta\', \'release\'',
                        required=True)
