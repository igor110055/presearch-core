...
def install(download_dir, path):
    ...
    if is_mac_os:
        ...
        for x in result_lines:
            x = x.strip()
            index = x.find('/Volumes/Presearch')
            if index > -1:
                volume = x[index:]
                break

        if volume is None:
            raise Exception('[ERROR] did not find "/Volumes/Presearch" sub-string in mount list!\n \
...
def get_github_token():
    ...
    if len(github_token) == 0:
        result = execute(['npm', 'config', 'get', 'PRESEARCH_GITHUB_TOKEN']).strip()
        if result == 'undefined':
...
def main():
    ...
    repo = GitHub(github_token).repos(PRESEARCH_REPO)
