...
// npm run test -- presearch_unit_tests --filter=PrivateCDNTest.*
...
TEST(PrivateCDNTest, GetServerUrlDevelopment) {
  ...
  ASSERT_EQ(url, "https://pcdn.presearch.software/test");
}

TEST(PrivateCDNTest, GetServerUrlStaging) {
  ledger::_environment = type::Environment::STAGING;
  const std::string url = GetServerUrl("/test");
  ASSERT_EQ(url, "https://pcdn.presearchsoftware.com/test");
}
