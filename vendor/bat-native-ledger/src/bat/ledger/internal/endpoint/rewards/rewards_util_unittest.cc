...
// npm run test -- presearch_unit_tests --filter=RewardsUtilTest.*
...
TEST(RewardsUtilTest, GetServerUrlDevelopment) {
  ...
  ASSERT_EQ(url, "https://rewards-dev.presearch.software/test");
}

TEST(RewardsUtilTest, GetServerUrlStaging) {
  ledger::_environment = type::Environment::STAGING;
  const std::string url = GetServerUrl("/test");
  ASSERT_EQ(url, "https://rewards-stg.presearchsoftware.com/test");
}
