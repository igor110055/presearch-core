...
// npm run test -- presearch_unit_tests --filter=PromotionsUtilTest.*
...
TEST(PromotionsUtilTest, GetServerUrlDevelopment) {
  ...
  ASSERT_EQ(url, "https://grant.rewards.presearch.software/test");
}

TEST(PromotionsUtilTest, GetServerUrlStaging) {
  ledger::_environment = type::Environment::STAGING;
  const std::string url = GetServerUrl("/test");
  ASSERT_EQ(url, "https://grant.rewards.presearchsoftware.com/test");
}
