...
PostWalletPresearch::PostWalletPresearch(LedgerImpl* ledger):
    ledger_(ledger) {
  DCHECK(ledger_);
}

PostWalletPresearch::~PostWalletPresearch() = default;

std::string PostWalletPresearch::GetUrl() {
  return GetServerUrl("/v3/wallet/presearch");
}

type::Result PostWalletPresearch::CheckStatusCode(const int status_code) {
...
type::Result PostWalletPresearch::ParseBody(
...
void PostWalletPresearch::Request(
  ...
  auto url_callback = std::bind(&PostWalletPresearch::OnRequest,
      this,
      _1,
      callback);
...
void PostWalletPresearch::OnRequest(
