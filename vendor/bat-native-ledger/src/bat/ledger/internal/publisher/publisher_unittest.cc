...
// npm run test -- presearch_unit_tests --filter=PublisherTest.*
...
TEST_F(PublisherTest, GetShareURL) {
  ...
  args.insert({"name", "presearch"});
  args.insert({"comment", "I just tipped someone using the Presearch browser."});
  args.insert({"hashtag", "TipWithPresearch"});
  EXPECT_EQ(Publisher::GetShareURL(args),
            "https://twitter.com/intent/tweet?text=I just tipped someone using "
            "the Presearch browser.%20%23TipWithPresearch");
  ...
  EXPECT_EQ(Publisher::GetShareURL(args),
            "https://twitter.com/intent/tweet?text=I just tipped someone using "
            "the Presearch browser.%20%23TipWithPresearch"
            "&url=https://twitter.com/presearch/status/794221010484502528");
}
