...
LedgerImpl::LedgerImpl(ledger::LedgerClient* client)
    : ledger_client_(client),
      ...
      media_(std::make_unique<presearchledger_media::Media>(this)),
...
presearchledger_media::Media* LedgerImpl::media() const {
  return media_.get();
}
...
void LedgerImpl::OnPostData(
  ...
  if (type == TWITCH_MEDIA_TYPE) {
    ...
    presearchledger_media::GetTwitchParts(post_data, &twitchParts);
  ...
  if (type == VIMEO_MEDIA_TYPE) {
    ...
    presearchledger_media::GetVimeoParts(post_data, &parts);
...
void LedgerImpl::GetRewardsInternalsInfo(
  ...
  type::PresearchWalletPtr wallet = wallet_->GetWallet();
  if (!wallet) {
...
void LedgerImpl::GetPresearchWallet(GetPresearchWalletCallback callback) {
  callback(wallet()->GetWallet());
}

std::string LedgerImpl::GetWalletPassphrase() const {
  const auto presearch_wallet = wallet()->GetWallet();
  if (!presearch_wallet) {
    return "";
  }

  return wallet()->GetWalletPassphrase(presearch_wallet->Clone());
}

void LedgerImpl::LinkPresearchWallet(const std::string& destination_payment_id,
                                 PostSuggestionsClaimCallback callback) {
  wallet()->LinkPresearchWallet(destination_payment_id, callback);
}
