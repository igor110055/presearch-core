...
std::string Wallet::GetWalletPassphrase(type::PresearchWalletPtr wallet) {
  if (!wallet) {
...
type::PresearchWalletPtr Wallet::GetWallet() {
  const std::string wallet_string =
      ledger_->ledger_client()->GetEncryptedStringState(state::kWalletPresearch);
  ...
  if (!value || !value->is_dict()) {
    BLOG(0, "Parsing of presearch wallet failed");
    return nullptr;
  }
  ...
  auto wallet = ledger::type::PresearchWallet::New();
...
bool Wallet::SetWallet(type::PresearchWalletPtr wallet) {
  if (!wallet) {
    BLOG(0, "Presearch wallet is null");
    return false;
  }
  ...
  BLOG_IF(0, !success, "Can't encrypt presearch wallet");

  return success;
}

void Wallet::LinkPresearchWallet(const std::string& destination_payment_id,
                             ledger::PostSuggestionsClaimCallback callback) {
  promotion_server_->post_claim_presearch()->Request(
