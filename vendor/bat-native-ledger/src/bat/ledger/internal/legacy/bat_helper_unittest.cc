...
// npm run test -- presearch_unit_tests --filter=BatHelperTest.*
...
TEST(BatHelperTest, HasSameDomainAndPath) {
  ...
  // regular url
  bool result = presearchledger_bat_helper::HasSameDomainAndPath(
      url, url_portion, path);
  ASSERT_EQ(result, true);

  // empty url with portion
  url = std::string();
  result = presearchledger_bat_helper::HasSameDomainAndPath(
      url, url_portion, path);
