...
// npm run test -- presearch_unit_tests --filter=VimeoTest.*
...
const char user_link[] =
    ...
    "<span class=\"userlink userlink--md\"><a href=\"/nejcpresearch\">Nejc</a>"
...
const char publisher_page[] =
    ...
    "<meta property=\"og:url\" content=\"https://vimeo.com/nejcpresearch\">"
...
TEST(VimeoTest, GetUrlFromVideoPage) {
  ...
  ASSERT_EQ(result, "https://vimeo.com/nejcpresearch/videos");
}
...
TEST(VimeoTest, IsExcludedPath) {
  ...
  result =
      Vimeo::IsExcludedPath("/presearch");
  ASSERT_EQ(result, false);
}
