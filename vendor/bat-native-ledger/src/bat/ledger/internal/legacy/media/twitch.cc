...
std::string Twitch::GetLinkType(const std::string& url,
  ...
  bool is_valid_twitch_path =
      presearchledger_bat_helper::HasSameDomainAndPath(
...
std::string Twitch::GetMediaIdFromUrl(
  ...
  std::string mediaId = presearchledger_media::ExtractData(url, "twitch.tv/", "/");

  if (url.find("twitch.tv/videos/") != std::string::npos) {
    mediaId = presearchledger_media::ExtractData(publisher_blob,
...
std::string Twitch::GetMediaKeyFromUrl(
  ...
  if (url.find("twitch.tv/videos/") != std::string::npos) {
    std::string vod_id = presearchledger_media::ExtractData(url,
                                                      "twitch.tv/videos/",
                                                      "/");
...
std::string Twitch::GetPublisherName(
    const std::string& publisher_blob) {
  return presearchledger_media::ExtractData(publisher_blob,
    "<h5 class>", "</h5>");
}
...
std::string Twitch::GetFaviconUrl(
  ...
  const std::string wrapper = presearchledger_media::ExtractData(publisher_blob,
    "class=\"tw-avatar tw-avatar--size-36\"",
    "</figure>");

  return presearchledger_media::ExtractData(wrapper, "src=\"", "\"");
}
