...
// npm run test -- presearch_unit_tests --filter=MediaHelperTest.*
...
TEST(MediaHelperTest, GetMediaKey) {
  // provider is missing
  std::string result = presearchledger_media::GetMediaKey("key", "");
  ASSERT_EQ(result, "");

  // key is missing
  result = presearchledger_media::GetMediaKey("", "youtube");
  ASSERT_EQ(result, "");

  // all ok
  result = presearchledger_media::GetMediaKey("key", "youtube");
...
TEST(MediaHelperTest, GetTwitchParts) {
  ...
  // string is empty
  presearchledger_media::GetTwitchParts("", &twitch_parts);
  ASSERT_EQ(twitch_parts.size(), 0u);

  // random string
  presearchledger_media::GetTwitchParts("this is random string", &twitch_parts);
  ASSERT_EQ(twitch_parts.size(), 0u);
  ...
  // all ok
  presearchledger_media::GetTwitchParts(post_data, &twitch_parts);
...
//  // string empty
  std::string result = presearchledger_media::ExtractData("", "/", "!");
  ASSERT_EQ(result, "");

  // missing start
  result = presearchledger_media::ExtractData("st/find/me!", "", "!");
