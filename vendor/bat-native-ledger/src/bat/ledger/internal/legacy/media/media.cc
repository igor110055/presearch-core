...
Media::Media(ledger::LedgerImpl* ledger):
  ledger_(ledger),
  media_youtube_(new presearchledger_media::YouTube(ledger)),
  media_twitch_(new presearchledger_media::Twitch(ledger)),
  media_reddit_(new presearchledger_media::Reddit(ledger)),
  media_vimeo_(new presearchledger_media::Vimeo(ledger)),
  media_github_(new presearchledger_media::GitHub(ledger)) {
}  // namespace presearchledger_media
...
std::string Media::GetLinkType(
  ...
  std::string type = presearchledger_media::YouTube::GetLinkType(url);
  if (HandledByGreaselion(type)) {
    return std::string();
  }

  if (type.empty()) {
    type = presearchledger_media::Twitch::GetLinkType(
        url,
        first_party_url,
        referrer);
  }
  ...
  if (type.empty()) {
    type = presearchledger_media::Vimeo::GetLinkType(url);
  }

  if (type.empty()) {
    type = presearchledger_media::GitHub::GetLinkType(url);
  }
