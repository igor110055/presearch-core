...
std::string YouTube::GetFavIconUrl(const std::string& data) {
  std::string favicon_url = presearchledger_media::ExtractData(
      data,
      "\"avatar\":{\"thumbnails\":[{\"url\":\"", "\"");

  if (favicon_url.empty()) {
    favicon_url = presearchledger_media::ExtractData(
      data,
...
std::string YouTube::GetChannelId(const std::string& data) {
  std::string id = presearchledger_media::ExtractData(data, "\"ucid\":\"", "\"");
  if (id.empty()) {
    id = presearchledger_media::ExtractData(
        data,
...
std::string YouTube::GetPublisherName(const std::string& data) {
  ...
  std::string publisher_json_name = presearchledger_media::ExtractData(
      data,
      "\"author\":\"", "\"");
  std::string publisher_json = "{\"presearch_publisher\":\"" +
      publisher_json_name + "\"}";
  // scraped data could come in with JSON code points added.
  // Make to JSON object above so we can decode.
  presearchledger_bat_helper::getJSONValue(
      "presearch_publisher", publisher_json, &publisher_name);
...
std::string YouTube::GetNameFromChannel(const std::string& data) {
  std::string publisher_name;
  const std::string publisher_json_name = presearchledger_media::ExtractData(data,
      "channelMetadataRenderer\":{\"title\":\"", "\"");
  const std::string publisher_json = "{\"presearch_publisher\":\"" +
      publisher_json_name + "\"}";
  // scraped data could come in with JSON code points added.
  // Make to JSON object above so we can decode.
  presearchledger_bat_helper::getJSONValue(
...
std::string YouTube::GetPublisherKeyFromUrl(
  ...
  const std::string id = presearchledger_media::ExtractData(path + "/",
      "/channel/", "/");
...
std::string YouTube::GetChannelIdFromCustomPathPage(
  ...
  return presearchledger_media::ExtractData(data,
      "{\"key\":\"browse_id\",\"value\":\"", "\"");
}
...
void YouTube::ProcessMedia(
  ...
  std::string media_key = presearchledger_media::GetMediaKey(media_id,
                                                         YOUTUBE_MEDIA_TYPE);
