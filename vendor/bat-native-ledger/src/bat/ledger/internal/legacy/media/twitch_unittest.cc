...
// npm run test -- presearch_unit_tests --filter=MediaTwitchTest.*
...
const char profile_html[] =
    ...
    "href=\"/presearchsoftware/videos\"><div class=\"tw-flex tw-pd-x-2 "
...
TEST(MediaTwitchTest, GetMediaIdFromParts) {
  ...
  result = Twitch::GetMediaIdFromParts({
    {"event", "minute-watched"},
    {"properties", ""},
    {"channel", "presearchsoftware"}
  });
  media_id = result.first;
  user_id = result.second;
  EXPECT_EQ(user_id, "presearchsoftware");
  ASSERT_EQ(media_id, "presearchsoftware");
  ...
  user_id = result.second;
  EXPECT_EQ(user_id, "presearchsoftware");
  ASSERT_EQ(media_id, "presearchsoftware_vod_123312312");
...
TEST(MediaTwitchTest, GetMediaURL) {
  ...
  // all ok
  result = Twitch::GetMediaURL("presearchsoftware");
  ASSERT_EQ(result, "https://www.twitch.tv/presearchsoftware");
}
...
TEST(MediaTwitchTest, GetMediaIdFromUrl) {
  ...
  std::string result = Twitch::GetMediaIdFromUrl(
      "https://www.twitch.tv/presearchsoftware", profile_html);

  ASSERT_EQ(result, "presearchsoftware");

  // longer url
  result = Twitch::GetMediaIdFromUrl(
      "https://www.twitch.tv/presearchsoftware/clips", profile_html);
...
TEST(MediaTwitchTest, GetMediaKeyFromUrl) {
  ...
  ASSERT_EQ(result, "twitch_presearchsoftware_vod_411403500");

  // regular id
  result = Twitch::GetMediaKeyFromUrl("presearchsoftware", "");
  ASSERT_EQ(result, "twitch_presearchsoftware");
}
...
TEST(MediaTwitchTest, GetFaviconUrl) {
  ...
  // blob is not correct
  result = Twitch::GetFaviconUrl("dfsfsdfsdfds", "presearchsoftware");
  ASSERT_EQ(result, "");

  // all ok
  result = Twitch::GetFaviconUrl(profile_html, "presearchsoftware");
  ASSERT_EQ(result,
...
TEST(MediaTwitchTest, UpdatePublisherData) {
  ...
  ASSERT_EQ(name, "presearchsoftware");
  ASSERT_EQ(favicon_url,
      "https://static-cdn.jtvnw.net/user-default-pictures/"
      "0ecbb6c3-fecb-4016-8115-aa467b7c36ed-profile_image-70x70.jpg");
}
