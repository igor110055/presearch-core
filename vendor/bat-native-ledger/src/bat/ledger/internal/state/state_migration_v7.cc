...
void StateMigrationV7::Migrate(ledger::ResultCallback callback) {
  const std::string presearch =
      ledger_->ledger_client()->GetStringState(kWalletPresearch);
  bool success =
      ledger_->ledger_client()->SetEncryptedStringState(kWalletPresearch, presearch);
