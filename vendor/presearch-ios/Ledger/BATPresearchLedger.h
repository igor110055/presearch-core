...
OBJC_EXPORT NSString * const BATPresearchLedgerErrorDomain NS_SWIFT_NAME(PresearchLedgerErrorDomain);

OBJC_EXPORT NSNotificationName const BATPresearchLedgerNotificationAdded NS_SWIFT_NAME(PresearchLedger.NotificationAdded);

typedef NSString *BATPresearchGeneralLedgerNotificationID NS_SWIFT_NAME(GeneralLedgerNotificationID) NS_STRING_ENUM;
OBJC_EXPORT BATPresearchGeneralLedgerNotificationID const BATPresearchGeneralLedgerNotificationIDWalletNowVerified;
OBJC_EXPORT BATPresearchGeneralLedgerNotificationID const BATPresearchGeneralLedgerNotificationIDWalletDisconnected;

OBJC_EXPORT
NS_SWIFT_NAME(PresearchLedger)
@interface BATPresearchLedger : NSObject

@property (nonatomic, weak) BATPresearchAds *ads;

@property (nonatomic, copy, nullable) BATFaviconFetcher faviconFetcher;

/// Create a presearch ledger that will read and write its state to the given path
...
/// Get the presearch wallet's payment ID and seed for ads confirmations
...
/// Links a desktop presearch wallet given some payment ID
- (void)linkPresearchWalletToPaymentId:(NSString *)paymentId
                        completion:(void (^)(BATResult result, NSString *drainID))completion
    NS_SWIFT_NAME(linkPresearchWallet(paymentId:completion:));

/// Obtain a drain status given some drain ID previously obtained from
/// `linkPresearchWalletToPaymentId:completion:`
...
///
/// Use `BATPresearchLedgerObserver` to retrieve a panel publisher if one is found
