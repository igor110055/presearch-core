...
#import "BATPresearchAds.h"
#import "BATPresearchAds+Private.h"
...
NSString * const BATPresearchLedgerErrorDomain = @"BATPresearchLedgerErrorDomain";
NSNotificationName const BATPresearchLedgerNotificationAdded = @"BATPresearchLedgerNotificationAdded";

BATPresearchGeneralLedgerNotificationID const BATPresearchGeneralLedgerNotificationIDWalletNowVerified = @"wallet_new_verified";
BATPresearchGeneralLedgerNotificationID const BATPresearchGeneralLedgerNotificationIDWalletDisconnected = @"wallet_disconnected";
...
@interface BATPresearchLedger () <NativeLedgerClientBridge> {
...
@property (nonatomic) NSHashTable<BATPresearchLedgerObserver *> *observers;
...
@implementation BATPresearchLedger
...
- (void)initializeLedgerService:(BOOL)executeMigrateScript completion:(nullable void (^)())completion
{
  ...
  ledger->Initialize(executeMigrateScript, ^(ledger::type::Result result){
    ...
    for (BATPresearchLedgerObserver *observer in [self.observers copy]) {
...
- (void)addObserver:(BATPresearchLedgerObserver *)observer
{
  [self.observers addObject:observer];
}

- (void)removeObserver:(BATPresearchLedgerObserver *)observer
{
...
- (void)createWallet:(void (^)(NSError * _Nullable))completion
{
  ...
  ledger->CreateWallet(^(ledger::type::Result result) {
    ...
    if (result != ledger::type::Result::WALLET_CREATED) {
      ...
      error = [NSError errorWithDomain:BATPresearchLedgerErrorDomain code:static_cast<NSInteger>(result) userInfo:userInfo];
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      for (BATPresearchLedgerObserver *observer in [strongSelf.observers copy]) {
        if (observer.walletInitalized) {
...
- (void)currentWalletInfo:(void (^)(BATPresearchWallet *_Nullable wallet))completion
{
  ledger->GetPresearchWallet(^(ledger::type::PresearchWalletPtr wallet){
    if (wallet.get() == nullptr) {
      completion(nil);
      return;
    }
    const auto bridgedWallet = [[BATPresearchWallet alloc] initWithPresearchWallet:*wallet];
...
- (void)linkPresearchWalletToPaymentId:(NSString *)paymentId completion:(void (^)(BATResult result, NSString *drainID))completion
{
  ledger->LinkPresearchWallet(paymentId.UTF8String, ^(ledger::type::Result result, std::string drain_id) {
                    ...
                    completion:(nullable void (^)(BATResult result))completion
{
  ledger->DisconnectWallet(walletType.UTF8String, ^(ledger::type::Result result){
    ...
    for (BATPresearchLedgerObserver *observer in self.observers) {
                     ...
                     onlyOnce:(BOOL)onlyOnce
{
  ...
  [NSNotificationCenter.defaultCenter postNotificationName:BATPresearchLedgerNotificationAdded object:nil];
...
- (void)resetState:(const std::string &)name callback:(ledger::ResultCallback)callback
{
  ...
  // In presearch-core, failed callback returns `LEDGER_ERROR`
...
{
  __weak BATPresearchLedger* weakSelf = self;
