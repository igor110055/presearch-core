...
@interface BATPresearchAds () <NativeAdsClientBridge> {
...
@implementation BATPresearchAds
...
+ (BATPresearchAdsSysInfo *)sysInfo
{
  auto sys_info = [[BATPresearchAdsSysInfo alloc] init];
  sys_info.isUncertainFuture = ads::g_sys_info.is_uncertain_future;

  return sys_info;
}

+ (void)setSysInfo:(BATPresearchAdsSysInfo *)sysInfo
{
  ads::g_sys_info.is_uncertain_future = sysInfo.isUncertainFuture;
}

+ (BATPresearchAdsBuildChannel *)buildChannel
{
  auto build_channel = [[BATPresearchAdsBuildChannel alloc] init];
...
+ (void)setBuildChannel:(BATPresearchAdsBuildChannel *)buildChannel
{
...
- (void)initializeIfAdsEnabled
{
  if (![self isAdsServiceRunning] && self.enabled) {
    ...
    [self.ledger currentWalletInfo:^(BATPresearchWallet * _Nullable wallet) {
...
- (BOOL)registerAdsResourcesForLanguageCode:(NSString*)languageCode {
  ...
  BLOG(1, @"Registering Presearch Ads Resources Installer (%@) with id %@",
...
- (const std::string)loadResourceForId:(const std::string&)id {
  const auto bundle = [NSBundle bundleForClass:[BATPresearchAds class]];
  const auto path =
...
{
  __weak BATPresearchAds* weakSelf = self;
