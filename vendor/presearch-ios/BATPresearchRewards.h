...
/// Configuration around presearch rewards for ads & ledger
OBJC_EXPORT
NS_SWIFT_NAME(PresearchRewardsConfiguration)
@interface BATPresearchRewardsConfiguration : NSObject <NSCopying>
...
@property (nonatomic, nullable) BATPresearchAdsBuildChannel *buildChannel;
...
@property (nonatomic, class, readonly) BATPresearchRewardsConfiguration *defaultConfiguration NS_SWIFT_NAME(default);
/// The staging configuration. Environment is staging, no changes to ads or ledger configuration
///
/// State is stored in Application Support
@property (nonatomic, class, readonly) BATPresearchRewardsConfiguration *stagingConfiguration NS_SWIFT_NAME(staging);
/// The production configuration. Environment is production, no changes to ads or ledger configuration
///
/// State is stored in Application Support
@property (nonatomic, class, readonly) BATPresearchRewardsConfiguration *productionConfiguration NS_SWIFT_NAME(production);
...
@property (nonatomic, class, readonly) BATPresearchRewardsConfiguration *testingConfiguration NS_SWIFT_NAME(testing);

@end

OBJC_EXPORT
NS_SWIFT_NAME(PresearchRewardsDelegate)
@protocol BATPresearchRewardsDelegate <NSObject>
@required
...
- (void)ledgerServiceDidStart:(BATPresearchLedger *)ledger;
...
/// A container for handling Presearch Rewards. Use `ads` to handle how many ads the users see,
/// when to display them. Use `ledger` to manage interactions between the users wallet & publishers
OBJC_EXPORT
NS_SWIFT_NAME(PresearchRewards)
@interface BATPresearchRewards : NSObject

@property (nonatomic, readonly) BATPresearchAds *ads;
/// Whether or not Presearch Ads is enabled
@property (nonatomic, assign, getter=isAdsEnabled) BOOL adsEnabled;
@property (nonatomic, readonly, nullable) BATPresearchLedger *ledger;
@property (nonatomic, weak) id<BATPresearchRewardsDelegate> delegate;

/// Resets the ads & ledger (by purging its data). This should likely never be used in production.
- (void)reset;

/// Create a PresearchRewards instance with a given configuration
- (instancetype)initWithConfiguration:(BATPresearchRewardsConfiguration *)configuration;
/// Create a PresearchRewards instance with a given configuration and custom ledger classes for mocking
- (instancetype)initWithConfiguration:(BATPresearchRewardsConfiguration *)configuration
                             delegate:(nullable id<BATPresearchRewardsDelegate>)delegate
...
OBJC_EXPORT
@interface BATPresearchRewards (Reporting)
