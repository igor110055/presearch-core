...
@implementation BATPresearchRewardsConfiguration

+ (BATPresearchRewardsConfiguration *)defaultConfiguration
{
  __auto_type config = [[BATPresearchRewardsConfiguration alloc] init];
...
+ (BATPresearchRewardsConfiguration *)stagingConfiguration
{
...
+ (BATPresearchRewardsConfiguration *)productionConfiguration
{
  __auto_type config = [self defaultConfiguration];
...
+ (BATPresearchRewardsConfiguration *)testingConfiguration
{
  __auto_type config = [self defaultConfiguration];
...
@interface BATPresearchRewards ()
@property (nonatomic) BATPresearchAds *ads;
@property (nonatomic, nullable) BATPresearchLedger *ledger;
@property (nonatomic, copy) BATPresearchRewardsConfiguration *configuration;
@property (nonatomic, assign) Class ledgerClass;
@property (nonatomic, assign) Class adsClass;
@end

@implementation BATPresearchRewards
                             ...
                             adsClass:(nullable Class)adsClass
{
  if ((self = [super init])) {
    ...
    self.ledgerClass = ledgerClass ?: BATPresearchLedger.class;
    self.adsClass = adsClass ?: BATPresearchAds.class;

    BATPresearchAds.debug = configuration.environment != BATEnvironmentProduction;
    BATPresearchAds.environment = configuration.environment;
    if (configuration.buildChannel != nil) {
      BATPresearchAds.buildChannel = configuration.buildChannel;
    }

    BATPresearchLedger.debug = configuration.environment != BATEnvironmentProduction;
    BATPresearchLedger.environment = configuration.environment;
    BATPresearchLedger.testing = configuration.testing;
    BATPresearchLedger.useShortRetries = configuration.useShortRetries;
    BATPresearchLedger.reconcileInterval = configuration.overridenNumberOfSecondsBetweenReconcile;
...
@implementation BATPresearchRewards (Reporting)
