...
version_info::Channel GetChannelImpl(std::string* modifier_out,
  ...
  if (modifier == LINUX_CHANNEL_DEV)
    modifier = PRESEARCH_LINUX_CHANNEL_DEV;
  if (modifier == LINUX_CHANNEL_STABLE) {
    channel = version_info::Channel::STABLE;
    modifier = PRESEARCH_LINUX_CHANNEL_STABLE;
  } else if (modifier == PRESEARCH_LINUX_CHANNEL_DEV) {
    channel = version_info::Channel::DEV;
    data_dir_suffix = "-Dev";
  } else if (modifier == LINUX_CHANNEL_BETA) {
    channel = version_info::Channel::BETA;
    data_dir_suffix = "-Beta";
  } else if (modifier == PRESEARCH_LINUX_CHANNEL_NIGHTLY) {
