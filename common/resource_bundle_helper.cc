...
base::FilePath GetResourcesPakFilePath() {
  ...
  return base::mac::PathForFrameworkBundleResource(
      CFSTR("presearch_resources.pak"));
#else
  base::FilePath pak_path;
  base::PathService::Get(base::DIR_MODULE, &pak_path);
  pak_path = pak_path.AppendASCII("presearch_resources.pak");
  return pak_path;
#endif  // OS_MAC
}
#endif  // OS_ANDROID
...
base::FilePath GetScaledResourcesPakFilePath(ui::ScaleFactor scale_factor) {
  ...
  const char* pak_file =
      (scale_factor == ui::SCALE_FACTOR_100P) ? "presearch_100_percent.pak"
                                              : "presearch_200_percent.pak";
#if defined(OS_MAC)
...
#if defined(OS_ANDROID)
  ui::PresearchLoadMainAndroidPackFile("assets/presearch_resources.pak",
                                   base::FilePath());
  ui::PresearchLoadPresearch100PercentPackFile("assets/presearch_100_percent.pak",
                                       base::FilePath());
#else
