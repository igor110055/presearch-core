...
TEST(PresearchChannelInfoTest, ChannelByNameTest) {
...
#if defined(OS_LINUX)
TEST(PresearchChannelInfoTest, ParentDirectoryOfUserDataDirectoryTest) {
  base::FilePath path;
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("PresearchSoftware", path.DirName().BaseName().AsUTF8Unsafe());
}

TEST(PresearchChannelInfoTest, DefaultUserDataDirectoryAndChannelTest) {
  base::FilePath path;
#if defined(OFFICIAL_BUILD)
  ...
  EXPECT_EQ(PRESEARCH_LINUX_CHANNEL_STABLE, chrome::GetChannelName());
  ...
  env->SetVar("CHROME_VERSION_EXTRA", PRESEARCH_LINUX_CHANNEL_DEV);
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Presearch-Browser-Dev", path.BaseName().AsUTF8Unsafe());
  EXPECT_EQ(version_info::Channel::DEV, chrome::GetChannel());
  EXPECT_EQ(PRESEARCH_LINUX_CHANNEL_DEV, chrome::GetChannelName());
  ...
  env->SetVar("CHROME_VERSION_EXTRA", PRESEARCH_LINUX_CHANNEL_NIGHTLY);
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Presearch-Browser-Nightly", path.BaseName().AsUTF8Unsafe());
  EXPECT_EQ(version_info::Channel::CANARY, chrome::GetChannel());
  EXPECT_EQ(PRESEARCH_LINUX_CHANNEL_NIGHTLY, chrome::GetChannelName());
#else  // OFFICIAL_BUILD
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
