...
PresearchContentClient::PresearchContentClient() {}

PresearchContentClient::~PresearchContentClient() {}

base::RefCountedMemory* PresearchContentClient::GetDataResourceBytes(
    int resource_id) {
  if (resource_id == IDR_FLAGS_UI_FLAGS_JS) {
    ...
    const std::string flags_js =
        ...
        resource_bundle.LoadDataResourceString(
            IDR_FLAGS_UI_PRESEARCH_FLAGS_OVERRIDES_JS);
...
void PresearchContentClient::AddAdditionalSchemes(Schemes* schemes) {
  ChromeContentClient::AddAdditionalSchemes(schemes);
  schemes->standard_schemes.push_back(content::kPresearchUIScheme);
  schemes->secure_schemes.push_back(content::kPresearchUIScheme);
  schemes->cors_enabled_schemes.push_back(content::kPresearchUIScheme);
  schemes->savable_schemes.push_back(content::kPresearchUIScheme);
}

void PresearchContentClient::AddContentDecryptionModules(
