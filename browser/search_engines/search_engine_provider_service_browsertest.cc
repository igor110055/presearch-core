...
#include "presearch/browser/search_engines/guest_window_search_engine_provider_service.h"
...
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  EXPECT_FALSE(presearch::UseAlternativeSearchEngineProviderEnabled(profile));
  ...
  presearch::ToggleUseAlternativeSearchEngineProvider(profile);
  EXPECT_TRUE(presearch::UseAlternativeSearchEngineProviderEnabled(profile));
...
// https://github.com/presearch/presearch-browser/issues/1452
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  presearch::ToggleUseAlternativeSearchEngineProvider(private_window_2->profile());
}
...
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  int default_provider_id = presearch::IsRegionForQwant(tor_profile) ?
...
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  int default_provider_id =
      presearch::IsRegionForQwant(tor_profile)
          ? TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_QWANT
          : TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_DUCKDUCKGO;
  auto* service = TemplateURLServiceFactory::GetForProfile(tor_profile);
  EXPECT_EQ(service->GetDefaultSearchProvider()->data().prepopulate_id,
...
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  if (presearch::IsRegionForQwant(guest_profile))
    return;
  ...
  EXPECT_FALSE(presearch::UseAlternativeSearchEngineProviderEnabled(guest_profile));

  presearch::ToggleUseAlternativeSearchEngineProvider(guest_profile);
  EXPECT_TRUE(presearch::UseAlternativeSearchEngineProviderEnabled(guest_profile));
  int provider_id =
      TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_DUCKDUCKGO;
