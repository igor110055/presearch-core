...
TEST(PresearchResourcesUtil, CheckIds) {
  const struct {
    const char* name;
    int id;
  } kCases[] = {
    // IDRs from presearch/app/theme/presearch_theme_resources.grd should be valid.
...
#if defined(OS_WIN)
TEST(PresearchResourcesWinTest, CheckStringsForInstaller) {
  // Test whether strings for installer are filled.
  // This test is added because these strings are initially empty and filled
  // with presearch's one.
  // presearch::InitializeResourceBundle can only be called once per unittest
