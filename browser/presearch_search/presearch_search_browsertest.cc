...
class PresearchSearchTest : public InProcessBrowserTest {
 public:
  PresearchSearchTest() {}
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_->RegisterRequestHandler(base::BindRepeating(
        &PresearchSearchTest::HandleRequest, base::Unretained(this)));
    ...
    presearch_search::PresearchSearchHost::SetBackupProviderForTest(url);
  }
  ...
  std::unique_ptr<net::test_server::HttpResponse> HandleRequest(
    ...
    if (request.GetURL().path_piece() == "/sw.js" ||
        request.GetURL().path_piece() == "/presearchsearch.html")
      return nullptr;
...
IN_PROC_BROWSER_TEST_F(PresearchSearchTest, CheckForAFunction) {
  GURL url = https_server()->GetURL(kAllowedDomain, "/presearchsearch.html");
...
IN_PROC_BROWSER_TEST_F(PresearchSearchTest, CheckForAFunctionDev) {
  GURL url = https_server()->GetURL(kAllowedDomainDev, "/presearchsearch.html");
  ui_test_utils::NavigateToURL(browser(), url);
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  WaitForLoadStop(contents);
...
IN_PROC_BROWSER_TEST_F(PresearchSearchTest, CheckForAnUndefinedFunction) {
  GURL url = https_server()->GetURL(kNotAllowedDomain, "/presearchsearch.html");
  ui_test_utils::NavigateToURL(browser(), url);
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  WaitForLoadStop(contents);
