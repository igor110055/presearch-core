...
int OnBeforeURLRequest_StaticRedirectWorkForGURL(
  ...
  if (!safebrowsing_endpoint.empty() &&
    ...
    replacements.SetHostStr(kPresearchSafeBrowsingSslProxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return net::OK;
  }
  ...
  if (!safebrowsing_endpoint.empty() &&
    ...
    replacements.SetHostStr(kPresearchSafeBrowsing2Proxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return net::OK;
  }
  ...
  if (autofill_pattern.MatchesURL(request_url)) {
    replacements.SetSchemeStr("https");
    replacements.SetHostStr(kPresearchStaticProxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return net::OK;
  }
  ...
  if (gvt1_pattern.MatchesURL(request_url) &&
    ...
    replacements.SetSchemeStr("https");
    replacements.SetHostStr(kPresearchRedirectorProxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return net::OK;
  }
  ...
  if (translate_pattern.MatchesURL(request_url)) {
    ...
    *new_url =
      GURL(kPresearchTranslateEndpoint).ReplaceComponents(replacements);
    return net::OK;
  }

  if (translate_language_pattern.MatchesURL(request_url)) {
    *new_url = GURL(kPresearchTranslateLanguageEndpoint);
    return net::OK;
  }
#endif
