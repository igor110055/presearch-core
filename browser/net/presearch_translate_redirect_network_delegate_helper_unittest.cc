...
class PresearchTranslateRedirectNetworkDelegateHelperTest
    : public testing::Test {
 public:
  PresearchTranslateRedirectNetworkDelegateHelperTest()
      : thread_bundle_(content::TestBrowserThreadBundle::IO_MAINLOOP),
        context_(new net::TestURLRequestContext(true)) {}
  ~PresearchTranslateRedirectNetworkDelegateHelperTest() override {}
...
TEST_F(PresearchTranslateRedirectNetworkDelegateHelperTest,
  ...
  std::for_each(paths.begin(), paths.end(),
    ...
    std::shared_ptr<presearch::PresearchRequestInfo> before_url_context(
        new presearch::PresearchRequestInfo());
    presearch::PresearchRequestInfo::FillCTXFromRequest(request.get(),
        before_url_context);
    presearch::ResponseCallback callback;

    GURL expected_url(kPresearchTranslateServer + path_string);
...
TEST_F(PresearchTranslateRedirectNetworkDelegateHelperTest,
  ...
  std::for_each(urls.begin(), urls.end(),
    ...
    GURL expected_url(kPresearchTranslateServer + url.path());
    int ret = OnBeforeURLRequest_TranslateRedirectWork(callback,
        before_url_context);
    EXPECT_EQ(before_url_context->new_url_spec, expected_url);
    EXPECT_EQ(ret, net::OK);
...
TEST_F(PresearchTranslateRedirectNetworkDelegateHelperTest,
  ...
  GURL expected_url(kPresearchTranslateEndpoint + std::string("?") + url.query());
  int ret = OnBeforeURLRequest_TranslateRedirectWork(callback,
      before_url_context);
  EXPECT_EQ(before_url_context->new_url_spec, expected_url);
  EXPECT_EQ(ret, net::OK);
