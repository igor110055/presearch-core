...
TEST_F(IPFSRedirectNetworkDelegateHelperTest, TranslateIPFSURIHTTPScheme) {
  ...
  int rc = ipfs::OnBeforeURLRequest_IPFSRedirectWork(presearch::ResponseCallback(),
...
TEST_F(IPFSRedirectNetworkDelegateHelperTest, TranslateIPFSURIIPFSSchemeLocal) {
  ...
  auto presearch_request_info = std::make_shared<presearch::PresearchRequestInfo>(url);
  presearch_request_info->browser_context = profile();
  presearch_request_info->ipfs_gateway_url = GetLocalGateway();
  presearch_request_info->initiator_url = ipfs::GetIPFSGatewayURL(
      initiator_cid, "",
      ipfs::GetDefaultIPFSLocalGateway(chrome::GetChannel()));
  int rc = ipfs::OnBeforeURLRequest_IPFSRedirectWork(presearch::ResponseCallback(),
                                                     presearch_request_info);
  EXPECT_EQ(rc, net::OK);
  EXPECT_EQ(presearch_request_info->new_url_spec,
...
TEST_F(IPFSRedirectNetworkDelegateHelperTest, TranslateIPFSURIIPFSScheme) {
  ...
  auto presearch_request_info = std::make_shared<presearch::PresearchRequestInfo>(url);
  presearch_request_info->browser_context = profile();
  presearch_request_info->ipfs_gateway_url = GetPublicGateway();
  presearch_request_info->initiator_url = ipfs::GetIPFSGatewayURL(
...
TEST_F(IPFSRedirectNetworkDelegateHelperTest, HeadersIPFSWorkWithRedirect) {
  ...
  int rc = ipfs::OnHeadersReceived_IPFSRedirectWork(
      ...
      &allowed_unsafe_redirect_url, presearch::ResponseCallback(), request_info);
