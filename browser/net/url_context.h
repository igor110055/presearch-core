...
#ifndef PRESEARCH_BROWSER_NET_URL_CONTEXT_H_
#define PRESEARCH_BROWSER_NET_URL_CONTEXT_H_
...
int OnBeforeURLRequest(const presearch::ResponseCallback& next_callback,
                       std::shared_ptr<presearch::PresearchRequestInfo> ctx);
}  // namespace presearch_rewards

namespace presearch {

enum PresearchNetworkDelegateEventType {
  kOnBeforeRequest,
...
struct PresearchRequestInfo {
  PresearchRequestInfo();

  // For tests, should not be used directly.
  explicit PresearchRequestInfo(const GURL& url);
  ...
  bool allow_presearch_shields = true;
  ...
  PresearchNetworkDelegateEventType event_type = kUnknownEventType;
  ...
  static std::shared_ptr<presearch::PresearchRequestInfo> MakeCTX(
 ...
 private:
  ...
  friend class ::PresearchRequestHandler;

  GURL* new_url = nullptr;

  DISALLOW_COPY_AND_ASSIGN(PresearchRequestInfo);
};
...
#endif  // PRESEARCH_BROWSER_NET_URL_CONTEXT_H_
