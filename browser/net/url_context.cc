...
PresearchRequestInfo::PresearchRequestInfo() = default;

PresearchRequestInfo::PresearchRequestInfo(const GURL& url) : request_url(url) {}

PresearchRequestInfo::~PresearchRequestInfo() = default;

// static
std::shared_ptr<presearch::PresearchRequestInfo> PresearchRequestInfo::MakeCTX(
    const network::ResourceRequest& request,
    int render_process_id,
    int frame_tree_node_id,
    uint64_t request_identifier,
    content::BrowserContext* browser_context,
    std::shared_ptr<presearch::PresearchRequestInfo> old_ctx) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);

  auto ctx = std::make_shared<presearch::PresearchRequestInfo>();
  ...
  // (See |PresearchProxyingWebSocket|).
  if (ctx->tab_origin.is_empty()) {
    ctx->tab_origin = presearch_shields::PresearchShieldsWebContentsObserver::
  ...
  ctx->allow_presearch_shields =
      presearch_shields::GetPresearchShieldsEnabled(map, ctx->tab_origin);
  ctx->allow_ads = presearch_shields::GetAdControlType(map, ctx->tab_origin) ==
                   presearch_shields::ControlType::ALLOW;
  ctx->allow_http_upgradable_resource =
      !presearch_shields::GetHTTPSEverywhereEnabled(map, ctx->tab_origin);

  // HACK: after we fix multiple creations of PresearchRequestInfo we should
  // use only tab_origin. Since we recreate PresearchRequestInfo during consequent
  ...
  ctx->allow_referrers = presearch_shields::AllowReferrers(
      map,
  ...
  // presearch_proxying_url_loader_factory.cc is refactored. See
  // PresearchProxyingURLLoaderFactory::InProgressRequest::UpdateRequestInfo().
  if (old_ctx) {
    ctx->internal_redirect = old_ctx->internal_redirect;
    ctx->redirect_source = old_ctx->redirect_source;
  }
