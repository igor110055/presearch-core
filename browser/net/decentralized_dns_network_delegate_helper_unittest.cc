...
TEST_F(DecentralizedDnsNetworkDelegateHelperTest,
  ...
  int rc = OnBeforeURLRequest_DecentralizedDnsPreRedirectWork(
      ResponseCallback(), presearch_request_info);
  EXPECT_EQ(rc, net::OK);
  ...
  presearch_request_info->browser_context = profile()->GetPrimaryOTRProfile();
  ...
  // TLD is not .crypto
  presearch_request_info->request_url = GURL("http://test.com");
  rc = OnBeforeURLRequest_DecentralizedDnsPreRedirectWork(ResponseCallback(),
                                                          presearch_request_info);
  EXPECT_EQ(rc, net::OK);
  EXPECT_TRUE(presearch_request_info->new_url_spec.empty());
  presearch_request_info->request_url = url;
...
TEST_F(DecentralizedDnsNetworkDelegateHelperTest,
  ...
  OnBeforeURLRequest_DecentralizedDnsRedirectWork(
      ResponseCallback(), presearch_request_info, false, "");
  EXPECT_TRUE(presearch_request_info->new_url_spec.empty());

  OnBeforeURLRequest_DecentralizedDnsRedirectWork(ResponseCallback(),
                                                  presearch_request_info, true, "");
  EXPECT_TRUE(presearch_request_info->new_url_spec.empty());
  ...
  OnBeforeURLRequest_DecentralizedDnsRedirectWork(
      ResponseCallback(), presearch_request_info, true, result);
  EXPECT_EQ("ipfs://QmWrdNJWMbvRxxzLhojVKaBDswS4KNVM7LvjsN7QbDrvka",
            presearch_request_info->new_url_spec);
  ...
  OnBeforeURLRequest_DecentralizedDnsRedirectWork(
      ResponseCallback(), presearch_request_info, true, result);
  EXPECT_EQ("https://fallback1.test.com/", presearch_request_info->new_url_spec);
  ...
  OnBeforeURLRequest_DecentralizedDnsRedirectWork(
      ResponseCallback(), presearch_request_info, true, result);
  EXPECT_EQ("https://fallback2.test.com/", presearch_request_info->new_url_spec);
}
