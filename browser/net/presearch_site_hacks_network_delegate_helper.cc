...
void ApplyPotentialQueryStringFilter(std::shared_ptr<PresearchRequestInfo> ctx) {
  SCOPED_UMA_HISTOGRAM_TIMER("Presearch.SiteHacks.QueryFilter");

  if (!ctx->allow_presearch_shields) {
...
bool ApplyPotentialReferrerBlock(std::shared_ptr<PresearchRequestInfo> ctx) {
  ...
  if (presearch_shields::MaybeChangeReferrer(
          ctx->allow_referrers, ctx->allow_presearch_shields, GURL(ctx->referrer),
...
int OnBeforeStartTransaction_SiteHacksWork(
  ...
  if (IsUAWhitelisted(ctx->request_url)) {
    ...
    if (headers->GetHeader(kUserAgentHeader, &user_agent)) {
      ...
      // during redirects.
      if (std::string::npos == user_agent.find("Presearch")) {
  ...
  // |PresearchContentBrowserClient::MaybeHideReferrer|).
  if (!ctx->allow_referrers && ctx->allow_presearch_shields &&
      ctx->redirect_source.is_valid() &&
      ctx->resource_type == blink::mojom::ResourceType::kMainFrame &&
      !presearch_shields::IsSameOriginNavigation(ctx->redirect_source,
                                             ctx->request_url)) {
    // This is a hack that notifies the network layer.
    ctx->removed_headers.insert("X-Presearch-Cap-Referrer");
  }
  return net::OK;
}
