...
void ShouldBlockAdOnTaskRunner(std::shared_ptr<PresearchRequestInfo> ctx,
  ...
  g_presearch_browser_process->ad_block_service()->ShouldStartRequest(
...
void OnShouldBlockAdResult(const ResponseCallback& next_callback,
  ...
  if (ctx->blocked_by == kAdBlocked) {
    presearch_shields::DispatchBlockedEvent(
        ctx->request_url, ctx->render_frame_id, ctx->render_process_id,
        ctx->frame_tree_node_id, presearch_shields::kAds);
  }
  next_callback.Run();
}
...
class AdblockCnameResolveHostClient : public network::mojom::ResolveHostClient {
 ...
 public:
  ...
  void OnComplete(
    ...
    UMA_HISTOGRAM_TIMES("Presearch.ShieldsCNAMEBlocking.TotalResolutionTime",
...
void OnBeforeURLRequestAdBlockTP(const ResponseCallback& next_callback,
  ...
  scoped_refptr<base::SequencedTaskRunner> task_runner =
      g_presearch_browser_process->ad_block_service()->GetTaskRunner();
...
int OnBeforeURLRequest_AdBlockTPPreWork(const ResponseCallback& next_callback,
  ...
  if (ctx->request_url.is_empty() ||
      ...
      !ctx->allow_presearch_shields || ctx->allow_ads ||
      ctx->resource_type == PresearchRequestInfo::kInvalidResourceType) {
    return net::OK;
  }
  ...
  if (ctx->initiator_url.SchemeIs(kChromeExtensionScheme) &&
      !base::FeatureList::IsEnabled(
          ::presearch_shields::features::kPresearchExtensionNetworkBlocking)) {
    return net::OK;
  }
