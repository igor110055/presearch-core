...
PresearchProxyingWebSocket::PresearchProxyingWebSocket(
    ...
    PresearchRequestHandler* handler,
...
// static
PresearchProxyingWebSocket* PresearchProxyingWebSocket::ProxyWebSocket(
...
void PresearchProxyingWebSocket::Start() {
  ...
  if (proxy_has_extra_headers()) {
    continuation = base::BindRepeating(
        &PresearchProxyingWebSocket::ContinueToStartRequest,
        weak_factory_.GetWeakPtr());
  } else {
    continuation = base::BindRepeating(
        &PresearchProxyingWebSocket::OnBeforeRequestComplete,
        weak_factory_.GetWeakPtr());
  }

  ctx_ = presearch::PresearchRequestInfo::MakeCTX(request_, process_id_,
...
PresearchProxyingWebSocket::web_socket_factory() {
  return base::BindOnce(&PresearchProxyingWebSocket::WebSocketFactoryRun,
                        base::Unretained(this));
}

mojo::Remote<network::mojom::WebSocketHandshakeClient>
PresearchProxyingWebSocket::handshake_client() {
  return std::move(forwarding_handshake_client_);
}

bool PresearchProxyingWebSocket::proxy_has_extra_headers() {
  return proxy_trusted_header_client_.is_bound();
}

void PresearchProxyingWebSocket::WebSocketFactoryRun(
    const GURL& url,
...
void PresearchProxyingWebSocket::OnOpeningHandshakeStarted(
...
void PresearchProxyingWebSocket::ContinueToHeadersReceived() {
  auto continuation = base::BindRepeating(
      &PresearchProxyingWebSocket::OnHeadersReceivedComplete,
      weak_factory_.GetWeakPtr());
  ctx_ = presearch::PresearchRequestInfo::MakeCTX(request_, process_id_,
                                          frame_tree_node_id_, request_id_,
                                          browser_context_, ctx_);
...
void PresearchProxyingWebSocket::OnFailure(const std::string& message,
                                       int32_t net_error,
                                       int32_t response_code) {}

void PresearchProxyingWebSocket::OnConnectionEstablished(
...
void PresearchProxyingWebSocket::OnAuthRequired(
...
void PresearchProxyingWebSocket::OnBeforeSendHeaders(
...
void PresearchProxyingWebSocket::OnHeadersReceived(
...
void PresearchProxyingWebSocket::OnBeforeRequestComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnBeforeSendHeaders(
        request_.headers,
        base::BindOnce(
            &PresearchProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void PresearchProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy(
    int error_code,
  ...
  auto continuation = base::BindRepeating(
      &PresearchProxyingWebSocket::OnBeforeSendHeadersComplete,
      weak_factory_.GetWeakPtr());
...
void PresearchProxyingWebSocket::OnBeforeSendHeadersComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
         !receiver_as_handshake_client_.is_bound());
...
void PresearchProxyingWebSocket::ContinueToStartRequest(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  receiver_as_handshake_client_.set_disconnect_with_reason_handler(
      base::BindOnce(&PresearchProxyingWebSocket::OnMojoConnectionError,
                     base::Unretained(this)));
}

void PresearchProxyingWebSocket::OnHeadersReceivedCompleteFromProxy(
    int error_code,
...
void PresearchProxyingWebSocket::OnHeadersReceivedComplete(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnHeadersReceived(
        ...
        remote_endpoint_,
        base::BindOnce(
            &PresearchProxyingWebSocket::OnHeadersReceivedCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void PresearchProxyingWebSocket::PauseIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Pause();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Pause();
}

void PresearchProxyingWebSocket::ResumeIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Resume();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Resume();
}

void PresearchProxyingWebSocket::OnError(int error_code) {
  if (!is_done_) {
    is_done_ = true;
  }
...
// ResetWithReason
void PresearchProxyingWebSocket::OnMojoConnectionError(
