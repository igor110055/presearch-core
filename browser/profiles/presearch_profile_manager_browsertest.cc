...
class PresearchProfileManagerTest : public InProcessBrowserTest {
 public:
  PresearchProfileManagerTest() {
#if BUILDFLAG(IPFS_ENABLED)
...
// to presearch profile names (Profile X).
IN_PROC_BROWSER_TEST_F(PresearchProfileManagerTest,
...
IN_PROC_BROWSER_TEST_F(PresearchProfileManagerTest,
  ...
  EXPECT_NE(
      presearch_rewards::RewardsServiceFactory::GetForProfile(profile), nullptr);
  EXPECT_EQ(
      presearch_rewards::RewardsServiceFactory::GetForProfile(otr_profile),
      nullptr);
  EXPECT_EQ(
      presearch_rewards::RewardsServiceFactory::GetForProfile(guest_profile),
      nullptr);

  EXPECT_NE(presearch_ads::AdsServiceFactory::GetForProfile(profile), nullptr);
  EXPECT_EQ(presearch_ads::AdsServiceFactory::GetForProfile(otr_profile),
            nullptr);
  EXPECT_EQ(presearch_ads::AdsServiceFactory::GetForProfile(guest_profile),
            nullptr);
