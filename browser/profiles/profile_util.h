...
#ifndef PRESEARCH_BROWSER_PROFILES_PROFILE_UTIL_H_
#define PRESEARCH_BROWSER_PROFILES_PROFILE_UTIL_H_
...
// Called from PresearchAppearanceHandler and PresearchNewTabMessageHandler
...
// browser/presearch_browser_main_extra_parts.cc
void RecordInitialP3AValues(Profile* profile);

// Used for capturing the value of kPresearchCurrentDataVersion so that the
...
// GetBrowserContextRedirectedInIncognito or equivalent
#define PRESEARCH_GET_BROWSER_CONTEXT_TO_USE_WITHOUT_REDIRECT                      \
  if (presearch::IsSessionProfile(context)) {                                      \
    auto* parent = presearch::GetParentProfile(context);                           \
    context =                                                                  \
        context->IsOffTheRecord() ? parent->GetPrimaryOTRProfile() : parent; \
  }

#endif  // PRESEARCH_BROWSER_PROFILES_PROFILE_UTIL_H_
