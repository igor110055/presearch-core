...
PresearchSpellingOptionsSubMenuObserver::PresearchSpellingOptionsSubMenuObserver(
...
void PresearchSpellingOptionsSubMenuObserver::InitMenu(
    const content::ContextMenuParams& params) {
  // Let Chromium build the submenu.
  SpellingOptionsSubMenuObserver::InitMenu(params);

  // Assumptions:
  // 1. Use of spelling service is disabled in Presearch profile preferences.
...
void PresearchSpellingOptionsSubMenuObserver::SetGtestMode(
    PresearchSpellingOptionsSubMenuObserver::GtestMode mode) {
  gtest_mode_ = mode;
}
