...
// accesses resources.
class PresearchSpellingOptionsSubMenuObserverTest : public InProcessBrowserTest {
 public:
  PresearchSpellingOptionsSubMenuObserverTest() {}
  ~PresearchSpellingOptionsSubMenuObserverTest() override {}
  ...
  void Reset(bool incognito = false,
             PresearchSpellingOptionsSubMenuObserver::GtestMode gtest_mode =
                 PresearchSpellingOptionsSubMenuObserver::GTEST_MODE_NORMAL) {
    Clear();
    menu_.reset(new PresearchMockRenderViewContextMenu(
        incognito ? browser()->profile()->GetPrimaryOTRProfile()
                  : browser()->profile()));
    std::unique_ptr<PresearchSpellingOptionsSubMenuObserver> observer =
        std::make_unique<PresearchSpellingOptionsSubMenuObserver>(menu_.get(),
                                                              menu_.get(), 1);
  ...
  void CheckExpected() {
    for (size_t i = 0; i < menu()->GetMenuSize(); ++i) {
      PresearchMockRenderViewContextMenu::MockMenuItem item;
  ...
  PresearchMockRenderViewContextMenu* menu() { return menu_.get(); }
  SpellingOptionsSubMenuObserver* observer() { return observer_.get(); }

 private:
  std::unique_ptr<PresearchMockRenderViewContextMenu> menu_;
  std::unique_ptr<SpellingOptionsSubMenuObserver> observer_;

  DISALLOW_COPY_AND_ASSIGN(PresearchSpellingOptionsSubMenuObserverTest);
};

// Tests that "Ask Presearch for suggestions" isn't shown in the menu and the menu
// doesn't end with a separator.

IN_PROC_BROWSER_TEST_F(PresearchSpellingOptionsSubMenuObserverTest,
                       CheckAskPresearchNotShown) {
  ...
  // Test empty submenu.
  Reset(false, PresearchSpellingOptionsSubMenuObserver::GTEST_MODE_EMPTY_SUBMENU);
  InitMenu(false, "en-US", std::vector<std::string>());
...
IN_PROC_BROWSER_TEST_F(PresearchSpellingOptionsSubMenuObserverTest,
                       CheckAskPresearchNotShownIncognito) {
  // Test with spellcheck enabled.
  Reset(true);
  InitMenu(true, "en-US", std::vector<std::string>(1, "en-US"));
