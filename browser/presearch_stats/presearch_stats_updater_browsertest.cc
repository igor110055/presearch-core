...
class PresearchStatsUpdaterBrowserTest : public InProcessBrowserTest {
 public:
  void SetUp() override {
    presearch::PresearchReferralsService::SetReferralInitializedCallbackForTesting(
        base::BindRepeating(
            &PresearchStatsUpdaterBrowserTest::OnReferralInitialized,
            base::Unretained(this)));
    presearch_stats::PresearchStatsUpdater::SetStatsUpdatedCallbackForTesting(
        base::BindRepeating(
            &PresearchStatsUpdaterBrowserTest::OnStandardStatsUpdated,
            base::Unretained(this)));
    presearch_stats::PresearchStatsUpdater::SetStatsThresholdCallbackForTesting(
        base::BindRepeating(
            &PresearchStatsUpdaterBrowserTest::OnThresholdStatsUpdated,
            base::Unretained(this)));
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    command_line->AppendSwitchASCII(
        presearch_stats::switches::kPresearchStatsUpdaterServer,
  ...
  void SetBaseUpdateURLForTest() {
    ...
    env->SetVar("PRESEARCH_REFERRALS_SERVER",
                embedded_test_server()->host_port_pair().ToString());
    env->SetVar("PRESEARCH_REFERRALS_LOCAL", "1");  // use http for local testing
  }
  ...
  void OnStandardStatsUpdated(const GURL& update_url) {
    ...
    // We get //1/usage/presearch-core here, so ignore the first slash.
    EXPECT_STREQ(update_url.path().c_str() + 1, "/1/usage/presearch-core");
  ...
  void OnThresholdStatsUpdated(const GURL& update_url) {
    ...
    // We get //1/usage/presearch-core-threshold here, so ignore the first slash.
    EXPECT_STREQ(update_url.path().c_str() + 1,
                 "/1/usage/presearch-core-threshold");
    update_url_ = update_url.spec();
  }
...
IN_PROC_BROWSER_TEST_F(PresearchStatsUpdaterBrowserTest,
...
IN_PROC_BROWSER_TEST_F(PresearchStatsUpdaterBrowserTest,
  ...
  EXPECT_TRUE(
      g_presearch_browser_process->presearch_stats_updater()->MaybeDoThresholdPing(3));
...
class PresearchStatsUpdaterReferralCodeBrowserTest
    : public PresearchStatsUpdaterBrowserTest {
 public:
  void SetUp() override {
    ...
    presearch::PresearchReferralsService::SetPromoFilePathForTesting(promo_code_file);
    PresearchStatsUpdaterBrowserTest::SetUp();
  }
...
IN_PROC_BROWSER_TEST_F(PresearchStatsUpdaterReferralCodeBrowserTest,
