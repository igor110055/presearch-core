...
namespace ui {
// This resets dark mode to os theme when user changes presearch theme from
// dark or light to Same as Windows by using the value of registry because
// we changed ui::NativeTheme::dark_mode_ explicitly for using presearch theme
// like a system theme.
...
void SetSystemDarkMode(PresearchDarkModeType type) {
  ...
  ui::IgnoreSystemDarkModeChange(
      type != dark_mode::PresearchDarkModeType::PRESEARCH_DARK_MODE_TYPE_DEFAULT);
  ...
  internal::SetSystemDarkModeForNonDefaultMode(
      type == PresearchDarkModeType::PRESEARCH_DARK_MODE_TYPE_DARK);
}
