...
// TODO(simonhong): Get colors from presearch's palette.
// Omnibox text colors
...
PresearchThemeHelper::~PresearchThemeHelper() = default;

SkColor PresearchThemeHelper::GetDefaultColor(
    int id,
    bool incognito,
    const CustomThemeSupplier* theme_supplier) const {
  const bool is_presearch_theme_properties =
      PresearchThemeProperties::IsPresearchThemeProperties(id);
#if defined(OS_LINUX)
  // IF gtk theme is selected, respect it.
  if (!is_presearch_theme_properties && IsUsingSystemTheme(theme_supplier)) {
    return ThemeHelper::GetDefaultColor(id, incognito, theme_supplier);
  }
#endif

  if (!is_presearch_theme_properties && theme_supplier)
    return ThemeHelper::GetDefaultColor(id, incognito, theme_supplier);

  // Presearch Tor profiles are always 'incognito' (for now)
  if (!incognito && (is_tor_ || is_guest_)) {
    incognito = true;
  }
  const dark_mode::PresearchDarkModeType type =
      dark_mode::GetActivePresearchDarkModeType();
  const base::Optional<SkColor> presearchColor =
      MaybeGetDefaultColorForPresearchUi(id, incognito, is_tor_, type);
  if (presearchColor) {
    return presearchColor.value();
  }
  // Make sure we fallback to Chrome's dark theme (incognito) for our dark theme
  if (type == dark_mode::PresearchDarkModeType::PRESEARCH_DARK_MODE_TYPE_DARK) {
    incognito = true;
  }

  DCHECK(!is_presearch_theme_properties);
  return ThemeHelper::GetDefaultColor(id, incognito, theme_supplier);
}

base::Optional<SkColor> PresearchThemeHelper::GetOmniboxColor(
    int id,
    bool incognito,
  ...
  const bool dark = dark_mode::GetActivePresearchDarkModeType() ==
