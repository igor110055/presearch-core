...
using PresearchThemeServiceTest = InProcessBrowserTest;
...
class PresearchThemeServiceTestWithoutSystemTheme : public InProcessBrowserTest {
 public:
  PresearchThemeServiceTestWithoutSystemTheme() {
    dark_mode::SetUseSystemDarkModeEnabledForTest(false);
  }
};

IN_PROC_BROWSER_TEST_F(PresearchThemeServiceTestWithoutSystemTheme,
                       PresearchThemeChangeTest) {
  ...
  auto test_theme_property = PresearchThemeProperties::COLOR_FOR_TEST;
  ...
  EXPECT_EQ(PresearchThemeProperties::kLightColorForTest,
  ...
  EXPECT_EQ(
      dark_mode::PresearchDarkModeType::PRESEARCH_DARK_MODE_TYPE_DARK,
      dark_mode::GetActivePresearchDarkModeType());
  EXPECT_EQ(PresearchThemeProperties::kDarkColorForTest,
            tp.GetColor(test_theme_property));

  // Test dark theme private
  EXPECT_EQ(PresearchThemeProperties::kPrivateColorForTest,
            tp_private.GetColor(test_theme_property));
}

// Test whether appropriate native/web theme observer is called when presearch theme
// is changed.
IN_PROC_BROWSER_TEST_F(PresearchThemeServiceTest, ThemeObserverTest) {
...
IN_PROC_BROWSER_TEST_F(PresearchThemeServiceTest, SystemThemeChangeTest) {
  ...
  if (dark_mode::SystemDarkModeEnabled()) {
    dark_mode::SetPresearchDarkModeType(
        dark_mode::PresearchDarkModeType::PRESEARCH_DARK_MODE_TYPE_DEFAULT);
...
IN_PROC_BROWSER_TEST_F(PresearchThemeServiceTest, DarkModeChangeByRegTest) {
