...
constexpr char kSearchCountPrefName[] = "presearch.weekly_storage.search_count";
...
void RecordSearchEventP3A(uint64_t number_of_searches) {
  ...
  UMA_HISTOGRAM_EXACT_LINEAR("Presearch.Omnibox.SearchCount", answer,
                             base::size(kIntervals));
}

}  // namespace

PresearchOmniboxClientImpl::PresearchOmniboxClientImpl(
...
void PresearchOmniboxClientImpl::RegisterPrefs(PrefRegistrySimple* registry) {
  registry->RegisterListPref(kSearchCountPrefName);
}

const AutocompleteSchemeClassifier&
PresearchOmniboxClientImpl::GetSchemeClassifier() const {
  return scheme_classifier_;
}

bool PresearchOmniboxClientImpl::IsAutocompleteEnabled() const {
  return profile_->GetPrefs()->GetBoolean(kAutocompleteEnabled);
}

void PresearchOmniboxClientImpl::OnInputAccepted(const AutocompleteMatch& match) {
