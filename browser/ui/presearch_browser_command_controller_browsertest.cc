...
using PresearchBrowserCommandControllerTest = InProcessBrowserTest;

IN_PROC_BROWSER_TEST_F(PresearchBrowserCommandControllerTest,
                       PresearchCommandsEnableTest) {
  // Test normal browser's presearch commands status.
  auto* command_controller = browser()->command_controller();
#if BUILDFLAG(PRESEARCH_REWARDS_ENABLED)
  EXPECT_TRUE(command_controller->IsCommandEnabled(IDC_SHOW_PRESEARCH_REWARDS));
#else
  EXPECT_FALSE(command_controller->IsCommandEnabled(IDC_SHOW_PRESEARCH_REWARDS));
#endif

  EXPECT_TRUE(command_controller->IsCommandEnabled(IDC_SHOW_PRESEARCH_ADBLOCK));
  ...
  if (switches::IsSyncAllowedByFlag())
    EXPECT_TRUE(command_controller->IsCommandEnabled(IDC_SHOW_PRESEARCH_SYNC));
  else
    EXPECT_FALSE(command_controller->IsCommandEnabled(IDC_SHOW_PRESEARCH_SYNC));
#else
  EXPECT_FALSE(command_controller->IsCommandEnabled(IDC_SHOW_PRESEARCH_SYNC));
#endif

#if BUILDFLAG(PRESEARCH_WALLET_ENABLED)
  EXPECT_TRUE(command_controller->IsCommandEnabled(IDC_SHOW_PRESEARCH_WALLET));
#else
  EXPECT_FALSE(command_controller->IsCommandEnabled(IDC_SHOW_PRESEARCH_WALLET));
#endif
  ...
  // Create private browser and test its presearch commands status.
  ...
  // Create guest browser and test its presearch commands status.
