...
PresearchImportDataHandler::PresearchImportDataHandler() : weak_factory_(this) {}
PresearchImportDataHandler::~PresearchImportDataHandler() = default;

void PresearchImportDataHandler::StartImport(
  ...
  if (source_profile.importer_type == importer::TYPE_SAFARI) {
    // Start import if Presearch has full disk access permission.
    ...
    base::PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&PresearchImportDataHandler::OnGetDiskAccessPermission,
...
void PresearchImportDataHandler::OnGetDiskAccessPermission(
    const importer::SourceProfile& source_profile,
    uint16_t imported_items,
    bool allowed) {
  if (!allowed) {
...
void PresearchImportDataHandler::DidStopLoading() {
  Observe(nullptr);
