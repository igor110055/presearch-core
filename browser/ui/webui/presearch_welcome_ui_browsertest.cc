...
using PresearchWelcomeUIBrowserTest = InProcessBrowserTest;

// Check whether startup url at first run is our welcome page.
IN_PROC_BROWSER_TEST_F(PresearchWelcomeUIBrowserTest, PRE_StartupURLTest) {
...
IN_PROC_BROWSER_TEST_F(PresearchWelcomeUIBrowserTest, StartupURLTest) {
  Browser* new_browser = OpenNewBrowser(browser()->profile());
  ASSERT_TRUE(new_browser);
  TabStripModel* tab_strip = new_browser->tab_strip_model();
  ASSERT_EQ(1, tab_strip->count());
