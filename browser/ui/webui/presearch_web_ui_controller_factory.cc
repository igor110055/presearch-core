...
#include "presearch/browser/ui/webui/presearch_welcome_ui.h"
#include "presearch/browser/ui/webui/new_tab_page/presearch_new_tab_ui.h"
#endif

#if BUILDFLAG(PRESEARCH_REWARDS_ENABLED)
#include "presearch/browser/ui/webui/presearch_tip_ui.h"
...
#if BUILDFLAG(PRESEARCH_WALLET_ENABLED) && !defined(OS_ANDROID)
#include "presearch/browser/ui/webui/presearch_wallet_ui.h"
#endif

#if BUILDFLAG(IPFS_ENABLED)
#include "presearch/browser/ui/webui/ipfs_ui.h"
#include "presearch/components/ipfs/ipfs_utils.h"
#endif

#if BUILDFLAG(ENABLE_TOR)
#include "presearch/browser/ui/webui/tor_internals_ui.h"
#endif
...
WebUIController* NewWebUI(WebUI* web_ui, const GURL& url) {
  ...
  if (host == kAdblockHost) {
    return new PresearchAdblockUI(web_ui, url.host());
    ...
    return new PresearchWalletUI(web_ui, url.host());
#endif  // BUILDFLAG(PRESEARCH_WALLET_ENABLED)
#if BUILDFLAG(PRESEARCH_REWARDS_ENABLED)
  } else if (host == kRewardsPageHost) {
    return new PresearchRewardsPageUI(web_ui, url.host());
  } else if (host == kRewardsInternalsHost) {
    return new PresearchRewardsInternalsUI(web_ui, url.host());
#if !defined(OS_ANDROID)
  } else if (host == kTipHost) {
    return new PresearchTipUI(web_ui, url.host());
#endif  // !defined(OS_ANDROID)
#endif  // BUILDFLAG(PRESEARCH_REWARDS_ENABLED)
#if !defined(OS_ANDROID)
  } else if (host == kWelcomeHost) {
    return new PresearchWelcomeUI(web_ui, url.host());
  } else if (host == chrome::kChromeUISettingsHost) {
    return new PresearchSettingsUI(web_ui, url.host());
  } else if (host == chrome::kChromeUINewTabHost) {
    return new PresearchNewTabUI(web_ui, url.host());
#endif  // !defined(OS_ANDROID)
#if BUILDFLAG(ENABLE_TOR)
...
bool ShouldBlockRewardsWebUI(
  ...
  if (!base::FeatureList::IsEnabled(features::kPresearchRewards)) {
    return true;
  }
...
WebUI::TypeID PresearchWebUIControllerFactory::GetWebUIType(
      content::BrowserContext* browser_context, const GURL& url) {
#if defined(OS_ANDROID)
...
PresearchWebUIControllerFactory::CreateWebUIControllerForURL(WebUI* web_ui,
                                                         const GURL& url) {
...
// static
PresearchWebUIControllerFactory* PresearchWebUIControllerFactory::GetInstance() {
  return base::Singleton<PresearchWebUIControllerFactory>::get();
}

PresearchWebUIControllerFactory::PresearchWebUIControllerFactory() {
}
