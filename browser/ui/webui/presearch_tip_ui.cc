...
#include "presearch/components/presearch_rewards/resources/grit/presearch_rewards_tip_generated_map.h"
...
using presearch_rewards::RewardsServiceObserver;
...
class TipMessageHandler : public WebUIMessageHandler,
 ...
 private:
  ...
  presearch_ads::AdsService* ads_service_ = nullptr;  // NOT OWNED
...
void TipMessageHandler::TweetTip(const base::ListValue* args) {
  ...
  const std::string comment = l10n_util::GetStringFUTF8(
      IDS_PRESEARCH_REWARDS_LOCAL_COMPLIMENT_TWEET,
      base::UTF8ToUTF16(name));

  const std::string hashtag = l10n_util::GetStringUTF8(
      IDS_PRESEARCH_REWARDS_LOCAL_COMPLIMENT_TWEET_HASHTAG);
...
PresearchTipUI::PresearchTipUI(content::WebUI* web_ui, const std::string& name)
  ...
  CreateAndAddWebUIDataSource(web_ui, name, kPresearchRewardsTipGenerated,
                              kPresearchRewardsTipGeneratedSize,
                              IDR_PRESEARCH_REWARDS_TIP_HTML);

  web_ui->AddMessageHandler(std::make_unique<TipMessageHandler>());
}

PresearchTipUI::~PresearchTipUI() = default;
