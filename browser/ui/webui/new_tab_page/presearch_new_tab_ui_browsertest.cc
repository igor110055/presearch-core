...
#include "presearch/browser/extensions/presearch_extension_functional_test.h"
...
class PresearchNewTabUIBrowserTest : public extensions::ExtensionFunctionalTest {
 public:
...
IN_PROC_BROWSER_TEST_F(PresearchNewTabUIBrowserTest, StartupURLTest) {
...
// This test simply checks that by default the Presearch new tab page is used.
// It does this by loading the newtab page and then checking if
// window.presearch_new_tab exists.
IN_PROC_BROWSER_TEST_F(PresearchNewTabUIBrowserTest, PresearchNewTabIsDefault) {
  auto* contents = browser()->tab_strip_model()->GetActiveWebContents();
  GURL new_tab_url(chrome::kChromeUINewTabURL);
  ui_test_utils::NavigateToURL(browser(), new_tab_url);
  WaitForLoadStop(contents);
  bool is_presearch_new_tab = false;
  ASSERT_TRUE(content::ExecuteScriptAndExtractBool(
      contents,
      "window.domAutomationController.send(!!document.querySelector(`html[data-test-id='presearch-new-tab-page']`))",  // NOLINT
      &is_presearch_new_tab));
  ASSERT_TRUE(is_presearch_new_tab);
}
...
IN_PROC_BROWSER_TEST_F(PresearchNewTabUIBrowserTest, NewTabPageLocationOverride) {
