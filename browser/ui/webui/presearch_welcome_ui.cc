...
#include "presearch/browser/ui/webui/settings/presearch_import_data_handler.h"
#include "presearch/common/pref_names.h"
#include "presearch/common/webui_url_constants.h"
#include "presearch/components/presearch_welcome/resources/grit/presearch_welcome_generated_map.h"
...
void RecordP3AHistogram(int screen_number, bool finished) {
  ...
  UMA_HISTOGRAM_EXACT_LINEAR("Presearch.Welcome.InteractionStatus", answer, 3);
}
...
PresearchWelcomeUI::PresearchWelcomeUI(content::WebUI* web_ui, const std::string& name)
    : WebUIController(web_ui) {
  CreateAndAddWebUIDataSource(web_ui, name, kPresearchWelcomeGenerated,
                              kPresearchWelcomeGeneratedSize,
                              IDR_PRESEARCH_WELCOME_HTML,
                              /*disable_trusted_types_csp=*/true);

  web_ui->AddMessageHandler(std::make_unique<WelcomeDOMHandler>());
  web_ui->AddMessageHandler(
      std::make_unique<settings::PresearchImportDataHandler>());
...
PresearchWelcomeUI::~PresearchWelcomeUI() {
}
