...
#include "presearch/browser/ui/views/sidebar/sidebar_items_scroll_view.h"
...
#include "presearch/browser/ui/views/sidebar/sidebar_item_drag_context.h"
#include "presearch/browser/ui/views/sidebar/sidebar_item_view.h"
#include "presearch/browser/ui/views/sidebar/sidebar_items_contents_view.h"
...
constexpr char kSidebarItemDragType[] = "presearch/sidebar-item";
...
class SidebarItemsArrowView : public views::ImageButton {
 public:
  SidebarItemsArrowView() {
    ...
    focus_ring()->SetColor(gfx::kPresearchBlurple300);
  }
  ...
  void OnPaintBackground(gfx::Canvas* canvas) override {
    if (const ui::ThemeProvider* theme_provider = GetThemeProvider()) {
      const SkColor background_color = theme_provider->GetColor(
          PresearchThemeProperties::COLOR_SIDEBAR_BACKGROUND);
      ...
      if (GetState() == STATE_HOVERED) {
        const SkColor hovered_bg_color = theme_provider->GetColor(
            PresearchThemeProperties::COLOR_SIDEBAR_ARROW_BACKGROUND_HOVERED);
        cc::PaintFlags flags;
...
SidebarItemsScrollView::SidebarItemsScrollView(PresearchBrowser* browser)
    : browser_(browser),
...
void SidebarItemsScrollView::UpdateArrowViewsTheme() {
  if (const ui::ThemeProvider* theme_provider = GetThemeProvider()) {
    const SkColor arrow_normal = theme_provider->GetColor(
        PresearchThemeProperties::COLOR_SIDEBAR_ARROW_NORMAL);
    const SkColor arrow_disabled = theme_provider->GetColor(
        PresearchThemeProperties::COLOR_SIDEBAR_ARROW_DISABLED);
