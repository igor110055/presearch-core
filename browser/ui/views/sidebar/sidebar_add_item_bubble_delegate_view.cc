...
#include "presearch/browser/ui/views/sidebar/bubble_border_with_arrow.h"
#include "presearch/browser/ui/views/sidebar/sidebar_bubble_background.h"
    ...
    if (theme_provider_) {
      SetTextColor(
          ...
          theme_provider->GetColor(
              PresearchThemeProperties::COLOR_SIDEBAR_ADD_BUBBLE_ITEM_TEXT_NORMAL));
      SetTextColor(views::Button::STATE_HOVERED,
                   theme_provider->GetColor(
                       PresearchThemeProperties::
...
SidebarAddItemBubbleDelegateView::SidebarAddItemBubbleDelegateView(
  ...
  if (auto* theme_provider =
    ...
    set_color(theme_provider->GetColor(
        PresearchThemeProperties::COLOR_SIDEBAR_ADD_BUBBLE_BACKGROUND));
  }
  AddChildViews();
}
...
void SidebarAddItemBubbleDelegateView::AddChildViews() {
  ...
  if (theme_provider) {
    header->SetEnabledColor(theme_provider->GetColor(
        PresearchThemeProperties::COLOR_SIDEBAR_ADD_BUBBLE_HEADER_TEXT));
  }
  ...
  if (theme_provider) {
    separator->SetColor(theme_provider->GetColor(
        PresearchThemeProperties::COLOR_SIDEBAR_SEPARATOR));
  }
