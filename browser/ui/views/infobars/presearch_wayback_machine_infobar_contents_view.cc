...
PresearchWaybackMachineInfoBarContentsView::PresearchWaybackMachineInfoBarContentsView(
...
void PresearchWaybackMachineInfoBarContentsView::OnThemeChanged() {
  ...
  wayback_spot_graphic_->SetImage(
      rb.GetImageSkiaNamed(ui::NativeTheme::GetInstanceForNativeUi()->
          ShouldUseDarkColors() ? IDR_PRESEARCH_WAYBACK_INFOBAR_DARK
                                : IDR_PRESEARCH_WAYBACK_INFOBAR));
}

void PresearchWaybackMachineInfoBarContentsView::OnWaybackURLFetched(
...
void PresearchWaybackMachineInfoBarContentsView::HideInfobar() {
  ...
  for (size_t i = 0; i < infobar_service->infobar_count(); ++i) {
    ...
    if (infobar->delegate()->GetIdentifier() ==
        PresearchWaybackMachineInfoBarDelegate::WAYBACK_MACHINE_INFOBAR_DELEGATE) {
...
void PresearchWaybackMachineInfoBarContentsView::ButtonPressed() {
...
void PresearchWaybackMachineInfoBarContentsView::InitializeChildren() {
  ...
  auto* label = CreateLabel(
      l10n_util::GetStringUTF16(
          IDS_PRESEARCH_WAYBACK_MACHINE_INFOBAR_PAGE_MISSING_TEXT));
  label->SetFontList(
  ...
  label = CreateLabel(l10n_util::GetStringUTF16(
      IDS_PRESEARCH_WAYBACK_MACHINE_INFOBAR_ASK_ABOUT_CHECK_TEXT));
  views_visible_before_checking_.push_back(label);
  label->SetProperty(
      views::kMarginsKey,
      gfx::Insets(ChromeLayoutProvider::Get()->GetDistanceMetric(
  ...
  label = CreateLabel(
      l10n_util::GetStringUTF16(
          IDS_PRESEARCH_WAYBACK_MACHINE_INFOBAR_NOT_AVAILABLE_TEXT));
  ...
  button_ = new PresearchWaybackMachineInfoBarButtonContainer(base::BindRepeating(
      &PresearchWaybackMachineInfoBarContentsView::ButtonPressed,
...
views::Label* PresearchWaybackMachineInfoBarContentsView::CreateLabel(
...
void PresearchWaybackMachineInfoBarContentsView::UpdateChildrenVisibility(
...
SkColor PresearchWaybackMachineInfoBarContentsView::GetColor(int id) const {
  const auto* theme_provider = GetThemeProvider();
  return theme_provider ? theme_provider->GetColor(id)
                        : gfx::kPlaceholderColor;
}

void PresearchWaybackMachineInfoBarContentsView::FetchWaybackURL() {
  button_->StartThrobber();
  wayback_machine_url_fetcher_.Fetch(contents_->GetVisibleURL());
  Layout();
}

void PresearchWaybackMachineInfoBarContentsView::LoadURL(const GURL& url) {
