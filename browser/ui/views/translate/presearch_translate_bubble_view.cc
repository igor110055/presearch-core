...
PresearchTranslateBubbleView::PresearchTranslateBubbleView(
...
PresearchTranslateBubbleView::PresearchCreateViewBeforeTranslate() {
  ...
  auto dont_ask_button = std::make_unique<views::LabelButton>(
      base::BindRepeating(&PresearchTranslateBubbleView::ButtonPressed,
                          base::Unretained(this), BUTTON_ID_ALWAYS_TRANSLATE),
      l10n_util::GetStringUTF16(IDS_PRESEARCH_TRANSLATE_BUBBLE_DONT_ASK_AGAIN));
  ...
  auto accept_button = std::make_unique<views::MdTextButton>(
      ...
      l10n_util::GetStringUTF16(IDS_PRESEARCH_TRANSLATE_BUBBLE_INSTALL));
  ...
  auto cancel_button = std::make_unique<views::MdTextButton>(
      ...
      l10n_util::GetStringUTF16(IDS_PRESEARCH_TRANSLATE_BUBBLE_CANCEL));
...
void PresearchTranslateBubbleView::InstallGoogleTranslate() {
  if (!web_contents())
    return;
  ...
  PresearchTranslateIconView* translate_icon =
    static_cast<PresearchTranslateIconView*>(
...
void PresearchTranslateBubbleView::DisableOfferTranslatePref() {
  if (!web_contents())
    return;
...
void PresearchTranslateBubbleView::ButtonPressed(ButtonID button_id) {
  switch (button_id) {
    case BUTTON_ID_DONE: {
...
bool PresearchTranslateBubbleView::AcceleratorPressed(
...
bool PresearchTranslateBubbleView::ShouldShowWindowTitle() const {
  return true;
}

void PresearchTranslateBubbleView::Init() {
  TranslateBubbleView::Init();
  removed_translate_view_ = RemoveChildViewT(translate_view_);
  translate_view_ = AddChildView(PresearchCreateViewBeforeTranslate());
}
