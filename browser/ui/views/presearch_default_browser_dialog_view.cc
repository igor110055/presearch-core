...
#include "presearch/browser/ui/views/presearch_default_browser_dialog_view.h"

#include <utility>

#include "base/bind.h"
#include "base/memory/scoped_refptr.h"
#include "presearch/browser/ui/browser_dialogs.h"
...
void ShowDefaultBrowserDialog(Browser* browser) {
  constrained_window::CreateBrowserModalDialogViews(
      new PresearchDefaultBrowserDialogView(), browser->window()->GetNativeWindow())
      ->Show();
}
...
PresearchDefaultBrowserDialogView::PresearchDefaultBrowserDialogView() {
  SetButtonLabel(ui::DIALOG_BUTTON_OK,
                 l10n_util::GetStringUTF16(
                     IDS_PRESEARCH_DEFAULT_BROWSER_DIALOG_OK_BUTTON_LABEL));
  SetButtonLabel(ui::DIALOG_BUTTON_CANCEL,
                 l10n_util::GetStringUTF16(
                     IDS_PRESEARCH_DEFAULT_BROWSER_DIALOG_CANCEL_BUTTON_LABEL));

  SetAcceptCallback(
      base::BindOnce(&PresearchDefaultBrowserDialogView::OnAcceptButtonClicked,
                     base::Unretained(this)));
  SetCancelCallback(
      base::BindOnce(&PresearchDefaultBrowserDialogView::OnCancelButtonClicked,
                     base::Unretained(this)));

  CreateChildViews();
}

PresearchDefaultBrowserDialogView::~PresearchDefaultBrowserDialogView() = default;

void PresearchDefaultBrowserDialogView::CreateChildViews() {
  ...
  header_label_ = AddChildView(std::make_unique<views::Label>(
      l10n_util::GetStringUTF16(IDS_PRESEARCH_DEFAULT_BROWSER_DIALOG_HEADER_TEXT),
      header_font));
  ...
  contents_label_ = AddChildView(std::make_unique<views::Label>(
      l10n_util::GetStringUTF16(IDS_PRESEARCH_DEFAULT_BROWSER_DIALOG_CONTENTS_TEXT),
      contents_font));
  ...
  dont_ask_again_checkbox_ = AddChildView(std::make_unique<views::Checkbox>(
      l10n_util::GetStringUTF16(IDS_PRESEARCH_DEFAULT_BROWSER_DIALOG_DONT_ASK)));
}

std::unique_ptr<views::NonClientFrameView>
PresearchDefaultBrowserDialogView::CreateNonClientFrameView(views::Widget* widget) {
  if (!use_custom_frame())
...
ui::ModalType PresearchDefaultBrowserDialogView::GetModalType() const {
  return ui::MODAL_TYPE_WINDOW;
}

bool PresearchDefaultBrowserDialogView::ShouldShowCloseButton() const {
  return false;
}

void PresearchDefaultBrowserDialogView::OnDialogInitialized() {
  SetButtonRowInsets(gfx::Insets(0, kPadding, kPadding, kPadding));
}

void PresearchDefaultBrowserDialogView::OnCancelButtonClicked() {
  g_browser_process->local_state()->SetBoolean(
      kDefaultBrowserPromptEnabled, !dont_ask_again_checkbox_->GetChecked());
}

void PresearchDefaultBrowserDialogView::OnAcceptButtonClicked() {
