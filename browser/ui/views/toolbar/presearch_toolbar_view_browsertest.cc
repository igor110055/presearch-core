...
class PresearchToolbarViewTest : public InProcessBrowserTest {
 public:
  PresearchToolbarViewTest() = default;
  ~PresearchToolbarViewTest() override = default;
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(PresearchToolbarViewTest);
};

IN_PROC_BROWSER_TEST_F(PresearchToolbarViewTest,
    AvatarButtonNotShownSingleProfile) {
  EXPECT_EQ(false, is_avatar_button_shown());
}

IN_PROC_BROWSER_TEST_F(PresearchToolbarViewTest, AvatarButtonIsShownGuestProfile) {
  // Open a Guest window.
