...
#include "presearch/browser/ui/views/toolbar/speedreader_button.h"
...
#include "presearch/components/speedreader/speedreader_pref_names.h"
#endif
...
PresearchToolbarView::PresearchToolbarView(Browser* browser, BrowserView* browser_view)
    : ToolbarView(browser, browser_view), profile_observer_(this) {}

PresearchToolbarView::~PresearchToolbarView() {}

void PresearchToolbarView::Init() {
  ToolbarView::Init();

  // For non-normal mode, we don't have to more.
  if (display_mode_ != DisplayMode::NORMAL) {
    presearch_initialized_ = true;
    return;
  }
  ...
  edit_bookmarks_enabled_.Init(
      ...
      base::Bind(&PresearchToolbarView::OnEditBookmarksEnabledChanged,
                 base::Unretained(this)));
  // track changes in wide locationbar setting
  location_bar_is_wide_.Init(
      kLocationBarIsWide, profile->GetPrefs(),
      base::Bind(&PresearchToolbarView::OnLocationBarIsWideChanged,
                 base::Unretained(this)));
...
void PresearchToolbarView::OnEditBookmarksEnabledChanged() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
  Update(nullptr);
}

void PresearchToolbarView::OnLocationBarIsWideChanged() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
...
void PresearchToolbarView::OnThemeChanged() {
  ToolbarView::OnThemeChanged();

  if (!presearch_initialized_)
    return;
...
void PresearchToolbarView::OnProfileAdded(const base::FilePath& profile_path) {
  Update(nullptr);
}

void PresearchToolbarView::OnProfileWasRemoved(const base::FilePath& profile_path,
                                           const base::string16& profile_name) {
  Update(nullptr);
}

void PresearchToolbarView::LoadImages() {
...
void PresearchToolbarView::Update(content::WebContents* tab) {
...
void PresearchToolbarView::ShowBookmarkBubble(
    const GURL& url,
    bool already_bookmarked,
    bookmarks::BookmarkBubbleObserver* observer) {
  // Show BookmarkBubble attached to Presearch's bookmark button
...
void PresearchToolbarView::Layout() {
  ToolbarView::Layout();
...
void PresearchToolbarView::ResetLocationBarBounds() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
...
void PresearchToolbarView::ResetButtonBounds() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
