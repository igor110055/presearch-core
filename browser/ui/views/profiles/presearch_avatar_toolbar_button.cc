...
constexpr int kPresearchAvatarButtonHorizontalSpacing = 10;

class PresearchToolbarButtonHighlightPathGenerator
...
PresearchAvatarToolbarButton::PresearchAvatarToolbarButton(Browser* browser)
    : AvatarToolbarButton(browser) {
  // Replace ToolbarButton's highlight path generator.
  views::HighlightPathGenerator::Install(
      this, std::make_unique<PresearchToolbarButtonHighlightPathGenerator>());
}

void PresearchAvatarToolbarButton::SetHighlight(
...
void PresearchAvatarToolbarButton::UpdateColorsAndInsets() {
  ...
  if (delegate_->GetState() == State::kIncognitoProfile || is_tor) {
    ...
    gfx::Insets target_insets =
        ...
        *GetProperty(views::kInternalPaddingKey) +
        gfx::Insets(0, kPresearchAvatarButtonHorizontalSpacing);
    SetBorder(views::CreateEmptyBorder(target_insets));

    constexpr int kPresearchAvatarImageLabelSpacing = 8;
    SetImageLabelSpacing(kPresearchAvatarImageLabelSpacing);
    return;
  }
...
ui::ImageModel PresearchAvatarToolbarButton::GetAvatarIcon(
    ButtonState state,
...
base::string16 PresearchAvatarToolbarButton::GetAvatarTooltipText() const {
  if (browser_->profile()->IsTor())
