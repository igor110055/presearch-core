...
void PresearchIncognitoMenuView::BuildMenu() {
...
void PresearchIncognitoMenuView::AddTorButton() {
  if (ShouldShowTorProfileButton(browser()->profile())) {
    AddFeatureButton(
        l10n_util::GetStringUTF16(IDS_PROFILES_OPEN_TOR_PROFILE_BUTTON),
        base::BindRepeating(&PresearchIncognitoMenuView::OnTorProfileButtonClicked,
                            base::Unretained(this)),
        vector_icons::kLaunchIcon);
  }
}

void PresearchIncognitoMenuView::OnTorProfileButtonClicked() {
  TorProfileManager::SwitchToTorProfile(browser()->profile(),
                                        ProfileManager::CreateCallback());
}

base::string16 PresearchIncognitoMenuView::GetAccessibleWindowTitle() const {
  return browser()->profile()->IsTor()
             ? l10n_util::GetStringUTF16(IDS_TOR_PROFILE_NAME)
             : IncognitoMenuView::GetAccessibleWindowTitle();
}

void PresearchIncognitoMenuView::OnExitButtonClicked() {
