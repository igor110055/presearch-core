...
#include "presearch/browser/ui/views/bookmarks/bookmark_bar_instructions_view.h"
...
#include "presearch/browser/ui/presearch_view_ids.h"
...
BookmarkBarInstructionsView::BookmarkBarInstructionsView(Browser* browser)
    : instructions_(NULL),
      ...
      browser_(browser) {
  SetID(PRESEARCH_VIEW_ID_BOOKMARK_IMPORT_INSTRUCTION_VIEW);
  instructions_ =
...
void BookmarkBarInstructionsView::UpdateColors() {
  ...
  SkColor text_color = theme_provider->GetColor(
      PresearchThemeProperties::COLOR_BOOKMARK_BAR_INSTRUCTIONS_TEXT);
