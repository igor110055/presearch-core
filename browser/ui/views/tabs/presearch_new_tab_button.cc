...
#include "presearch/browser/ui/views/tabs/presearch_new_tab_button.h"
...
// static
const gfx::Size PresearchNewTabButton::kButtonSize{20, 20};

gfx::Size PresearchNewTabButton::CalculatePreferredSize() const {
  // Overriden so that we use Presearch's custom button size
...
SkPath PresearchNewTabButton::GetBorderPath(const gfx::Point& origin,
                                        float scale,
                                        bool extend_to_top) const {
  // Overriden to use Presearch's non-circular shape
...
void PresearchNewTabButton::PaintIcon(gfx::Canvas* canvas) {
...
gfx::Insets PresearchNewTabButton::GetInsets() const {
