...
#include "presearch/browser/ui/content_settings/presearch_autoplay_content_setting_bubble_model.h"
...
PresearchAutoplayContentSettingBubbleModel::PresearchAutoplayContentSettingBubbleModel(
    Delegate* delegate,
...
PresearchAutoplayContentSettingBubbleModel::
~PresearchAutoplayContentSettingBubbleModel() {}

void PresearchAutoplayContentSettingBubbleModel::CommitChanges() {
  if (settings_changed()) {
...
bool PresearchAutoplayContentSettingBubbleModel::settings_changed() const {
  return selected_item() != bubble_content().radio_group.default_item;
}

void PresearchAutoplayContentSettingBubbleModel::SetTitle() {
  set_title(l10n_util::GetStringUTF16(IDS_BLOCKED_AUTOPLAY_TITLE));
}

void PresearchAutoplayContentSettingBubbleModel::SetRadioGroup() {
...
void PresearchAutoplayContentSettingBubbleModel::SetNarrowestContentSetting(
    ContentSetting setting) {
  if (!GetProfile())
    return;
