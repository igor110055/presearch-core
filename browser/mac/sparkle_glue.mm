...
#import "presearch/browser/mac/su_updater.h"
...
- (void)registerWithSparkle {
  // This can be called by PresearchBrowserMainPartsMac::PreMainMessageLoopStart()
  // again when browser is relaunched.
  if (registered_)
    return;

  DCHECK(presearch::UpdateEnabled());
  DCHECK(su_updater_);
  ...
  constexpr NSTimeInterval kPresearchUpdateCheckIntervalInSec = 3 * 60 * 60;
  [su_updater_ setUpdateCheckInterval:kPresearchUpdateCheckIntervalInSec];
...
- (void)updater:(id)updater didFinishLoadingAppcast:(id)appcast {
  VLOG(0) << "presearch update: did finish loading appcast";

  [self updateStatus:kAutoupdateChecking version:nil error:nil];
}

- (void)updater:(id)updater didFindValidUpdate:(id)item {
  VLOG(0) << "presearch update: did find valid update with " +
...
- (void)updaterDidNotFindUpdate:(id)updater {
  VLOG(0) << "presearch update: did not find update";
...
- (void)updater:(id)updater
  ...
  VLOG(0) << "presearch update: willDownloadUpdate with " +
             GetDescriptionFromAppcastItem(item);
...
- (void)updater:(id)updater
    failedToDownloadUpdate:(id)item
                     error:(NSError *)error {
  VLOG(0) << "presearch update: failed to download update with " +
             GetDescriptionFromAppcastItem(item) +
...
- (void)userDidCancelDownload:(id)updater {
  VLOG(0) << "presearch update: user did cancel download";
  [self updateStatus:kAutoupdateInstallFailed
               version:nil
                 error:nil];
}
...
- (void)updater:(id)updater willInstallUpdate:(id)item {
  VLOG(0) << "presearch update: will install update with " +
             GetDescriptionFromAppcastItem(item);
  [self updateStatus:kAutoupdateInstalling
             version:nil
               error:nil];
...
- (void)updater:(id)updater
  ...
  VLOG(0) << "presearch update: will install update on quit with " +
             GetDescriptionFromAppcastItem(item);
...
- (void)updater:(id)updater didAbortWithError:(NSError *)error {
  VLOG(0) << "presearch update: did abort with error: " +
