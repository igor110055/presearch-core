...
#include "presearch/browser/browsing_data/presearch_browsing_data_remover_delegate.h"
...
#include "presearch/common/extensions/api/presearch_today.h"
...
PresearchBrowsingDataRemoverDelegate::PresearchBrowsingDataRemoverDelegate(
    content::BrowserContext* browser_context)
    : ChromeBrowsingDataRemoverDelegate(browser_context),
      profile_(Profile::FromBrowserContext(browser_context)) {}

PresearchBrowsingDataRemoverDelegate::~PresearchBrowsingDataRemoverDelegate() = default;

void PresearchBrowsingDataRemoverDelegate::RemoveEmbedderData(
  ...
  if (remove_mask & chrome_browsing_data_remover::DATA_TYPE_HISTORY) {
    ...
    if (event_router) {
      std::unique_ptr<base::ListValue> args(
          extensions::api::presearch_today::OnClearHistory::Create().release());
      std::unique_ptr<extensions::Event> event(new extensions::Event(
          extensions::events::PRESEARCH_START,
          extensions::api::presearch_today::OnClearHistory::kEventName,
          std::move(args)));
...
void PresearchBrowsingDataRemoverDelegate::ClearShieldsSettings(
  ...
  auto* provider =
      static_cast<content_settings::PresearchPrefProvider*>(map->GetPrefProvider());
...
#if BUILDFLAG(IPFS_ENABLED)
void PresearchBrowsingDataRemoverDelegate::WaitForIPFSRepoGC(
...

  {
    ...
    // Because we set maximum IPFS storage size as 1GB in Presearch, ipfs repo gc
...
// data.
void PresearchBrowsingDataRemoverDelegate::ClearIPFSCache() {
  auto* service =
  ...
  base::ThreadPool::PostTaskAndReply(
      ...
      base::BindOnce(&PresearchBrowsingDataRemoverDelegate::WaitForIPFSRepoGC,
