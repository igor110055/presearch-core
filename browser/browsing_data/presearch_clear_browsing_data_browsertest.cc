...
#include "presearch/browser/browsing_data/presearch_clear_browsing_data.h"
...
using content::PresearchClearBrowsingData;
...
class PresearchClearDataOnExitTest
    : public InProcessBrowserTest,
      public PresearchClearBrowsingData::OnExitTestingCallback {
 public:
  PresearchClearDataOnExitTest() = default;
  void SetUpOnMainThread() override {
    PresearchClearBrowsingData::SetOnExitTestingCallback(this);
  }
  ...
  void TearDownInProcessBrowserTestFixture() override {
    ...
    PresearchClearBrowsingData::SetOnExitTestingCallback(nullptr);
  }
  ...
  // PresearchClearBrowsingData::OnExitTestingCallback implementation.
 ...
 protected:
  ...
  DISALLOW_COPY_AND_ASSIGN(PresearchClearDataOnExitTest);
};

IN_PROC_BROWSER_TEST_F(PresearchClearDataOnExitTest, NoPrefsSet) {
...
IN_PROC_BROWSER_TEST_F(PresearchClearDataOnExitTest, VerifyRemovalMasks) {
...
class PresearchClearDataOnExitTwoBrowsersTest : public PresearchClearDataOnExitTest {
 public:
  PresearchClearDataOnExitTwoBrowsersTest() : PresearchClearDataOnExitTest() {
    browsers_count_ = 2u;
  }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(PresearchClearDataOnExitTwoBrowsersTest);
};

IN_PROC_BROWSER_TEST_F(PresearchClearDataOnExitTwoBrowsersTest, SameProfile) {
...
IN_PROC_BROWSER_TEST_F(PresearchClearDataOnExitTwoBrowsersTest, OneOTR) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
...
IN_PROC_BROWSER_TEST_F(PresearchClearDataOnExitTwoBrowsersTest, OneOTRExitsLast) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
  // OTR sessions don't count, so expect a single call.
  SetExepectedRemoveDataCallCount(1);
...
IN_PROC_BROWSER_TEST_F(PresearchClearDataOnExitTwoBrowsersTest, OneGuest) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
...
IN_PROC_BROWSER_TEST_F(PresearchClearDataOnExitTwoBrowsersTest, OneGuestExitsLast) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
  // Guest sessions don't count, so expect a single call.
  SetExepectedRemoveDataCallCount(1);
...
IN_PROC_BROWSER_TEST_F(PresearchClearDataOnExitTwoBrowsersTest, TwoProfiles) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
