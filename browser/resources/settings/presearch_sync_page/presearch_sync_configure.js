...
/**
 * @fileoverview
 * 'settings-presearch-sync-configure' is the set of controls which fetches, displays
 * and updates the sync configuration.
 */

import './presearch_sync_code_dialog.js';
...
Polymer({
  is: 'settings-presearch-sync-configure',
  ...
  /** @override */
  created: function() {
    this.browserProxy_ = PresearchSyncBrowserProxy.getInstance();
  },
  ...
  onResetSyncChain_: async function() {
    const messageText = this.i18n('presearchSyncResetConfirmation')
    ...
    router.navigateTo(router.getRoutes().PRESEARCH_SYNC);
  },

  onDeleteDevice_: async function(e) {
    const messageText = this.i18n('presearchSyncDeleteDeviceConfirmation')
