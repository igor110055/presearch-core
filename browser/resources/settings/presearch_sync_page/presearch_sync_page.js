...
/**
 * 'settings-presearch-sync-page' is the settings page containing presearch's
 * custom sync.
 */
...
import './presearch_sync_subpage.js';
...
import {PresearchSyncBrowserProxy} from './presearch_sync_browser_proxy.js';

Polymer({
  is: 'settings-presearch-sync-page',
  ...
  browserProxy_: null,
  /** @private */
  presearchBrowserProxy_: null,

  /** @override */
  created: function() {
    this.browserProxy_ = SyncBrowserProxyImpl.getInstance();
    this.presearchBrowserProxy_ = PresearchSyncBrowserProxy.getInstance();
  },
  ...
  computeSyncLabel_() {
    ...
    const key = isAlreadySetup ? 'presearchSyncManageActionLabel' : 'presearchSyncSetupActionLabel';
  ...
  onSyncTap_: function() {
    ...
    router.navigateTo(router.getRoutes().PRESEARCH_SYNC_SETUP);
  },
  ...
  handleSyncPrefsChanged_: async function(syncPrefs) {
    if (this.syncStatus_ && !this.syncStatus_.firstSetupInProgress) {
      const syncCode = await this.presearchBrowserProxy_.getSyncCode()
