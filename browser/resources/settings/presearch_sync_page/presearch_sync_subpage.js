...
import './presearch_sync_configure.js';
import './presearch_sync_setup.js';
...
*/
Polymer({
  is: 'settings-presearch-sync-subpage',
  ...
  */
  ...
  attached: function() {
    ...
    if (router.getCurrentRoute() == router.getRoutes().PRESEARCH_SYNC_SETUP) {
  ...
  detached: function() {
    const router = Router.getInstance();
    if (router.getRoutes().PRESEARCH_SYNC_SETUP.contains(router.getCurrentRoute())) {
  ...
  */
  ...
  onNavigateToPage_: function() {
    const router = Router.getInstance();
    assert(router.getCurrentRoute() == router.getRoutes().PRESEARCH_SYNC_SETUP);
    if (this.beforeunloadCallback_) {
      return;
    }
