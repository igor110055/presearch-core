...
RegisterPolymerComponentBehaviors({
  'settings-import-data-dialog': [{
    registered: function () {
      ...
      if (!oldPrefsChanged) {
        console.error('[Presearch Settings Overrides] cannot find prefsChanged_ on ImportDataDialog')
        return
      }
      this.prefsChanged_ = function () {
        if (typeof this.noImportDataTypeSelected_ !== 'boolean') {
          console.error('[Presearch Settings Overrides] cannot find noImportDataTypeSelected_ on ImportDataDialog')
          return
        }
