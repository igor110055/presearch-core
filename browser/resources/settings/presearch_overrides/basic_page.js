...
import '../presearch_default_extensions_page/presearch_default_extensions_page.m.js'
import '../default_presearch_shields_page/default_presearch_shields_page.m.js'
import '../social_blocking_page/social_blocking_page.m.js'
import '../presearch_ipfs_page/presearch_ipfs_page.m.js'
import '../presearch_wallet_page/presearch_wallet_page.m.js'
import '../presearch_sync_page/presearch_sync_page.js'
import '../presearch_help_tips_page/presearch_help_tips_page.m.js'
import '../presearch_new_tab_page/presearch_new_tab_page.m.js'
...
RegisterPolymerTemplateModifications({
  'settings-basic-page': (templateContent) => {
    ...
    if (!r.BASIC) {
      console.error('[Presearch Settings Overrides] Routes: could not find BASIC page')
    }
    ...
    if (pageVisibility.presearchSync) {
      r.PRESEARCH_SYNC = r.BASIC.createSection('/presearchSync', 'presearchSync')
      r.PRESEARCH_SYNC_SETUP = r.PRESEARCH_SYNC.createChild('/presearchSync/setup');
    }
    if (pageVisibility.presearchIPFS) {
      r.PRESEARCH_IPFS = r.BASIC.createSection('/ipfs', 'ipfs')
    }
    if (pageVisibility.presearchWallet) {
      r.PRESEARCH_WALLET = r.BASIC.createSection('/wallet', 'wallet')
    }

    r.PRESEARCH_HELP_TIPS = r.BASIC.createSection('/presearchHelpTips', 'presearchHelpTips')
    r.PRESEARCH_NEW_TAB = r.BASIC.createSection('/newTab', 'newTab')
    if (r.SITE_SETTINGS) {
      r.SITE_SETTINGS_AUTOPLAY = r.SITE_SETTINGS.createChild('autoplay')
    } else if (!isGuest) {
      console.error('[Presearch Settings Overrides] Routes: could not find SITE_SETTINGS page')
    }
    ...
    if (r.AUTOFILL && r.ADVANCED) {
      ...
      console.error('[Presearch Settings Overrides] Could not move autofill route to advanced route', r)
    }
    ...
    if (r.PRIVACY && r.ADVANCED) {
      ...
      console.error('[Presearch Settings Overrides] Could not move privacy route to advanced route', r)
    }
    ...
    if (!basicPageEl) {
      ...
      sectionGetStarted.content.appendChild(createSectionElement(
        'getStarted',
        'presearchGetStartedTitle',
        'presearch-settings-getting-started',
        {
          prefs: '{{prefs}}',
      ...
      sectionExtensions.content.appendChild(createSectionElement(
        'extensions',
        'presearchDefaultExtensions',
        'settings-presearch-default-extensions-page',
        {
          prefs: '{{prefs}}'
        }
      ))
      ...
      sectionIPFS.setAttribute('if', '[[showPage_(pageVisibility.presearchIPFS)]]')
      sectionIPFS.content.appendChild(createSectionElement(
        'ipfs',
        'presearchIPFS',
        'settings-presearch-ipfs-page',
        {
          prefs: '{{prefs}}'
        }
      ))
      ...
      sectionWallet.setAttribute('if', '[[showPage_(pageVisibility.presearchWallet)]]')
      sectionWallet.content.appendChild(createSectionElement(
        'wallet',
        'presearchWallet',
        'settings-presearch-wallet-page',
        {
          prefs: '{{prefs}}'
        }
      ))
      ...
      sectionSync.setAttribute('if', '[[showPage_(pageVisibility.presearchSync)]]')
      sectionSync.content.appendChild(createSectionElement(
        'presearchSync',
        'presearchSync',
        'settings-presearch-sync-page',
        {}
      ))
      ...
      sectionShields.content.appendChild(createSectionElement(
        'shields',
        'presearchShieldsTitle',
        'settings-default-presearch-shields-page',
        {
          prefs: '{{prefs}}'
        }
      ))
      ...
      sectionHelpTips.setAttribute('if', '[[showPage_(pageVisibility.presearchHelpTips)]]')
      sectionHelpTips.content.appendChild(createSectionElement(
        'presearchHelpTips',
        'presearchHelpTips',
        'settings-presearch-help-tips-page',
        {
          prefs: '{{prefs}}'
        }
      ))
      ...
      sectionNewTab.content.appendChild(createSectionElement(
        'newTab',
        'presearchNewTab',
        'settings-presearch-new-tab-page',
        {
          prefs: '{{prefs}}'
        }
      ))
      ...
      if (!advancedToggleTemplate) {
        console.error('[Presearch Settings Overrides] Could not find advanced toggle template')
      }
      const advancedToggleText = advancedToggleTemplate.content.querySelector('cr-button span')
      if (!advancedToggleText) {
        console.error('[Presearch Settings Overrides] Could not find advanced toggle text')
      }
      advancedToggleText.innerText = loadTimeData.getString('presearchAdditionalSettingsTitle')
