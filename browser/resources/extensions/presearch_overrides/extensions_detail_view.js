...
import {RegisterPolymerTemplateModifications, RegisterPolymerComponentBehaviors} from 'chrome://presearch-resources/polymer_overriding.js'
...
RegisterPolymerTemplateModifications({
  'extensions-detail-view': (templateContent) => {
    ...
    if (!optionsTemplate) {
      console.error('[Presearch Extensions Overrides] Could not find optionsTemplate')
      return
    }
    ...
    if (!incognitoTemplate) {
      console.error('[Presearch Extensions Overrides] Could not find incognitoTemplate')
      return
    }
    let incognitoWarningDiv = incognitoTemplate.content.querySelector('.section-content')
    if (!incognitoWarningDiv) {
      console.error('[Presearch Extensions Overrides] Could not find incognitoWarningDiv')
      return
    }
