...
#include "presearch/components/presearch_shields/browser/domain_block_navigation_throttle.h"
...
#include "presearch/components/cosmetic_filters/browser/cosmetic_filters_resources.h"
#include "presearch/components/cosmetic_filters/common/cosmetic_filters.mojom.h"
...
using presearch_shields::GetPresearchShieldsEnabled;
using presearch_shields::GetFingerprintingControlType;
...
#include "presearch/components/presearch_webtorrent/browser/content_browser_client_helper.h"
#endif

#if BUILDFLAG(IPFS_ENABLED)
#include "presearch/browser/ipfs/content_browser_client_helper.h"
#include "presearch/browser/ipfs/ipfs_service_factory.h"
#include "presearch/components/ipfs/ipfs_constants.h"
#include "presearch/components/ipfs/ipfs_navigation_throttle.h"
#endif

#if BUILDFLAG(DECENTRALIZED_DNS_ENABLED)
#include "presearch/components/decentralized_dns/decentralized_dns_navigation_throttle.h"
#endif

#if BUILDFLAG(PRESEARCH_REWARDS_ENABLED)
#include "presearch/components/presearch_rewards/browser/rewards_protocol_handler.h"
#endif

#if BUILDFLAG(ENABLE_TOR)
#include "presearch/browser/tor/onion_location_navigation_throttle_delegate.h"
#include "presearch/browser/tor/tor_profile_service_factory.h"
#include "presearch/components/tor/onion_location_navigation_throttle.h"
#include "presearch/components/tor/tor_navigation_throttle.h"
#endif

#if BUILDFLAG(ENABLE_SPEEDREADER)
#include "presearch/browser/speedreader/speedreader_tab_helper.h"
#include "presearch/components/speedreader/speedreader_throttle.h"
#include "third_party/blink/public/mojom/loader/resource_load_info.mojom-shared.h"
#endif

#if BUILDFLAG(BINANCE_ENABLED)
#include "presearch/browser/binance/binance_protocol_handler.h"
#endif

#if BUILDFLAG(GEMINI_ENABLED)
#include "presearch/browser/gemini/gemini_protocol_handler.h"
#endif
...
#include "presearch/browser/new_tab/new_tab_shows_navigation_throttle.h"
#endif
...
bool HandleURLReverseOverrideRewrite(GURL* url,
  ...
  if (PresearchContentBrowserClient::HandleURLOverrideRewrite(url, browser_context))
    return true;
...
void BindCosmeticFiltersResources(
  ...
  mojo::MakeSelfOwnedReceiver(
      std::make_unique<cosmetic_filters::CosmeticFiltersResources>(
          settings_map, g_presearch_browser_process->ad_block_service()),
      std::move(receiver));
}

#if BUILDFLAG(PRESEARCH_WALLET_ENABLED)
void MaybeBindPresearchWalletProvider(
    content::RenderFrameHost* const frame_host,
    mojo::PendingReceiver<presearch_wallet::mojom::PresearchWalletProvider> receiver) {
  auto* context = frame_host->GetBrowserContext();
  if (!presearch_wallet::IsAllowedForContext(context))
    return;

  PresearchWalletService* service =
      PresearchWalletServiceFactory::GetInstance()->GetForContext(context);

  mojo::MakeSelfOwnedReceiver(
      std::make_unique<presearch_wallet::PresearchWalletProviderImpl>(
          service->AsWeakPtr()),
      std::move(receiver));
}
#endif

void BindPresearchSearchHost(
    int process_id,
    mojo::PendingReceiver<presearch_search::mojom::PresearchSearchFallback> receiver) {
  ...
  mojo::MakeSelfOwnedReceiver(
      std::make_unique<presearch_search::PresearchSearchHost>(
...
PresearchContentBrowserClient::PresearchContentBrowserClient()
...
PresearchContentBrowserClient::CreateBrowserMainParts(
    const content::MainFunctionParams& parameters) {
  std::unique_ptr<content::BrowserMainParts> main_parts =
      ChromeContentBrowserClient::CreateBrowserMainParts(parameters);
  ChromeBrowserMainParts* chrome_main_parts =
      static_cast<ChromeBrowserMainParts*>(main_parts.get());
  chrome_main_parts->AddParts(std::make_unique<PresearchBrowserMainExtraParts>());
  return main_parts;
}

void PresearchContentBrowserClient::BrowserURLHandlerCreated(
...
void PresearchContentBrowserClient::RenderProcessWillLaunch(
    content::RenderProcessHost* host) {
  Profile* profile = Profile::FromBrowserContext(host->GetBrowserContext());
  PresearchRendererUpdaterFactory::GetForProfile(profile)
...
PresearchContentBrowserClient::AllowWebBluetooth(
    content::BrowserContext* browser_context,
...
void PresearchContentBrowserClient::ExposeInterfacesToRenderer(
  ...
  registry->AddInterface(
      base::BindRepeating(&BindPresearchSearchHost, render_process_host->GetID()),
      content::GetUIThreadTaskRunner({}));
}

void PresearchContentBrowserClient::RegisterBrowserInterfaceBindersForFrame(
...
#if BUILDFLAG(PRESEARCH_WALLET_ENABLED)
  if (presearch_wallet::IsNativeWalletEnabled()) {
    map->Add<presearch_wallet::mojom::PresearchWalletProvider>(
        base::BindRepeating(&MaybeBindPresearchWalletProvider));
  }
#endif
}

bool PresearchContentBrowserClient::HandleExternalProtocol(
    const GURL& url,
...
#if BUILDFLAG(PRESEARCH_REWARDS_ENABLED)
  if (presearch_rewards::IsRewardsProtocol(url)) {
    presearch_rewards::HandleRewardsProtocol(url, std::move(web_contents_getter),
                                         page_transition, has_user_gesture);
    return true;
  }
#endif
...
void PresearchContentBrowserClient::AppendExtraCommandLineSwitches(
  ...
  if (process_type == switches::kRendererProcess) {
    ...
    command_line->AppendSwitchASCII("presearch_session_token",
                                    base::NumberToString(session_token));
  }
}

std::vector<std::unique_ptr<blink::URLLoaderThrottle>>
PresearchContentBrowserClient::CreateURLLoaderThrottles(
  ...
  if (tab_helper && tab_helper->IsActiveForMainFrame() &&
    ...
    result.push_back(std::make_unique<speedreader::SpeedReaderThrottle>(
        g_presearch_browser_process->speedreader_rewriter_service(),
...
bool PresearchContentBrowserClient::WillCreateURLLoaderFactory(
    content::BrowserContext* browser_context,
  ...
  use_proxy = PresearchProxyingURLLoaderFactory::MaybeProxyRequest(
...
bool PresearchContentBrowserClient::WillInterceptWebSocket(
    content::RenderFrameHost* frame) {
  return (frame != nullptr);
}

void PresearchContentBrowserClient::CreateWebSocket(
    content::RenderFrameHost* frame,
    ...
    mojo::PendingRemote<network::mojom::WebSocketHandshakeClient>
        handshake_client) {
  auto* proxy = PresearchProxyingWebSocket::ProxyWebSocket(
...
void PresearchContentBrowserClient::MaybeHideReferrer(
    content::BrowserContext* browser_context,
  ...
  const bool allow_referrers = presearch_shields::AllowReferrers(
      HostContentSettingsMapFactory::GetForProfile(profile), document_url);
  const bool shields_up = presearch_shields::GetPresearchShieldsEnabled(
      HostContentSettingsMapFactory::GetForProfile(profile), document_url);

  content::Referrer new_referrer;
  if (presearch_shields::MaybeChangeReferrer(allow_referrers, shields_up,
...
GURL PresearchContentBrowserClient::GetEffectiveURL(
    content::BrowserContext* browser_context,
    const GURL& url) {
  Profile* profile = Profile::FromBrowserContext(browser_context);
  if (!profile)
...
// [static]
bool PresearchContentBrowserClient::HandleURLOverrideRewrite(
    GURL* url,
    content::BrowserContext* browser_context) {
  if (url->host() == chrome::kChromeUISyncHost) {
    GURL::Replacements replacements;
    replacements.SetHostStr(chrome::kChromeUISettingsHost);
    replacements.SetPathStr(kPresearchSyncPath);
  ...
  auto* service = PresearchWalletServiceFactory::GetForContext(browser_context);
...
PresearchContentBrowserClient::CreateThrottlesForNavigation(
...
#if BUILDFLAG(ENABLE_PRESEARCH_WEBTORRENT)
  throttles.push_back(
      std::make_unique<extensions::PresearchWebTorrentNavigationThrottle>(handle));
#endif
  ...
  if (std::unique_ptr<
          ...
          presearch_shields::DomainBlockNavigationThrottle::MaybeCreateThrottleFor(
              handle, g_presearch_browser_process->ad_block_service(),
