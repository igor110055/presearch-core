...
PresearchComponentInstallerPolicy::PresearchComponentInstallerPolicy(
...
bool PresearchComponentInstallerPolicy::VerifyInstallation(
    const base::DictionaryValue& manifest,
...
bool PresearchComponentInstallerPolicy::
SupportsGroupPolicyEnabledComponentUpdates() const {
  return false;
}

bool PresearchComponentInstallerPolicy::RequiresNetworkEncryption() const {
  return false;
}

update_client::CrxInstaller::Result
PresearchComponentInstallerPolicy::OnCustomInstall(
  const base::DictionaryValue& manifest,
  const base::FilePath& install_dir) {
  return Result(InstallError::NONE);
}

void PresearchComponentInstallerPolicy::OnCustomUninstall() {
}

void PresearchComponentInstallerPolicy::ComponentReady(
...
base::FilePath PresearchComponentInstallerPolicy::GetRelativeInstallDir() const {
...
void PresearchComponentInstallerPolicy::GetHash(std::vector<uint8_t>* hash) const {
...
std::string PresearchComponentInstallerPolicy::GetName() const {
  return name_;
}

update_client::InstallerAttributes
PresearchComponentInstallerPolicy::GetInstallerAttributes() const {
...
void RegisterComponent(
  ...
  auto installer = base::MakeRefCounted<component_updater::ComponentInstaller>(
      std::make_unique<PresearchComponentInstallerPolicy>(
