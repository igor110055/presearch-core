...
using presearch_shields::ResetPresearchShieldsEnabled;
using presearch_shields::SetPresearchShieldsEnabled;
using presearch_shields::SetCosmeticFilteringControlType;
...
class DomainBlockTest : public DomainBlockTestBase {
 public:
  DomainBlockTest() { feature_list_.InitAndEnableFeature(kPresearchDomainBlock); }
...
IN_PROC_BROWSER_TEST_F(DomainBlockTest, NoInterstitialUnlessAggressive) {
  ...
  BlockDomainByURL(url);
  SetCosmeticFilteringControlType(content_settings(), ControlType::BLOCK, url);
  SetPresearchShieldsEnabled(content_settings(), false /* enable */, url);
  NavigateTo(url);
  ASSERT_FALSE(IsShowingInterstitial());
}
