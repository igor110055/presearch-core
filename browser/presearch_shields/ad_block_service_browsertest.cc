...
#include "presearch/browser/presearch_shields/ad_block_service_browsertest.h"
...
void AdBlockServiceTest::PreRunTestOnMainThread() {
  ...
  ASSERT_TRUE(g_presearch_browser_process->ad_block_service()->IsInitialized());
}
...
void AdBlockServiceTest::UpdateAdBlockInstanceWithRules(
  ...
  g_presearch_browser_process->ad_block_service()->ResetForTest(rules, resources);
}

void AdBlockServiceTest::AssertTagExists(const std::string& tag,
                                         bool expected_exists) const {
  bool exists_default =
      g_presearch_browser_process->ad_block_service()->TagExists(tag);
...
bool AdBlockServiceTest::InstallDefaultAdBlockExtension(
    ...
    int expected_change) {
  presearch_shields::AdBlockService::SetComponentIdAndBase64PublicKeyForTest(
  ...
  g_presearch_browser_process->ad_block_service()->OnComponentReady(
...
bool AdBlockServiceTest::InstallRegionalAdBlockExtension(
  ...
  presearch_shields::AdBlockRegionalService::
      SetComponentIdAndBase64PublicKeyForTest(
  ...
  EXPECT_EQ(g_presearch_browser_process->ad_block_regional_service_manager()
...
bool AdBlockServiceTest::InstallTrackingProtectionExtension() {
  ...
  g_presearch_browser_process->tracking_protection_service()->OnComponentReady(
...
bool AdBlockServiceTest::StartAdBlockRegionalServices() {
  g_presearch_browser_process->ad_block_regional_service_manager()->Start();
  if (!g_presearch_browser_process->ad_block_regional_service_manager()
           ->IsInitialized())
    return false;
  return true;
}
...
void AdBlockServiceTest::WaitForAdBlockServiceThreads() {
  scoped_refptr<base::ThreadTestHelper> tr_helper(new base::ThreadTestHelper(
      g_presearch_browser_process->local_data_files_service()->GetTaskRunner()));
...
void AdBlockServiceTest::WaitForPresearchExtensionShieldsDataReady() {
  ...
  ExtensionTestMessageListener extension_listener(
      "presearch-extension-shields-data-ready", false);
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest,
  ...
  ASSERT_TRUE(g_presearch_browser_process->ad_block_custom_filters_service()
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SocialButttonAdBlockTagTest) {
  UpdateAdBlockInstanceWithRules(
      base::StringPrintf("||example.com^$tag=%s",
                         presearch_shields::kFacebookEmbeds)
          .c_str());
  EXPECT_EQ(browser()->profile()->GetPrefs()->GetUint64(kAdsBlocked), 0ULL);
  GURL tab_url = embedded_test_server()->GetURL("b.com", kAdBlockTestPage);
  g_presearch_browser_process->ad_block_service()->EnableTag(
      presearch_shields::kFacebookEmbeds, true);
  WaitForAdBlockServiceThreads();
  GURL resource_url =
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ResetPreservesTags) {
  ...
  WaitForAdBlockServiceThreads();
  UpdateAdBlockInstanceWithRules("");
  AssertTagExists(presearch_shields::kFacebookEmbeds, true);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, TagPrefsControlTags) {
  ...
  AssertTagExists(presearch_shields::kFacebookEmbeds, true);
  AssertTagExists(presearch_shields::kTwitterEmbeds, true);
  AssertTagExists(presearch_shields::kLinkedInEmbeds, false);
  ...
  AssertTagExists(presearch_shields::kFacebookEmbeds, true);
  AssertTagExists(presearch_shields::kTwitterEmbeds, true);
  AssertTagExists(presearch_shields::kLinkedInEmbeds, true);

  prefs->SetBoolean(kFBEmbedControlType, false);
  WaitForAdBlockServiceThreads();
  AssertTagExists(presearch_shields::kFacebookEmbeds, false);
  AssertTagExists(presearch_shields::kTwitterEmbeds, true);
  AssertTagExists(presearch_shields::kLinkedInEmbeds, true);
  ...
  WaitForAdBlockServiceThreads();
  AssertTagExists(presearch_shields::kFacebookEmbeds, false);
  AssertTagExists(presearch_shields::kTwitterEmbeds, false);
  AssertTagExists(presearch_shields::kLinkedInEmbeds, true);
...
class CosmeticFilteringFlagDisabledTest : public AdBlockServiceTest {
 public:
  CosmeticFilteringFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kPresearchAdblockCosmeticFiltering);
  }
...
IN_PROC_BROWSER_TEST_F(CosmeticFilteringFlagDisabledTest,
  ...
  UpdateAdBlockInstanceWithRules(
      "b.com###ad-banner\n"
      "##.ad");
  WaitForPresearchExtensionShieldsDataReady();
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringDisabled) {
  presearch_shields::SetCosmeticFilteringControlType(
      content_settings(), presearch_shields::ControlType::ALLOW, GURL());
  UpdateAdBlockInstanceWithRules(
      "b.com###ad-banner\n"
      "##.ad");
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringHide1pContent) {
  presearch_shields::SetCosmeticFilteringControlType(
      content_settings(), presearch_shields::ControlType::BLOCK, GURL());
  UpdateAdBlockInstanceWithRules("b.com##.fpsponsored\n");
