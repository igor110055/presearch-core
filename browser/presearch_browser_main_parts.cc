...
#include "presearch/common/presearch_constants.h"
...
#if BUILDFLAG(ENABLE_PRESEARCH_SYNC) && !defined(OS_ANDROID)
#include "presearch/browser/infobars/sync_v2_migrate_infobar_delegate.h"
...
void PresearchBrowserMainParts::PostBrowserStart() {
  ...
  for (Profile* profile : profile_manager->GetLoadedProfiles()) {
    const base::FilePath tor_legacy_session_path =
        profile->GetPath()
            .Append(presearch::kSessionProfileDir)
  ...
  if (browser) {
    ...
    if (active_web_contents) {
      ...
      if (infobar_service) {
        PresearchConfirmP3AInfoBarDelegate::Create(
        ...
        SyncV2MigrateInfoBarDelegate::Create(infobar_service, is_v2_user,
                                             profile(), browser);
#endif  // BUILDFLAG(ENABLE_PRESEARCH_SYNC)
      }
    }
  }
...
void PresearchBrowserMainParts::PreShutdown() {
  content::PresearchClearBrowsingData::ClearOnExit();
}

void PresearchBrowserMainParts::PreProfileInit() {
  ChromeBrowserMainParts::PreProfileInit();
#if !defined(OS_ANDROID)
  auto* command_line = base::CommandLine::ForCurrentProcess();
  if (!base::FeatureList::IsEnabled(presearch_sync::features::kPresearchSync)) {
...
void PresearchBrowserMainParts::PostProfileInit() {
