...
bool IsAlreadyRegistered(ComponentUpdateService* cus) {
  ...
  return std::find(component_ids.begin(), component_ids.end(),
                   PresearchDrmTabHelper::kWidevineComponentId) !=
         component_ids.end();
}
#if !defined(OS_LINUX)
...
// static
const char PresearchDrmTabHelper::kWidevineComponentId[] =
    "oimompecagnajdejgnnjijobebaeigek";

PresearchDrmTabHelper::PresearchDrmTabHelper(content::WebContents* contents)
...
PresearchDrmTabHelper::~PresearchDrmTabHelper() {}

bool PresearchDrmTabHelper::ShouldShowWidevineOptIn() const {
...
void PresearchDrmTabHelper::DidStartNavigation(
...
void PresearchDrmTabHelper::OnWidevineKeySystemAccessRequest() {
...
void PresearchDrmTabHelper::OnEvent(Events event, const std::string& id) {
...
WEB_CONTENTS_USER_DATA_KEY_IMPL(PresearchDrmTabHelper)
