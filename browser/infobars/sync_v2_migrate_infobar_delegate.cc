...
void SyncV2MigrateInfoBarDelegate::Create(
  ...
  const bool is_flag_enabled =
      base::FeatureList::IsEnabled(presearch_sync::features::kPresearchSync);
  if (!is_flag_enabled) {
    return;
  }
  presearch_sync::Prefs presearch_sync_prefs(profile->GetPrefs());
  const bool was_v1_user = presearch_sync_prefs.IsSyncV1Enabled();
  if (!was_v1_user) {
    // Not v1 user
    return;
  }
  const bool has_dismissed = presearch_sync_prefs.IsSyncMigrateNoticeDismissed();
  if (has_dismissed) {
    return;
  }
  if (is_v2_user) {
    ...
    // rather than here.
    presearch_sync_prefs.SetDismissSyncMigrateNotice(true);
    return;
  }
  // Show infobar
...
base::string16 SyncV2MigrateInfoBarDelegate::GetMessageText() const {
  return l10n_util::GetStringUTF16(IDS_PRESEARCH_SYNC_V2_MIGRATE_INFOBAR_MESSAGE);
}
...
base::string16 SyncV2MigrateInfoBarDelegate::GetButtonLabel(
  ...
  return l10n_util::GetStringUTF16(IDS_PRESEARCH_SYNC_V2_MIGRATE_INFOBAR_COMMAND);
}
