...
base::string16 CryptoWalletsInfoBarDelegate::GetMessageText() const {
  if (subtype_ == InfobarSubType::LOAD_CRYPTO_WALLETS) {
    return l10n_util::GetStringUTF16(IDS_PRESEARCH_CRYPTO_WALLETS_LAZY_LOAD_TEXT);
  }
  return l10n_util::GetStringUTF16(IDS_PRESEARCH_CRYPTO_WALLETS_INFOBAR_TEXT);
}
...
base::string16 CryptoWalletsInfoBarDelegate::GetButtonLabel(
  ...
  if (subtype_ == InfobarSubType::LOAD_CRYPTO_WALLETS) {
    if (button == BUTTON_CANCEL) {
      return l10n_util::GetStringUTF16(IDS_PRESEARCH_CRYPTO_WALLETS_SETTINGS);
    }
    return l10n_util::GetStringUTF16(
        IDS_PRESEARCH_CRYPTO_WALLETS_START_AND_RELOAD);
  }

  if (button == BUTTON_CANCEL) {
    return l10n_util::GetStringUTF16(IDS_PRESEARCH_CRYPTO_WALLETS_DONT_ASK);
  }

  return l10n_util::GetStringUTF16(
      IDS_PRESEARCH_CRYPTO_WALLETS_SETUP_CRYPTO_WALLETS);
}
  ...
  if (infobar() && infobar()->owner()) {
    ...
    if (web_contents) {
      ...
      user_prefs::UserPrefs::Get(browser_context)->
          SetInteger(kPresearchWalletWeb3Provider,
              static_cast<int>(PresearchWalletWeb3ProviderTypes::CRYPTO_WALLETS));
      Browser* browser = chrome::FindBrowserWithWebContents(web_contents);
      presearch::ShowPresearchWallet(browser);
    }
  }
  return true;
}
...
bool CryptoWalletsInfoBarDelegate::Cancel() {
  ...
  if (web_contents) {
    ...
    Browser* browser = chrome::FindBrowserWithWebContents(web_contents);
    presearch::ShowWalletSettings(browser);
  }
  return true;
}
