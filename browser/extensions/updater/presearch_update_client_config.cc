...
std::string PresearchUpdateClientConfig::GetChannel() const {
  return std::string("stable");
}

scoped_refptr<update_client::NetworkFetcherFactory>
PresearchUpdateClientConfig::GetNetworkFetcherFactory() {
...
bool PresearchUpdateClientConfig::EnabledCupSigning() const {
  return false;
}

PresearchUpdateClientConfig::~PresearchUpdateClientConfig() = default;

// static
scoped_refptr<ChromeUpdateClientConfig> PresearchUpdateClientConfig::Create(
    content::BrowserContext* context,
    base::Optional<GURL> update_url_override) {
  FactoryCallback& factory = GetFactoryCallback();
  return factory.is_null() ? base::MakeRefCounted<PresearchUpdateClientConfig>(
