...
PresearchExtensionManagement::PresearchExtensionManagement(Profile* profile)
  ...
  providers_.push_back(std::make_unique<PresearchExtensionProvider>());
  local_state_pref_change_registrar_.Init(g_browser_process->local_state());
#if BUILDFLAG(ENABLE_TOR)
  local_state_pref_change_registrar_.Add(
      tor::prefs::kTorDisabled,
      base::BindRepeating(&PresearchExtensionManagement::OnTorDisabledChanged,
...
PresearchExtensionManagement::~PresearchExtensionManagement() {
  local_state_pref_change_registrar_.RemoveAll();
}

void PresearchExtensionManagement::OnExtensionLoaded(
...
void PresearchExtensionManagement::OnExtensionUnloaded(
    content::BrowserContext* browser_context,
    const Extension* extension,
...
void PresearchExtensionManagement::OnTorDisabledChanged() {
#if BUILDFLAG(ENABLE_TOR)
  if (TorProfileServiceFactory::IsTorDisabled())
    g_presearch_browser_process->tor_client_updater()->Cleanup();
#endif
}

void PresearchExtensionManagement::Cleanup(content::BrowserContext* context) {
  ...
  if (ipfs::IsIpfsDisabledByPolicy(context))
    g_presearch_browser_process->ipfs_client_updater()->Cleanup();
#endif
}
