...
class MockBrowserClient : public content::ContentBrowserClient {
 public:
  ...
  // Only construct an PresearchWebTorrentNavigationThrottle so that we can test it
  // in isolation.
  std::vector<std::unique_ptr<NavigationThrottle>> CreateThrottlesForNavigation(
      content::NavigationHandle* handle) override {
    std::vector<std::unique_ptr<NavigationThrottle>> throttles;
    throttles.push_back(
        std::make_unique<PresearchWebTorrentNavigationThrottle>(handle));
    return throttles;
  }
};
...
class PresearchWebTorrentNavigationThrottleUnitTest
    : public content::RenderViewHostTestHarness {
 public:
  PresearchWebTorrentNavigationThrottleUnitTest()
  ...
  void AddExtension() {
    ...
    extension_ = ExtensionBuilder()
                     ...
                     .SetID(presearch_webtorrent_extension_id)
                     .Build();
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(PresearchWebTorrentNavigationThrottleUnitTest);
};

// Tests the basic case of loading a URL, it should proceed.
TEST_F(PresearchWebTorrentNavigationThrottleUnitTest, ExternalWebPage) {
  ...
  auto throttle =
      std::make_unique<PresearchWebTorrentNavigationThrottle>(&test_handle);
...
TEST_F(PresearchWebTorrentNavigationThrottleUnitTest,
...
TEST_F(PresearchWebTorrentNavigationThrottleUnitTest, WebTorrentUrlInstalled) {
  AddExtension();
  web_contents_tester()->NavigateAndCommit(GURL("http://example.com"));
  content::RenderFrameHost* host =
      render_frame_host_tester(main_rfh())->AppendChild("child");
...
// disabled.
TEST_F(PresearchWebTorrentNavigationThrottleUnitTest, WebTorrentDisabledByPref) {
