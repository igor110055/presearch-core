...
class PresearchWalletAPIBrowserTest : public InProcessBrowserTest,
    public InfoBarManager::Observer {
 public:
  PresearchWalletAPIBrowserTest() : infobar_observer_(this),
      infobar_added_(false) {
  }

  void WaitForPresearchExtensionAdded() {
    // Presearch extension must be loaded, otherwise dapp detection events
    // could be missed from a race condition.
    ExtensionTestMessageListener extension_listener("presearch-extension-enabled",
        false);
  ...
  ~PresearchWalletAPIBrowserTest() override {
  }
...
IN_PROC_BROWSER_TEST_F(PresearchWalletAPIBrowserTest, DappDetectionTestAccept) {
  ...
  WaitForPresearchExtensionAdded();
  InfoBarService* infobar_service =
      InfoBarService::FromWebContents(active_contents());
  ...
  ASSERT_EQ(provider, PresearchWalletWeb3ProviderTypes::ASK);
...
IN_PROC_BROWSER_TEST_F(PresearchWalletAPIBrowserTest, InfoBarDontAsk) {
  if (presearch_wallet::IsNativeWalletEnabled()) {
    browser()->profile()->GetPrefs()->SetInteger(
        kPresearchWalletWeb3Provider,
        static_cast<int>(PresearchWalletWeb3ProviderTypes::ASK));
  ...
  auto provider_before = static_cast<PresearchWalletWeb3ProviderTypes>(
      browser()->profile()->GetPrefs()->GetInteger(kPresearchWalletWeb3Provider));
  ASSERT_EQ(provider_before, PresearchWalletWeb3ProviderTypes::ASK);
  // Click "Don't ask again"
  CryptoWalletsInfoBarCancel(
      ConfirmInfoBarDelegate::BUTTON_OK |
      ConfirmInfoBarDelegate::BUTTON_CANCEL);
  // Provider type should now be none
  auto provider_after = static_cast<PresearchWalletWeb3ProviderTypes>(
      browser()->profile()->GetPrefs()->GetInteger(kPresearchWalletWeb3Provider));
  ASSERT_EQ(provider_after, PresearchWalletWeb3ProviderTypes::NONE);
  RemoveInfoBarObserver(infobar_service);
}

IN_PROC_BROWSER_TEST_F(PresearchWalletAPIBrowserTest,
    FakeInstallMetaMask) {
  if (presearch_wallet::IsNativeWalletEnabled()) {
    browser()->profile()->GetPrefs()->SetInteger(
        kPresearchWalletWeb3Provider,
  ...
  auto provider = static_cast<PresearchWalletWeb3ProviderTypes>(
      browser()->profile()->GetPrefs()->GetInteger(kPresearchWalletWeb3Provider));
  ASSERT_EQ(provider, PresearchWalletWeb3ProviderTypes::METAMASK);
}
...
IN_PROC_BROWSER_TEST_F(PresearchWalletAPIBrowserTest,
  ...
  auto provider = static_cast<PresearchWalletWeb3ProviderTypes>(
      browser()->profile()->GetPrefs()->GetInteger(kPresearchWalletWeb3Provider));
  if (presearch_wallet::IsNativeWalletEnabled()) {
    ASSERT_EQ(provider, PresearchWalletWeb3ProviderTypes::PRESEARCH_WALLET);
  } else {
    ASSERT_EQ(provider, PresearchWalletWeb3ProviderTypes::CRYPTO_WALLETS);
  }
}
...
IN_PROC_BROWSER_TEST_F(PresearchWalletAPIBrowserTest,
  ...
  browser()->profile()->GetPrefs()->SetInteger(kPresearchWalletWeb3Provider,
