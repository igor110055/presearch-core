...
class PresearchExtensionsAPIClientTests : public ChromeRenderViewHostTestHarness {
 public:
 ...
 protected:
  PresearchExtensionsAPIClientTests() = default;

 private:
  PresearchExtensionsAPIClient client_;

  DISALLOW_COPY_AND_ASSIGN(PresearchExtensionsAPIClientTests);
};

TEST_F(PresearchExtensionsAPIClientTests, IsPresearchProtectedUrlUphold) {
...
TEST_F(PresearchExtensionsAPIClientTests, IsPresearchProtectedUrlBinance) {
  auto create_request_params = [](const std::string& url) {
    const int kRendererProcessId = 2;
    WebRequestInfoInitParams request;
    request.url = GURL(url);
...
TEST_F(PresearchExtensionsAPIClientTests, IsPresearchProtectedUrlGemini) {
  auto create_request_params = [](const std::string& url) {
    const int kRendererProcessId = 2;
    WebRequestInfoInitParams request;
    request.url = GURL(url);
