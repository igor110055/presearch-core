...
PresearchExtensionService::PresearchExtensionService(Profile* profile,
...
PresearchExtensionService::~PresearchExtensionService() {
}

void PresearchExtensionService::AddComponentExtension(const Extension* extension) {
  ExtensionService::AddComponentExtension(extension);

  // Disable Presearch Rewards extension action for Guest and Tor profiles on all
  ...
  // the extension when PresearchActionViewController is queried about the
  // visibility of the action.
  if ((extension->id() == presearch_rewards_extension_id) &&
