...
KeyedService* GreaselionServiceFactory::BuildServiceInstanceFor(
  ...
  // Presearch browser process may be null if we are being created within a unit
  // test.
  if (g_presearch_browser_process)
    download_service = g_presearch_browser_process->greaselion_download_service();
