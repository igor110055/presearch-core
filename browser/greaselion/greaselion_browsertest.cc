...
class GreaselionServiceTest : public BaseLocalDataFilesBrowserTest {
 public:
  ...
  LocalDataFilesObserver* service() override {
    return g_presearch_browser_process->greaselion_download_service();
  }
  ...
  void WaitForService() override {
    ...
    greaselion::GreaselionDownloadService* download_service =
        g_presearch_browser_process->greaselion_download_service();
  ...
  void ClearRules() {
    g_presearch_browser_process->greaselion_download_service()->rules()->clear();
  }
...
// the parent component is updated while Presearch is running.)
