...
class PresearchIpfsClientUpdaterTest : public ExtensionBrowserTest {
 public:
  PresearchIpfsClientUpdaterTest() {}
  ...
  void SetComponentIdAndBase64PublicKeyForTest(
    ...
    ipfs::PresearchIpfsClientUpdater::SetComponentIdAndBase64PublicKeyForTest(
  ...
  bool InstallIpfsClientUpdater(const std::string& extension_dir) {
    ...
    g_presearch_browser_process->ipfs_client_updater()->OnComponentReady(
  ...
  void WaitForIpfsClientUpdaterThread() {
    scoped_refptr<base::ThreadTestHelper> io_helper(new base::ThreadTestHelper(
        g_presearch_browser_process->ipfs_client_updater()->GetTaskRunner()));
...
// installs the client.
IN_PROC_BROWSER_TEST_F(PresearchIpfsClientUpdaterTest, IpfsClientInstalls) {
  ...
  base::FilePath executable_path =
      g_presearch_browser_process->ipfs_client_updater()->GetExecutablePath();
  ASSERT_TRUE(PathExists(executable_path));
}

IN_PROC_BROWSER_TEST_F(PresearchIpfsClientUpdaterTest, IpfsExecutableReady) {
