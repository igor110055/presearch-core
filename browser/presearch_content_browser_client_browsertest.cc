...
class PresearchContentBrowserClientTest : public InProcessBrowserTest {
 public:
  void SetUp() override {
  ...
  void SetUpOnMainThread() override {
    ...
    extension_url_ = GURL(
        ...
        "presearch_webtorrent.html?magnet%3A%3Fxt%3Durn%3Abtih%"
    ...
    torrent_extension_url_ = GURL(
        "chrome-extension://lgjmpdmojkpocjcopdikifhejkkjglho/extension/"
        "presearch_webtorrent.html?https://webtorrent.io/torrents/"
        "sintel.torrent#ix=5");
    torrent_invalid_query_extension_url_ = GURL(
        "chrome-extension://lgjmpdmojkpocjcopdikifhejkkjglho/extension/"
        "presearch_webtorrent.html?chrome://settings");
  }
...
IN_PROC_BROWSER_TEST_F(PresearchContentBrowserClientTest, CanLoadChromeURL) {
...
IN_PROC_BROWSER_TEST_F(PresearchContentBrowserClientTest, CanLoadCustomPresearchPages) {
  std::vector<std::string> pages {
    "adblock",
...
IN_PROC_BROWSER_TEST_F(PresearchContentBrowserClientTest, CanLoadAboutHost) {
  std::vector<std::string> schemes{
      "chrome://",
      "presearch://",
  };
...
IN_PROC_BROWSER_TEST_F(PresearchContentBrowserClientTest, RewriteChromeSync) {
  std::vector<std::string> schemes{
      "presearch://",
      "chrome://",
  };
  ...
  for (const std::string& scheme : schemes) {
    ...
    EXPECT_STREQ(contents->GetController()
                     ...
                     .spec()
                     .c_str(),
                 "chrome://settings/presearchSync");
  }
}

IN_PROC_BROWSER_TEST_F(PresearchContentBrowserClientTest, RewriteMagnetURLURLBar) {
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
...
IN_PROC_BROWSER_TEST_F(PresearchContentBrowserClientTest, RewriteMagnetURLLink) {
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
...
IN_PROC_BROWSER_TEST_F(PresearchContentBrowserClientTest, TypedMagnetURL) {
...
IN_PROC_BROWSER_TEST_F(PresearchContentBrowserClientTest,
...
IN_PROC_BROWSER_TEST_F(PresearchContentBrowserClientTest,
  ...
  ASSERT_FALSE(
      registry->enabled_extensions().Contains(presearch_webtorrent_extension_id));
...
class PresearchContentBrowserClientReferrerTest
    : public PresearchContentBrowserClientTest {
 public:
...
IN_PROC_BROWSER_TEST_F(PresearchContentBrowserClientReferrerTest,
                       DefaultBehaviour) {
  ...
  content_settings()->SetContentSettingCustomScope(
      ...
      ContentSettingsPattern::Wildcard(), ContentSettingsType::PRESEARCH_REFERRERS,
