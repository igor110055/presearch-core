...
IN_PROC_BROWSER_TEST_F(TorProfileManagerTest,
  ...
  EXPECT_EQ(presearch_rewards::RewardsServiceFactory::GetForProfile(tor_profile),
            nullptr);
  EXPECT_EQ(presearch_ads::AdsServiceFactory::GetForProfile(tor_profile), nullptr);
#if BUILDFLAG(IPFS_ENABLED)
...
IN_PROC_BROWSER_TEST_F(TorProfileManagerTest,
  ...
  ContentSetting setting = GetScriptSetting(tor_content_settings, presearch_url);
  ...
  setting = GetScriptSetting(tor_content_settings, presearch_url);
  ...
  SetScriptSetting(tor_content_settings,
                   ContentSettingsPattern::FromURL(presearch_url),
                   CONTENT_SETTING_BLOCK);
  setting = GetScriptSetting(parent_content_settings, presearch_url);
  EXPECT_EQ(setting, CONTENT_SETTING_ALLOW);
  setting = GetScriptSetting(tor_content_settings, presearch_url);
  EXPECT_EQ(setting, CONTENT_SETTING_BLOCK);
}
