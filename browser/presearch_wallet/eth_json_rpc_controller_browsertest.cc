...
// npm run test -- presearch_browser_tests --filter=EthJsonRpcBrowserTest.*
 ...
 public:
  ...
  PresearchWalletService* GetPresearchWalletService() {
    PresearchWalletService* service =
        PresearchWalletServiceFactory::GetInstance()->GetForContext(
            browser()->profile());
    EXPECT_TRUE(service);
    return service;
  }

  presearch_wallet::EthJsonRpcController* GetEthJsonRpcController() {
    return GetPresearchWalletService()->controller();
  }
