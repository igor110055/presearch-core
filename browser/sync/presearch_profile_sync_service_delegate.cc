...
PresearchProfileSyncServiceDelegate::PresearchProfileSyncServiceDelegate(
...
void PresearchProfileSyncServiceDelegate::OnDeviceInfoChange() {
  ...
  if (!found_local_device) {
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE,
        base::BindOnce(
            &PresearchProfileSyncServiceDelegate::OnSelfDeviceInfoDeleted,
            weak_ptr_factory_.GetWeakPtr()));
  }
}

void PresearchProfileSyncServiceDelegate::OnSelfDeviceInfoDeleted() {
  profile_sync_service_->OnSelfDeviceInfoDeleted(base::DoNothing::Once());
}

void PresearchProfileSyncServiceDelegate::SuspendDeviceObserverForOwnReset() {
  if (device_info_observer_.IsObserving(device_info_tracker_)) {
    device_info_observer_.Remove(device_info_tracker_);
  }
}

void PresearchProfileSyncServiceDelegate::ResumeDeviceObserver() {
  if (!device_info_observer_.IsObserving(device_info_tracker_)) {
    device_info_observer_.Add(device_info_tracker_);
  }
}
