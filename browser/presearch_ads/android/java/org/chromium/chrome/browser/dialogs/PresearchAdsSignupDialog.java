...
public class PresearchAdsSignupDialog {
    ...
    public static boolean shouldShowExistingUserDialog(Context context) {
        boolean shouldShow = shouldShowOnboardingDialog() && !PackageUtils.isFirstInstall(context)
                && !PresearchAdsNativeHelper.nativeIsPresearchAdsEnabled(
                        Profile.getLastUsedRegularProfile())
                && PresearchAdsNativeHelper.nativeIsLocaleValid(Profile.getLastUsedRegularProfile())
                && ChromeFeatureList.isEnabled(PresearchFeatureList.PRESEARCH_REWARDS);
    ...
    public static boolean showAdsInBackground() {
        return PresearchRewardsPreferences.getPrefAdsInBackgroundEnabled();
    }
    ...
    private static void enqueueOnboardingNotification(Context context, boolean useCustomNotification) {
        if(!OnboardingPrefManager.getInstance().isOnboardingNotificationShown()) {
            ...
            intent.putExtra(PresearchOnboardingNotification.USE_CUSTOM_NOTIFICATION, useCustomNotification);
            am.set(
    ...
    public static void showNewUserDialog(Context context) {
        AlertDialog alertDialog = new AlertDialog.Builder(context, R.style.PresearchDialogTheme)
        .setView(R.layout.presearch_ads_new_user_dialog_layout)
        .setPositiveButton(R.string.presearch_ads_offer_positive, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                ...
                PresearchRewardsNativeWorker presearchRewardsNativeWorker = PresearchRewardsNativeWorker.getInstance();
    ...
    public static void showExistingUserDialog(Context context) {
        AlertDialog alertDialog = new AlertDialog.Builder(context, R.style.PresearchDialogTheme)
        .setView(R.layout.presearch_ads_existing_user_dialog_layout)
        .setPositiveButton(R.string.presearch_ads_offer_positive, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                // Enable ads
