...
#include "presearch/browser/android/presearch_sync_worker.h"
...
#include "presearch/build/android/jni_headers/PresearchSyncWorker_jni.h"
...
#define DB_FILE_NAME      "presearch_sync_db"

PresearchSyncWorker::PresearchSyncWorker(JNIEnv* env,
                                 const base::android::JavaRef<jobject>& obj)
    : weak_java_presearch_sync_worker_(env, obj) {
  Java_PresearchSyncWorker_setNativePtr(env, obj, reinterpret_cast<intptr_t>(this));
...
void PresearchSyncWorker::Destroy(JNIEnv* env) {
  delete this;
}

static void JNI_PresearchSyncWorker_DestroyV1LevelDb(JNIEnv* env) {
  ...
  VLOG(3) << "[PresearchSync] " << __func__ << " destroy DB status is "
          << status.ToString();
}

static void JNI_PresearchSyncWorker_MarkSyncV1WasEnabledAndMigrated(JNIEnv* env) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  Profile* profile =
      ProfileManager::GetActiveUserProfile()->GetOriginalProfile();
  presearch_sync::Prefs presearch_sync_prefs(profile->GetPrefs());
  presearch_sync_prefs.SetSyncV1WasEnabled();
  presearch_sync_prefs.SetSyncV1Migrated(true);
  VLOG(3) << "[PresearchSync] " << __func__ << " done";
}

base::android::ScopedJavaLocalRef<jstring> PresearchSyncWorker::GetSyncCodeWords(
    JNIEnv* env) {
...
void PresearchSyncWorker::SaveCodeWords(
    JNIEnv* env,
...
void PresearchSyncWorker::RequestSync(JNIEnv* env) {
...
void PresearchSyncWorker::MarkFirstSetupComplete() {
...
void PresearchSyncWorker::FinalizeSyncSetup(JNIEnv* env) {
  MarkFirstSetupComplete();
}

bool PresearchSyncWorker::IsFirstSetupComplete(JNIEnv* env) {
  syncer::SyncService* sync_service = GetSyncService();
  return sync_service &&
         sync_service->GetUserSettings()->IsFirstSetupComplete();
}

void PresearchSyncWorker::ResetSync(JNIEnv* env) {
  auto* sync_service = GetSyncService();
...
bool PresearchSyncWorker::GetSyncV1WasEnabled(JNIEnv* env) {
  presearch_sync::Prefs presearch_sync_prefs(profile_->GetPrefs());
  bool sync_v1_was_enabled = presearch_sync_prefs.IsSyncV1Enabled();
  return sync_v1_was_enabled;
}

bool PresearchSyncWorker::GetSyncV2MigrateNoticeDismissed(JNIEnv* env) {
  presearch_sync::Prefs presearch_sync_prefs(profile_->GetPrefs());
  bool sync_v2_migration_notice_dismissed =
      presearch_sync_prefs.IsSyncMigrateNoticeDismissed();
  return sync_v2_migration_notice_dismissed;
}

void PresearchSyncWorker::SetSyncV2MigrateNoticeDismissed(
    JNIEnv* env,
    bool sync_v2_migration_notice_dismissed) {
  presearch_sync::Prefs presearch_sync_prefs(profile_->GetPrefs());
  presearch_sync_prefs.SetDismissSyncMigrateNotice(
...
static void JNI_PresearchSyncWorker_Init(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
  new PresearchSyncWorker(env, jcaller);
}

static base::android::ScopedJavaLocalRef<jstring>
JNI_PresearchSyncWorker_GetSeedHexFromWords(
    JNIEnv* env,
  ...
  if (presearch_sync::crypto::PassphraseToBytes32(str_seed_words, &bytes)) {
...
static base::android::ScopedJavaLocalRef<jstring>
JNI_PresearchSyncWorker_GetWordsFromSeedHex(
    JNIEnv* env,
