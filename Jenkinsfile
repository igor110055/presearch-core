        ...
        stage('build') {
            ...
            steps {
                script {
                    ...
                    OTHER_REPO = REPO.equals('presearch-browser') ? 'presearch-core' : 'presearch-browser'
                    PLATFORM = JOB_NAME.substring(JOB_NAME.indexOf('-build-pr') + 10, JOB_NAME.indexOf('/PR-'))
                    PIPELINE_NAME = 'pr-presearch-browser-' + CHANGE_BRANCH.replace('/', '-') + '-' + PLATFORM
                    ...
                    withCredentials([usernamePassword(credentialsId: 'presearch-builds-github-token-for-pr-builder', usernameVariable: 'PR_BUILDER_USER', passwordVariable: 'PR_BUILDER_TOKEN')]) {
                        GITHUB_API = 'https://api.github.com/repos/presearch'
                        GITHUB_AUTH_HEADERS = [[name: 'Authorization', value: 'token ' + PR_BUILDER_TOKEN]]
                        def prDetails = readJSON(text: httpRequest(url: GITHUB_API + '/' + REPO + '/pulls?head=presearch:' + CHANGE_BRANCH, customHeaders: GITHUB_AUTH_HEADERS, quiet: true).content)[0]
                        ...
                        if (branchExistsInOtherRepo) {
                            def otherPrDetails = readJSON(text: httpRequest(url: GITHUB_API + '/' + OTHER_REPO + '/pulls?head=presearch:' + CHANGE_BRANCH, customHeaders: GITHUB_AUTH_HEADERS, quiet: true).content)[0]
                            if (otherPrDetails) {
                            ...
                            definition {
                                cpsScm {
                                    scm {
                                        git {
                                            remote {
                                                credentials('presearch-builds-github-token-for-pr-builder')
                                                github('presearch/devops', 'https')
                                            }
                                            branch('master')
                                        }
                                    }
                                    scriptPath("jenkins/jobs/browser/pr-presearch-browser-${PLATFORM}.Jenkinsfile")
                                }
                            }
                        }
                    """)
