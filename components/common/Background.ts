...
export function send<U= void, T= void> (messageType: string, payload?: T): Promise<U> {
  ...
  console.debug(`Sending data to presearch extension for ${messageType}`, { messageType, payload })
  return new Promise(function (resolve) {
    chrome.runtime.sendMessage(presearchExtensionId, { messageType, payload }, function (responseData: U) {
      console.debug(`got response from presearch extension for "${messageType}"`, responseData)
