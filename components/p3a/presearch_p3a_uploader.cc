...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotation(
  ...
  if (upload_type == "p3a") {
    return net::DefineNetworkTrafficAnnotation("metrics_report_uma", R"(
        ...
        policy {
          cookies_allowed: NO
          setting:
            "Users can enable or disable it in presearch://settings/privacy"
  ...
  return net::DefineNetworkTrafficAnnotation("metrics_report_uma", R"(
      ...
      policy {
        cookies_allowed: NO
        setting:
          "Users can enable or disable it by enabling or disabling Presearch rewards
         or ads in presearch://rewards"
         policy_exception_justification:
           "Not implemented."
      })");
}
...
PresearchP3AUploader::PresearchP3AUploader(
...
PresearchP3AUploader::~PresearchP3AUploader() = default;

void PresearchP3AUploader::UploadLog(const std::string& compressed_log_data,
                                 const std::string& upload_type) {
  auto resource_request = std::make_unique<network::ResourceRequest>();
  if (upload_type == "p2a") {
    resource_request->url = p2a_endpoint_;
    resource_request->headers.SetHeader("X-Presearch-P2A", "?1");
  } else if (upload_type == "p3a") {
    resource_request->url = p3a_endpoint_;
    resource_request->headers.SetHeader("X-Presearch-P3A", "?1");
  } else {
    NOTREACHED();
  }
  ...
  url_loader_->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
      ...
      base::BindOnce(&PresearchP3AUploader::OnUploadComplete,
                     base::Unretained(this)));
}

void PresearchP3AUploader::OnUploadComplete(
