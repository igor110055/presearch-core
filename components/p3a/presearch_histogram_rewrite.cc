...
// Please keep this list sorted and synced with |DoHistogramPresearchzation|.
constexpr const char* kPresearchzationHistograms[] = {
...
// Records the given sample using the proper Presearch way.
void DoHistogramPresearchzation(const char* histogram_name,
                            uint64_t name_hash,
  ...
  if (strcmp("Bookmarks.Count.OnProfileLoad", histogram_name) == 0) {
    ...
    UMA_HISTOGRAM_EXACT_LINEAR("Presearch.Core.BookmarksCountOnProfileLoad.2",
  ...
  if (strcmp("DefaultBrowser.State", histogram_name) == 0) {
    ...
    UMA_HISTOGRAM_BOOLEAN("Presearch.Core.IsDefault", answer);
  }
  ...
  if (strcmp("Extensions.LoadExtension", histogram_name) == 0) {
    ...
    UMA_HISTOGRAM_EXACT_LINEAR("Presearch.Core.NumberOfExtensions", answer, 3);
    return;
  }
  ...
  if (strcmp("Tabs.TabCount", histogram_name) == 0 ||
    ...
    UMA_HISTOGRAM_EXACT_LINEAR("Presearch.Core.TabCount", answer, 4);
    return;
  }
  ...
  if (strcmp("Tabs.WindowCount", histogram_name) == 0) {
    ...
    UMA_HISTOGRAM_EXACT_LINEAR("Presearch.Core.WindowCount.2", answer, 3);
    return;
  }
}
...
void SetupHistogramsPresearchization() {
  for (const char* histogram_name : kPresearchzationHistograms) {
    base::StatisticsRecorder::SetCallback(
        histogram_name,
        base::BindRepeating(&DoHistogramPresearchzation));
  }
}
