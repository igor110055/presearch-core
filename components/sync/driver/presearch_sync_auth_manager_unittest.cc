...
class PresearchSyncAuthManagerTest : public testing::Test {
 protected:
  ...
  PresearchSyncAuthManagerTest() : identity_env_(&test_url_loader_factory_) {}

  ~PresearchSyncAuthManagerTest() override {}

  std::unique_ptr<PresearchSyncAuthManager> CreateAuthManager(
      const AccountStateChangedCallback& account_state_changed,
      const CredentialsChangedCallback& credentials_changed) {
    return std::make_unique<PresearchSyncAuthManager>(
        identity_env_.identity_manager(), account_state_changed,
        credentials_changed);
  }

  void SetNetworkTime() {
    presearch_sync::NetworkTimeHelper::GetInstance()->SetNetworkTimeForTest(
...
TEST_F(PresearchSyncAuthManagerTest, IgnoresEventsIfNotRegistered) {
...
TEST_F(PresearchSyncAuthManagerTest, GetAccessToken) {
  base::MockCallback<AccountStateChangedCallback> account_state_changed;
  base::MockCallback<CredentialsChangedCallback> credentials_changed;
  ...
  const std::string kPresearchServerKeyHeaderString =
    std::string(kPresearchServicesKeyHeader) + ": " + PRESEARCH_SERVICES_KEY;

  ASSERT_EQ(auth_manager->GetCredentials().access_token,
            std::string(kAccessToken) + "\r\n" + kPresearchServerKeyHeaderString);
...
TEST_F(PresearchSyncAuthManagerTest, Reset) {
  base::MockCallback<AccountStateChangedCallback> account_state_changed;
  base::MockCallback<CredentialsChangedCallback> credentials_changed;
...
TEST_F(PresearchSyncAuthManagerTest, MalformedSyncCode) {
  base::MockCallback<AccountStateChangedCallback> account_state_changed;
  base::MockCallback<CredentialsChangedCallback> credentials_changed;
  EXPECT_CALL(account_state_changed, Run()).Times(0);
  EXPECT_CALL(credentials_changed, Run()).Times(0);
  ...
  auth_manager->DeriveSigningKeys("presearch5566");
  EXPECT_TRUE(
      auth_manager->GetActiveAccountInfo().account_info.account_id.empty());
}
