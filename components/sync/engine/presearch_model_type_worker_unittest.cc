...
#include "presearch/components/sync/engine/presearch_model_type_worker.h"
...
class PresearchModelTypeWorkerTest : public ::testing::Test {
 protected:
  explicit PresearchModelTypeWorkerTest(ModelType model_type = PREFERENCES)
      : model_type_(model_type), is_processor_disconnected_(false) {}

  ~PresearchModelTypeWorkerTest() override {}
  ...
  void InitializeWithState(const ModelType type, const ModelTypeState& state) {
    ...
    processor->SetDisconnectCallback(
        base::BindOnce(&PresearchModelTypeWorkerTest::DisconnectProcessor,
    ...
    worker_ = std::make_unique<PresearchModelTypeWorker>(
  ...
  PresearchModelTypeWorker* worker() { return worker_.get(); }
 ...
 private:
  ...
  std::unique_ptr<PresearchModelTypeWorker> worker_;
...
TEST_F(PresearchModelTypeWorkerTest, ResetProgressMarker) {
  g_minimal_time_between_reset_marker =
      PresearchModelTypeWorker::MinimalTimeBetweenResetForTests();
  ...
  for (const auto& err : err_codes) {
    ...
    for (size_t i = 0;
         i < PresearchModelTypeWorker::GetFailuresToResetMarkerForTests() - 1;
         ++i) {
      auto time_override = AdvanceTimeToAllowResetMarker();
      worker()->OnCommitResponse(CommitResponseDataList(),
...
TEST_F(PresearchModelTypeWorkerTest, ResetProgressMarkerMaxPeriod) {
  NormalInitialize();
  auto error_response_list =
      MakeErrorResponseList(CommitResponse_ResponseType_CONFLICT);

  for (size_t i = 0;
       i < PresearchModelTypeWorker::GetFailuresToResetMarkerForTests() - 1; ++i) {
...
TEST_F(PresearchModelTypeWorkerTest, ResetProgressMarkerDisabledFeature) {
  base::test::ScopedFeatureList feature_list;
  feature_list.InitAndDisableFeature(features::kPresearchSyncResetProgressMarker);
