...
const base::Feature kPresearchSyncResetProgressMarker{
...
PresearchModelTypeWorker::PresearchModelTypeWorker(
    ModelType type,
...
void PresearchModelTypeWorker::OnCommitResponse(
    const CommitResponseDataList& committed_response_list,
    const FailedCommitResponseDataList& error_response_list) {
  ModelTypeWorker::OnCommitResponse(committed_response_list,
                                    error_response_list);

  if (!base::FeatureList::IsEnabled(features::kPresearchSyncResetProgressMarker)) {
    return;
  }
...
// static
size_t PresearchModelTypeWorker::GetFailuresToResetMarkerForTests() {
  return kFailuresToResetMarker;
}

// static
base::TimeDelta PresearchModelTypeWorker::MinimalTimeBetweenResetForTests() {
  return kMinimalTimeBetweenResetMarker;
}

bool PresearchModelTypeWorker::IsResetProgressMarkerRequired(
    const FailedCommitResponseDataList& error_response_list) {
  if (!last_reset_marker_time_.is_null() &&
    ...
    // in less than 30mins
    // P3A sample is 1
    base::UmaHistogramExactLinear("Presearch.Sync.ProgressTokenEverReset", 1, 1);
    return false;
  }
...
void PresearchModelTypeWorker::ResetProgressMarker() {
  VLOG(1) << "Reset progress marker for type " << ModelTypeToString(type_);
  // Normal reset of progress marker due to 7th failure
  // P3A sample is 0
  base::UmaHistogramExactLinear("Presearch.Sync.ProgressTokenEverReset", 0, 1);
