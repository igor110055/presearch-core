...
#include "presearch/components/greaselion/browser/greaselion_service_impl.h"
...
ConvertGreaselionRuleToExtensionOnTaskRunner(
  ...
  // rule name to a known Presearch domain and hash the result to create a
  // public key.
  ...
  if (!command_line.HasSwitch(presearch_component_updater::kUseGoUpdateDev) &&
      !base::FeatureList::IsEnabled(
          presearch_component_updater::kUseDevUpdaterUrl)) {
...
GreaselionServiceImpl::GreaselionServiceImpl(
  ...
  state_[GreaselionFeature::SUPPORTS_MINIMUM_PRESEARCH_VERSION] = true;
}
