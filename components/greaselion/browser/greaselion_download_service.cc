...
#include "presearch/components/greaselion/browser/switches.h"
...
// Note(petemill): "presearch" instead of "browser" version in order
// to preserve some sense of cross-browser targetting of the scripts.
const char kMinimumPresearchVersion[] = "minimum_presearch_version";
// precondition keys
...
const char kSupportsMinimumPresearchVersion[] =
    "supports-minimum-presearch-version";
...
void GreaselionRule::Parse(base::DictionaryValue* preconditions_value,
                           ...
                           const std::string& minimum_presearch_version_value,
  ...
  if (preconditions_value) {
    for (const auto& kv : preconditions_value->DictItems()) {
      ...
      } else if (kv.first == kSupportsMinimumPresearchVersion) {
        preconditions_.supports_minimum_presearch_version = condition;
      } else {
  ...
  minimum_presearch_version_ = minimum_presearch_version_value;
...
bool GreaselionRule::Matches(
  ...
  if (!PreconditionFulfilled(preconditions_.auto_contribution_enabled,
    ...
    return false;
  if (!PreconditionFulfilled(preconditions_.supports_minimum_presearch_version,
                          state[greaselion::SUPPORTS_MINIMUM_PRESEARCH_VERSION]))
    return false;
  if (!PreconditionFulfilled(preconditions_.ads_enabled,
                             state[greaselion::ADS]))
    return false;
  // Validate against browser version.
  if (base::Version::IsValidWildcardString(minimum_presearch_version_)) {
    bool rule_version_is_higher_than_browser =
        (browser_version.CompareToWildcardString(minimum_presearch_version_) < 0);
...
void GreaselionDownloadService::OnDATFileDataReady(std::string contents) {
  ...
  for (base::Value& rule_it : root_list->GetList()) {
    ...
    const std::string* minimum_presearch_version_ptr = rule_it.FindStringPath(
        kMinimumPresearchVersion);
    const std::string minimum_presearch_version_value =
        minimum_presearch_version_ptr ? *minimum_presearch_version_ptr : "";
    ...
    rule->Parse(preconditions_value, urls_value, scripts_value, run_at_value,
        minimum_presearch_version_value, messages_path, resource_dir_);
