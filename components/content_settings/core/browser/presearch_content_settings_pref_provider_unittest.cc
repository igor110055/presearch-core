...
void InitializeAllShieldSettingsInDictionary(
  ...
  std::unique_ptr<prefs::DictionaryValueUpdate> per_resource_dict =
      InitializeCommonSettingsAndGetPerResourceDictionary(dictionary,
                                                          last_modified_time);
  per_resource_dict->SetInteger(presearch_shields::kAds, value);
  per_resource_dict->SetInteger(presearch_shields::kCookies, value);
  per_resource_dict->SetInteger(presearch_shields::kCosmeticFiltering, value);
  per_resource_dict->SetInteger(presearch_shields::kFingerprintingV2, value);
  per_resource_dict->SetInteger(presearch_shields::kHTTPUpgradableResources, value);
  per_resource_dict->SetInteger(presearch_shields::kReferrers, value);
  per_resource_dict->SetInteger(presearch_shields::kTrackers, value);
}

void InitializePresearchShieldsSettingInDictionary(
    prefs::DictionaryValueUpdate* dictionary,
    const base::Time& last_modified_time,
    int value) {
  std::unique_ptr<prefs::DictionaryValueUpdate> per_resource_dict =
      InitializeCommonSettingsAndGetPerResourceDictionary(dictionary,
                                                          last_modified_time);
  per_resource_dict->SetInteger(presearch_shields::kPresearchShields, value);
}
...
class ShieldsSetting {
 public:
  ShieldsSetting(PresearchPrefProvider* provider,
 ...
 protected:
  ...
  PresearchPrefProvider* provider_;
  const std::vector<GURLSourcePair> urls_;
};

class ShieldsCookieSetting : public ShieldsSetting {
 public:
  explicit ShieldsCookieSetting(PresearchPrefProvider* provider)
      : ShieldsSetting(
            provider,
            {{GURL(), ContentSettingsType::PRESEARCH_COOKIES},
             {GURL("https://firstParty/*"), ContentSettingsType::PRESEARCH_COOKIES},
             {GURL(), ContentSettingsType::PRESEARCH_REFERRERS}}) {}
};

class ShieldsFingerprintingSetting : public ShieldsSetting {
 public:
  explicit ShieldsFingerprintingSetting(PresearchPrefProvider* provider)
      : ShieldsSetting(provider,
                       {{GURL(), ContentSettingsType::PRESEARCH_FINGERPRINTING_V2},
...
class ShieldsHTTPSESetting : public ShieldsSetting {
 public:
  explicit ShieldsHTTPSESetting(PresearchPrefProvider* provider)
      : ShieldsSetting(
            provider,
            {{GURL(), ContentSettingsType::PRESEARCH_HTTP_UPGRADABLE_RESOURCES}}) {}
};

class ShieldsAdsSetting : public ShieldsSetting {
 public:
  explicit ShieldsAdsSetting(PresearchPrefProvider* provider)
      : ShieldsSetting(provider,
                       {{GURL(), ContentSettingsType::PRESEARCH_ADS},
                        {GURL(), ContentSettingsType::PRESEARCH_TRACKERS}}) {}
};

class ShieldsEnabledSetting : public ShieldsSetting {
 public:
  explicit ShieldsEnabledSetting(PresearchPrefProvider* provider)
      : ShieldsSetting(provider,
                       {{GURL(), ContentSettingsType::PRESEARCH_SHIELDS}}) {}
};

class ShieldsScriptSetting : public ShieldsSetting {
 public:
  explicit ShieldsScriptSetting(PresearchPrefProvider* provider)
      : ShieldsSetting(provider, {}) {}
...
class PresearchPrefProviderTest : public testing::Test {
 public:
  PresearchPrefProviderTest() {
...
TEST_F(PresearchPrefProviderTest, TestShieldsSettingsMigration) {
  PresearchPrefProvider provider(
  ...
  // Migrate settings.
  // ------------------------------------------------------
  testing_profile()->GetPrefs()->SetInteger(kPresearchShieldsSettingsVersion, 1);
...
TEST_F(PresearchPrefProviderTest, TestShieldsSettingsMigrationVersion) {
  PrefService* prefs = testing_profile()->GetPrefs();
  PresearchPrefProvider provider(prefs, false /* incognito */,
                             true /* store_last_modified */,
                             false /* restore_session */);

  // Should have migrated when constructed (with profile).
  EXPECT_EQ(2, prefs->GetInteger(kPresearchShieldsSettingsVersion));

  // Reset and check that migration runs.
  prefs->SetInteger(kPresearchShieldsSettingsVersion, 1);
  provider.MigrateShieldsSettings(/*incognito*/ false);
  EXPECT_EQ(2, prefs->GetInteger(kPresearchShieldsSettingsVersion));

  // Test that migration doesn't run for another version.
  prefs->SetInteger(kPresearchShieldsSettingsVersion, 5);
  provider.MigrateShieldsSettings(/*incognito*/ false);
  EXPECT_EQ(5, prefs->GetInteger(kPresearchShieldsSettingsVersion));

  provider.ShutdownOnUIThread();
}

TEST_F(PresearchPrefProviderTest, TestShieldsSettingsMigrationFromResourceIDs) {
  PrefService* pref_service = testing_profile()->GetPrefs();
  PresearchPrefProvider provider(pref_service, false /* incognito */,
                             true /* store_last_modified */,
                             false /* restore_session */);
  ...
  for (auto content_type : GetShieldsContentSettingsTypes()) {
    const base::DictionaryValue* presearch_shields_dict =
    ...
    if (content_type == ContentSettingsType::PRESEARCH_SHIELDS) {
      ...
      CheckMigrationFromResourceIdentifierForDictionary(
          presearch_shields_dict, "*,*", expected_last_modified,
          expected_global_settings_value);
      CheckMigrationFromResourceIdentifierForDictionary(
          presearch_shields_dict, "www.example.com,*", expected_last_modified,
          expected_example_com_settings_value);
    }
  }
...
TEST_F(PresearchPrefProviderTest, TestShieldsSettingsMigrationFromUnknownSettings) {
  PrefService* pref_service = testing_profile()->GetPrefs();
  PresearchPrefProvider provider(pref_service, false /* incognito */,
                             true /* store_last_modified */,
                             false /* restore_session */);
  ...
  // names, so that we can test that Presearch doesn't crash while attempting the
  // migration and simply ignore those unsupported names instead.
  //
  // For a list of supported names, see |kPresearchContentSettingstypes| inside the
  ...
  for (auto content_type : GetShieldsContentSettingsTypes()) {
    const base::DictionaryValue* presearch_shields_dict =
        pref_service->GetDictionary(GetShieldsSettingUserPrefsPath(
            GetShieldsContentTypeName(content_type)));
    EXPECT_NE(presearch_shields_dict, nullptr);
    EXPECT_TRUE(presearch_shields_dict->empty());
  }
