...
Rule CloneRule(const Rule& rule, bool reverse_patterns = false) {
  // presearch plugin rules incorrectly use first party url as primary
  ...
  if (primary_pattern ==
    ...
    DCHECK(reverse_patterns);  // we should only hit this for presearch plugin rules
...
class PresearchShieldsRuleIterator : public RuleIterator {
 public:
  explicit PresearchShieldsRuleIterator(std::vector<Rule> rules)
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(PresearchShieldsRuleIterator);
};
...
// static
void PresearchPrefProvider::CopyPluginSettingsForMigration(PrefService* prefs) {
  ...
  prefs->Set("presearch.migrate.content_settings.exceptions.plugins", *plugins);
...
PresearchPrefProvider::PresearchPrefProvider(PrefService* prefs,
                                     bool off_the_record,
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&PresearchPrefProvider::OnCookiePrefsChanged,
                          base::Unretained(this)));

  MigrateShieldsSettings(off_the_record);

  OnCookieSettingsChanged(ContentSettingsType::PRESEARCH_COOKIES);
...
PresearchPrefProvider::~PresearchPrefProvider() {}

void PresearchPrefProvider::ShutdownOnUIThread() {
  RemoveObserver(this);
  PrefProvider::ShutdownOnUIThread();
}

// static
void PresearchPrefProvider::RegisterProfilePrefs(
    user_prefs::PrefRegistrySyncable* registry) {
  PrefProvider::RegisterProfilePrefs(registry);
  // Register shields settings migration pref.
  registry->RegisterIntegerPref(kPresearchShieldsSettingsVersion, 1);

  // migration of obsolete plugin prefs
  registry->RegisterDictionaryPref(
      "presearch.migrate.content_settings.exceptions.plugins");
...
void PresearchPrefProvider::MigrateShieldsSettings(bool incognito) {
...
void PresearchPrefProvider::MigrateShieldsSettingsFromResourceIds() {
  PresearchPrefProvider::CopyPluginSettingsForMigration(prefs_);
  ...
  for (base::DictionaryValue::Iterator i(*plugins_dictionary); !i.IsAtEnd();
    ...
    if (settings_dictionary->GetDictionary(kPerResourcePath,
      ...
      for (base::DictionaryValue::Iterator j(*resource_dictionary);
        ...
        if (resource_identifier == presearch_shields::kObsoleteAds)
          shields_preference_name = presearch_shields::kAds;
        else if (resource_identifier == presearch_shields::kObsoleteCookies)
          shields_preference_name = presearch_shields::kCookies;
        else
        ...
        if (shields_preference_name == presearch_shields::kPresearchShields &&
            patterns_string == "*,*") {
          continue;
        }
  ...
  prefs_->ClearPref("presearch.migrate.content_settings.exceptions.plugins");
}

void PresearchPrefProvider::MigrateShieldsSettingsFromResourceIdsForOneType(
...
void PresearchPrefProvider::MigrateShieldsSettingsV1ToV2() {
  // Check if migration is needed.
  if (prefs_->GetInteger(kPresearchShieldsSettingsVersion) != 1)
    return;

  // All sources in Presearch-specific ContentSettingsType(s) we want to migrate.
  ...
  prefs_->SetInteger(kPresearchShieldsSettingsVersion, 2);
}

void PresearchPrefProvider::MigrateShieldsSettingsV1ToV2ForOneType(
...
bool PresearchPrefProvider::SetWebsiteSetting(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
    ContentSettingsType content_type,
    std::unique_ptr<base::Value>&& in_value,
    const ContentSettingConstraints& constraints) {
  // handle changes to presearch cookie settings from chromium cookie settings UI
  if (content_type == ContentSettingsType::COOKIES) {
    auto* value = in_value.get();
    auto match = std::find_if(
        presearch_cookie_rules_[off_the_record_].begin(),
        presearch_cookie_rules_[off_the_record_].end(),
    ...
    if (match != presearch_cookie_rules_[off_the_record_].end()) {
      ...
      // convert to legacy firstParty format for presearch plugin settings
      if (plugin_primary_pattern == plugin_secondary_pattern) {
        plugin_secondary_pattern =
            ContentSettingsPattern::FromString("https://firstParty/*");
      }

      // change to type ContentSettingsType::PRESEARCH_COOKIES
      return SetWebsiteSettingInternal(
          plugin_primary_pattern, plugin_secondary_pattern,
          ContentSettingsType::PRESEARCH_COOKIES, std::move(in_value), constraints);
    }
  }
...
bool PresearchPrefProvider::SetWebsiteSettingInternal(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
    ContentSettingsType content_type,
    std::unique_ptr<base::Value>&& in_value,
...
std::unique_ptr<RuleIterator> PresearchPrefProvider::GetRuleIterator(
      ContentSettingsType content_type,
  ...
  if (content_type == ContentSettingsType::COOKIES) {
    ...
    return std::make_unique<PresearchShieldsRuleIterator>(std::move(rules));
  }

  return PrefProvider::GetRuleIterator(content_type, incognito);
}

void PresearchPrefProvider::UpdateCookieRules(ContentSettingsType content_type,
                                          bool incognito) {
  auto& rules = cookie_rules_[incognito];
  auto old_rules = std::move(presearch_cookie_rules_[incognito]);

  rules.clear();
  presearch_cookie_rules_[incognito].clear();
  ...
  if (prefs_->GetBoolean(kGoogleLoginControlType)) {
    ...
    presearch_cookie_rules_[incognito].emplace_back(CloneRule(google_auth_rule));
    ...
    presearch_cookie_rules_[incognito].emplace_back(CloneRule(firebase_rule));
  }
  ...
  auto presearch_shields_iterator = PrefProvider::GetRuleIterator(
      ContentSettingsType::PRESEARCH_SHIELDS, incognito);

  // collect shield rules
  std::vector<Rule> shield_rules;
  while (presearch_shields_iterator && presearch_shields_iterator->HasNext()) {
    shield_rules.emplace_back(CloneRule(presearch_shields_iterator->Next()));
  }

  presearch_shields_iterator.reset();

  // add presearch cookies after checking shield status
  auto presearch_cookies_iterator = PrefProvider::GetRuleIterator(
      ContentSettingsType::PRESEARCH_COOKIES, incognito);

  // Matching cookie rules against shield rules.
  while (presearch_cookies_iterator && presearch_cookies_iterator->HasNext()) {
    auto rule = presearch_cookies_iterator->Next();
    if (IsActive(rule, shield_rules)) {
      rules.emplace_back(CloneRule(rule, true));
      presearch_cookie_rules_[incognito].emplace_back(CloneRule(rule, true));
    }
  }
      ...
      rules.emplace_back(
          Rule(ContentSettingsPattern::Wildcard(),
               ...
               base::Value::FromUniquePtrValue(
                   ContentSettingToValue(CONTENT_SETTING_ALLOW)),
               base::Time(), SessionModel::Durable));
      presearch_cookie_rules_[incognito].emplace_back(
          Rule(ContentSettingsPattern::Wildcard(),
               shield_rule.primary_pattern,
               base::Value::FromUniquePtrValue(
                   ContentSettingToValue(CONTENT_SETTING_ALLOW)),
  ...
  std::vector<Rule> presearch_cookie_updates;
  for (const auto& new_rule : presearch_cookie_rules_[incognito]) {
    auto match = std::find_if(
        old_rules.begin(),
        old_rules.end(),
    ...
    if (match == old_rules.end()) {
      presearch_cookie_updates.emplace_back(CloneRule(new_rule));
    }
  }
  ...
  for (const auto& old_rule : old_rules) {
    auto match = std::find_if(
        presearch_cookie_rules_[incognito].begin(),
        presearch_cookie_rules_[incognito].end(),
    ...
    if (match == presearch_cookie_rules_[incognito].end()) {
      presearch_cookie_updates.emplace_back(
          Rule(old_rule.primary_pattern, old_rule.secondary_pattern,
               base::Value(), old_rule.expiration, old_rule.session_model));
    }
  }

  // Notify presearch cookie changes as ContentSettingsType::COOKIES
  if (initialized_ && (content_type == ContentSettingsType::PRESEARCH_COOKIES ||
                       content_type == ContentSettingsType::PRESEARCH_SHIELDS)) {
    // PostTask here to avoid content settings autolock DCHECK
    base::PostTask(
        FROM_HERE,
        {content::BrowserThread::UI, base::TaskPriority::USER_VISIBLE},
        base::BindOnce(&PresearchPrefProvider::NotifyChanges,
                       weak_factory_.GetWeakPtr(),
                       std::move(presearch_cookie_updates), incognito));
  }
}

void PresearchPrefProvider::NotifyChanges(const std::vector<Rule>& rules,
                                      bool incognito) {
...
void PresearchPrefProvider::OnCookiePrefsChanged(
    const std::string& pref) {
  OnCookieSettingsChanged(ContentSettingsType::PRESEARCH_COOKIES);
}

void PresearchPrefProvider::OnCookieSettingsChanged(
    ContentSettingsType content_type) {
  UpdateCookieRules(content_type, true);
  UpdateCookieRules(content_type, false);
}

void PresearchPrefProvider::OnContentSettingChanged(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
    ContentSettingsType content_type) {
  if (content_type == ContentSettingsType::COOKIES ||
      content_type == ContentSettingsType::PRESEARCH_COOKIES ||
      content_type == ContentSettingsType::PRESEARCH_SHIELDS) {
