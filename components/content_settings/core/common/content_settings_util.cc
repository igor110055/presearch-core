...
ContentSetting GetDefaultFromContentSettingsType(
  ...
  } else if (content_type == ContentSettingsType::PRESEARCH_COSMETIC_FILTERING) {
    return secondary_url == GURL("https://firstParty/")
        ? CONTENT_SETTING_ALLOW
        : CONTENT_SETTING_BLOCK;
  } else if (content_type == ContentSettingsType::PRESEARCH_TRACKERS) {
    return CONTENT_SETTING_BLOCK;
  } else if (content_type ==
             ContentSettingsType::PRESEARCH_HTTP_UPGRADABLE_RESOURCES) {
    return CONTENT_SETTING_BLOCK;
  } else if (content_type == ContentSettingsType::PRESEARCH_SHIELDS) {
    return CONTENT_SETTING_ALLOW;
  } else if (content_type == ContentSettingsType::PRESEARCH_REFERRERS) {
    return CONTENT_SETTING_BLOCK;
  } else if (content_type == ContentSettingsType::PRESEARCH_COOKIES) {
    return secondary_url == GURL("https://firstParty/")
        ? CONTENT_SETTING_BLOCK
        : CONTENT_SETTING_ALLOW;
  }
