...
class PresearchContentSettingsAgentImplBrowserTest : public InProcessBrowserTest {
 public:
  PresearchContentSettingsAgentImplBrowserTest()
      : https_server_(net::EmbeddedTestServer::TYPE_HTTPS) {
    feature_list_.InitAndEnableFeature(net::features::kPresearchEphemeralStorage);
  }
  ...
  void SetUpOnMainThread() override {
    ...
    browser_content_client_.reset(new PresearchContentBrowserClient());
    ...
    https_server_.RegisterRequestMonitor(base::BindRepeating(
        &PresearchContentSettingsAgentImplBrowserTest::SaveReferrer,
  ...
  void BlockReferrers() {
    content_settings()->SetContentSettingCustomScope(
        ...
        ContentSettingsType::PRESEARCH_REFERRERS, CONTENT_SETTING_BLOCK);
    ContentSettingsForOneType settings;
    content_settings()->GetSettingsForOneType(
        ContentSettingsType::PRESEARCH_REFERRERS, &settings);
  ...
  void AllowReferrers() {
    content_settings()->SetContentSettingCustomScope(
        top_level_page_pattern(), ContentSettingsPattern::Wildcard(),
        ContentSettingsType::PRESEARCH_REFERRERS, CONTENT_SETTING_ALLOW);
    ContentSettingsForOneType settings;
    content_settings()->GetSettingsForOneType(
        ContentSettingsType::PRESEARCH_REFERRERS, &settings);
    EXPECT_EQ(settings.size(), 1u);
  ...
  void Block3PCookies() {
    presearch_shields::SetCookieControlType(content_settings(),
  ...
  void ShieldsDown() {
    presearch_shields::SetPresearchShieldsEnabled(content_settings(),
                                          false,
                                          top_level_page_url());
  }
  ...
  void BlockThirdPartyFingerprinting() {
    presearch_shields::SetFingerprintingControlType(content_settings(),
                                                ControlType::BLOCK_THIRD_PARTY,
                                                top_level_page_url());
  }
 ...
 private:
  ...
  std::unique_ptr<PresearchContentBrowserClient> browser_content_client_;
...
class PresearchContentSettingsAgentImplNoEphemeralStorageBrowserTest
    : public PresearchContentSettingsAgentImplBrowserTest {
 public:
  PresearchContentSettingsAgentImplNoEphemeralStorageBrowserTest() {
    feature_list_.Reset();
    feature_list_.InitAndDisableFeature(net::features::kPresearchEphemeralStorage);
  }
};

IN_PROC_BROWSER_TEST_F(PresearchContentSettingsAgentImplBrowserTest,
                       FarbleGetImageData) {
...
class PresearchContentSettingsAgentImplV2BrowserTest
    : public PresearchContentSettingsAgentImplBrowserTest {
 public:
  void SetUp() override { PresearchContentSettingsAgentImplBrowserTest::SetUp(); }

 private:
  base::test::ScopedFeatureList scoped_feature_list_;
};

IN_PROC_BROWSER_TEST_F(PresearchContentSettingsAgentImplV2BrowserTest,
                       WebGLReadPixels) {
...
IN_PROC_BROWSER_TEST_F(PresearchContentSettingsAgentImplV2BrowserTest,
                       FarbleGetImageData) {
  // Farbling should be default when kPresearchFingerprintingV2 is enabled
  ...
  // settings and kPresearchFingerprintingV2 is enabled
  BlockFingerprinting();
  NavigateToPageWithIframe();
  hash = -1;
  EXPECT_TRUE(
  ...
  // content settings and kPresearchFingerprintingV2 is enabled
  SetFingerprintingDefault();
  NavigateToPageWithIframe();
  hash = -1;
  EXPECT_TRUE(
...
IN_PROC_BROWSER_TEST_F(PresearchContentSettingsAgentImplBrowserTest, BlockCookies) {
  BlockCookies();
...
IN_PROC_BROWSER_TEST_F(PresearchContentSettingsAgentImplBrowserTest, AllowCookies) {
  AllowCookies();
...
IN_PROC_BROWSER_TEST_F(PresearchContentSettingsAgentImplBrowserTest,
                       ChromiumCookieBlockOverridesPresearchAllowCookiesTopLevel) {
  AllowCookies();
  HostContentSettingsMap* content_settings =
...
IN_PROC_BROWSER_TEST_F(
    PresearchContentSettingsAgentImplNoEphemeralStorageBrowserTest,
    ChromiumCookieBlockOverridesPresearchAllowCookiesIframe) {
  AllowCookies();
  HostContentSettingsMap* content_settings =
      HostContentSettingsMapFactory::GetForProfile(browser()->profile());
  content_settings->SetContentSettingCustomScope(
...
IN_PROC_BROWSER_TEST_F(PresearchContentSettingsAgentImplBrowserTest, BlockScripts) {
  BlockScripts();
...
IN_PROC_BROWSER_TEST_F(PresearchContentSettingsAgentImplBrowserTest, AllowScripts) {
  AllowScripts();
