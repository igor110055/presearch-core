...
PresearchWaybackMachineTabHelper::PresearchWaybackMachineTabHelper(
...
PresearchWaybackMachineTabHelper::~PresearchWaybackMachineTabHelper() = default;

void PresearchWaybackMachineTabHelper::set_delegate(
    std::unique_ptr<PresearchWaybackMachineDelegate> delegate) {
  DCHECK(delegate);
  delegate_ = std::move(delegate);
}

void PresearchWaybackMachineTabHelper::DidFinishNavigation(
  ...
  if (const net::HttpResponseHeaders* header =
    ...
    base::SequencedTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::BindOnce(&PresearchWaybackMachineTabHelper::CreateInfoBar,
                                  weak_factory_.GetWeakPtr()));
  }
}

void PresearchWaybackMachineTabHelper::CreateInfoBar() {
  DCHECK(delegate_);
  delegate_->CreateInfoBar(web_contents());
}

bool PresearchWaybackMachineTabHelper::IsWaybackMachineEnabled() const {
  return pref_service_->GetBoolean(kPresearchWaybackMachineEnabled);
}

bool PresearchWaybackMachineTabHelper::ShouldAttachWaybackMachineInfoBar(
...
WEB_CONTENTS_USER_DATA_KEY_IMPL(PresearchWaybackMachineTabHelper)
