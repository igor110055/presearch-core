...
bool IsWebtorrentEnabled(content::BrowserContext* browser_context) {
  ...
  return registry->enabled_extensions().Contains(presearch_webtorrent_extension_id);
}
...
bool IsWebtorrentURL(const GURL& url) {
  if (url.SchemeIs(extensions::kExtensionScheme) &&
      ...
      (url.ExtractFileName() == presearch_webtorrent_extension_filename ||
       url.ExtractFileName() == presearch_webtorrent_extension_filename2)) {
    return true;
  }
