...
const updateCustomFilters = debounce((customFilters: string) => {
  chrome.send('presearch_adblock.updateCustomFilters', [customFilters])
}, 1500)
...
const adblockReducer: Reducer<AdBlock.State | undefined> = (state: AdBlock.State | undefined, action) => {
  ...
  switch (action.type) {
    case types.ADBLOCK_ENABLE_FILTER_LIST:
      chrome.send('presearch_adblock.enableFilterList', [action.payload.uuid, action.payload.enabled])
      state = {
        ...state,
        settings: {
          ...state.settings,
    ...
    case types.ADBLOCK_GET_CUSTOM_FILTERS:
      chrome.send('presearch_adblock.getCustomFilters')
      break
    case types.ADBLOCK_GET_REGIONAL_LISTS:
      chrome.send('presearch_adblock.getRegionalLists')
      break
