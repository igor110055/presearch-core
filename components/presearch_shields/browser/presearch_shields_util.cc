...
#include "presearch/components/content_settings/core/common/content_settings_util.h"
...
void RecordShieldsToggled(PrefService* local_state) {
  ::presearch_shields::MaybeRecordShieldsUsageP3A(::presearch_shields::kShutOffShields,
                                              local_state);
}

void RecordShieldsSettingChanged(PrefService* local_state) {
  ::presearch_shields::MaybeRecordShieldsUsageP3A(
      ::presearch_shields::kChangedPerSiteShields, local_state);
}
...
void SetPresearchShieldsEnabled(HostContentSettingsMap* map,
                            bool enable,
                            const GURL& url,
  ...
  map->SetContentSettingCustomScope(
      ...
      // this is 'allow_presearch_shields' so 'enable' == 'allow'
      enable ? CONTENT_SETTING_ALLOW : CONTENT_SETTING_BLOCK);

  RecordShieldsToggled(local_state);
}

void ResetPresearchShieldsEnabled(HostContentSettingsMap* map, const GURL& url) {
  if (url.is_valid() && !url.SchemeIsHTTPOrHTTPS())
    return;
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::PRESEARCH_SHIELDS, CONTENT_SETTING_DEFAULT);
}

bool GetPresearchShieldsEnabled(HostContentSettingsMap* map, const GURL& url) {
  if (url.is_valid() && !url.SchemeIsHTTPOrHTTPS())
    return false;

  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::PRESEARCH_SHIELDS);

  // see EnablePresearchShields - allow and default == true
...
void SetAdControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::PRESEARCH_ADS, GetDefaultBlockFromControlType(type));

  map->SetContentSettingCustomScope(primary_pattern,
                                    ContentSettingsPattern::Wildcard(),
                                    ContentSettingsType::PRESEARCH_TRACKERS,
...
ControlType GetAdControlType(HostContentSettingsMap* map, const GURL& url) {
  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::PRESEARCH_ADS);
...
ControlType GetCosmeticFilteringControlType(HostContentSettingsMap* map,
  ...
  ContentSetting setting = map->GetContentSetting(
      url, GURL(), ContentSettingsType::PRESEARCH_COSMETIC_FILTERING);
...
bool ShouldDoCosmeticFiltering(HostContentSettingsMap* map, const GURL& url) {
  return base::FeatureList::IsEnabled(
             features::kPresearchAdblockCosmeticFiltering) &&
         GetPresearchShieldsEnabled(map, url) &&
...
bool ShouldDoDomainBlocking(HostContentSettingsMap* map, const GURL& url) {
  ...
  if (!base::FeatureList::IsEnabled(presearch_shields::features::kPresearchDomainBlock))
    return false;

  // Don't block if Presearch Shields is down (this also handles cases where
  // the URL is not HTTP(S))
  if (!presearch_shields::GetPresearchShieldsEnabled(map, url))
    return false;

  // Don't block unless ad blocking is "aggressive"
  if (presearch_shields::GetCosmeticFilteringControlType(map, url) !=
      ControlType::BLOCK)
    return false;
...
void SetCookieControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(primary_pattern,
                                    ContentSettingsPattern::Wildcard(),
                                    ContentSettingsType::PRESEARCH_REFERRERS,
                                    GetDefaultBlockFromControlType(type));

  map->SetContentSettingCustomScope(
      primary_pattern,
      ContentSettingsPattern::FromString("https://firstParty/*"),
      ContentSettingsType::PRESEARCH_COOKIES, GetDefaultAllowFromControlType(type));

  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::PRESEARCH_COOKIES, GetDefaultBlockFromControlType(type));
...
ControlType GetCookieControlType(HostContentSettingsMap* map, const GURL& url) {
  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::PRESEARCH_COOKIES);

  ContentSetting fp_setting = map->GetContentSetting(
      url, GURL("https://firstParty/"), ContentSettingsType::PRESEARCH_COOKIES);
...
bool AllowReferrers(HostContentSettingsMap* map, const GURL& url) {
  ContentSetting setting =
      map->GetContentSetting(url, GURL(), ContentSettingsType::PRESEARCH_REFERRERS);
...
void SetFingerprintingControlType(HostContentSettingsMap* map,
  ...
  map->SetContentSettingCustomScope(
      ...
      ContentSettingsType::PRESEARCH_FINGERPRINTING_V2, CONTENT_SETTING_DEFAULT);
  map->SetContentSettingCustomScope(
      primary_pattern, ContentSettingsPattern::Wildcard(),
      ContentSettingsType::PRESEARCH_FINGERPRINTING_V2, CONTENT_SETTING_DEFAULT);
  ...
  map->SetContentSettingCustomScope(
      ...
      ContentSettingsType::PRESEARCH_FINGERPRINTING_V2, content_setting);
...
ControlType GetFingerprintingControlType(HostContentSettingsMap* map,
  ...
  ContentSetting fp_setting =
      GetPresearchFPContentSettingFromRules(fingerprinting_rules, url);
...
bool GetHTTPSEverywhereEnabled(HostContentSettingsMap* map, const GURL& url) {
  ContentSetting setting = map->GetContentSetting(
      url, GURL(), ContentSettingsType::PRESEARCH_HTTP_UPGRADABLE_RESOURCES);
...
void DispatchBlockedEvent(const GURL& request_url,
  ...
  PresearchShieldsWebContentsObserver::DispatchBlockedEvent(
      block_type, request_url.spec(), render_process_id, render_frame_id,
      frame_tree_node_id);

#if BUILDFLAG(ENABLE_PRESEARCH_PERF_PREDICTOR)
  presearch_perf_predictor::PerfPredictorTabHelper::DispatchBlockedEvent(
