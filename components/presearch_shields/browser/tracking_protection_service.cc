...
bool TrackingProtectionService::ShouldStoreState(HostContentSettingsMap* map,
  ...
  if (!presearch_shields::GetPresearchShieldsEnabled(map, starting_site))
    return true;


  if (presearch_shields::GetCookieControlType(map, starting_site) !=
      ControlType::BLOCK)
    return true;
...
#else  // !BUILDFLAG(PRESEARCH_STP_ENABLED)
bool TrackingProtectionService::ShouldStoreState(HostContentSettingsMap* map,
                                                 int render_process_id,
                                                 int render_frame_id,
                                                 const GURL& origin_url) const {
...
#endif  // BUILDFLAG(PRESEARCH_STP_ENABLED)
...
bool TrackingProtectionService::ShouldStartRequest(
  ...
  // combined with presearch/ad-block.
