...
#include "presearch/components/presearch_shields/browser/domain_block_tab_storage.h"
...
bool ShouldBlockDomainOnTaskRunner(
    ...
    const GURL& url) {
  SCOPED_UMA_HISTOGRAM_TIMER("Presearch.DomainBlock.ShouldBlock");
...
DomainBlockNavigationThrottle::WillStartRequest() {
  ...
  if (!presearch_shields::ShouldDoDomainBlocking(content_settings_, request_url))
    return content::NavigationThrottle::PROCEED;
