...
std::string GetTagFromPrefName(const std::string& pref_name) {
  if (pref_name == kFBEmbedControlType) {
    return presearch_shields::kFacebookEmbeds;
  }
  if (pref_name == kTwitterEmbedControlType) {
    return presearch_shields::kTwitterEmbeds;
  }
  if (pref_name == kLinkedInEmbedControlType) {
    return presearch_shields::kLinkedInEmbeds;
  }
  return "";
}
...
AdBlockRegionalServiceManager* AdBlockService::regional_service_manager() {
  if (!regional_service_manager_)
    regional_service_manager_ =
        presearch_shields::AdBlockRegionalServiceManagerFactory(
            component_delegate_);
...
AdBlockService::custom_filters_service() {
  if (!custom_filters_service_)
    custom_filters_service_ =
        presearch_shields::AdBlockCustomFiltersServiceFactory(component_delegate_);
...
void AdBlockPrefService::OnPreferenceChanged(const std::string& pref_name) {
  ...
  g_presearch_browser_process->ad_block_service()->EnableTag(tag, enabled);
  g_presearch_browser_process->ad_block_regional_service_manager()->EnableTag(
      tag, enabled);
  g_presearch_browser_process->ad_block_custom_filters_service()->EnableTag(
      tag, enabled);
}
