  ...
  return net::DefineNetworkTrafficAnnotation("eth_json_rpc_controller", R"(
      semantics {
        ...
        description:
          ...
          "on behalf of the user interacting with the native Presearch wallet."
        trigger:
          "Triggered by uses of the native Presearch wallet."
        data:
...
bool GetUseStagingInfuraEndpoint() {
  std::string project_id(PRESEARCH_INFURA_PROJECT_ID);
  std::unique_ptr<base::Environment> env(base::Environment::Create());
  return env->HasVar("PRESEARCH_INFURA_STAGING");
}
...
void EthJsonRpcController::SetNetwork(Network network) {
  ...
  const std::string spec =
      base::StringPrintf(GetUseStagingInfuraEndpoint()
                             ? "https://%s-staging-infura.presearchsoftware.com/%s"
                             : "https://%s-infura.presearch.org/%s",
