...
std::string PresearchIpfsClientUpdater::g_ipfs_client_component_id_(
    kIpfsClientComponentId);
std::string PresearchIpfsClientUpdater::g_ipfs_client_component_base64_public_key_(
    kIpfsClientComponentBase64PublicKey);

PresearchIpfsClientUpdater::PresearchIpfsClientUpdater(
...
void PresearchIpfsClientUpdater::Register() {
  if (registered_)
    return;

  PresearchComponent::Register(kIpfsClientComponentName,
...
void PresearchIpfsClientUpdater::SetExecutablePath(const base::FilePath& path) {
  executable_path_ = path;
  for (Observer& observer : observers_)
    observer.OnExecutableReady(path);
}

base::FilePath PresearchIpfsClientUpdater::GetExecutablePath() const {
  return executable_path_;
}

void PresearchIpfsClientUpdater::OnEvent(Events event, const std::string& id) {
...
void PresearchIpfsClientUpdater::OnComponentReady(const std::string& component_id,
                                              const base::FilePath& install_dir,
                                              const std::string& manifest) {
  base::PostTaskAndReplyWithResult(
      GetTaskRunner().get(), FROM_HERE,
      base::BindOnce(&InitExecutablePath, install_dir),
      base::BindOnce(&PresearchIpfsClientUpdater::SetExecutablePath,
                     weak_ptr_factory_.GetWeakPtr()));
}

void PresearchIpfsClientUpdater::AddObserver(Observer* observer) {
  observers_.AddObserver(observer);
}

void PresearchIpfsClientUpdater::RemoveObserver(Observer* observer) {
  observers_.RemoveObserver(observer);
}

void PresearchIpfsClientUpdater::Cleanup() {
...
// static
void PresearchIpfsClientUpdater::SetComponentIdAndBase64PublicKeyForTest(
...
// The Presearch Ipfs client extension factory.
std::unique_ptr<PresearchIpfsClientUpdater> PresearchIpfsClientUpdaterFactory(
    PresearchComponent::Delegate* delegate,
    const base::FilePath& user_data_dir) {
  return std::make_unique<PresearchIpfsClientUpdater>(delegate, user_data_dir);
}
