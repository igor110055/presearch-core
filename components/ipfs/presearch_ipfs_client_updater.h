...
#ifndef PRESEARCH_COMPONENTS_IPFS_PRESEARCH_IPFS_CLIENT_UPDATER_H_
#define PRESEARCH_COMPONENTS_IPFS_PRESEARCH_IPFS_CLIENT_UPDATER_H_
...
class PresearchIpfsClientUpdaterTest;
...
static const char kIpfsClientComponentName[] =
    "Presearch Ipfs Client Updater (Windows)";
...
#elif defined(OS_MAC)
static const char kIpfsClientComponentName[] =
    "Presearch Ipfs Client Updater (Mac)";
...
static const char kIpfsClientComponentName[] =
    "Presearch Ipfs Client Updater (Linux)";
...
class PresearchIpfsClientUpdater : public PresearchComponent,
                               public PresearchComponent::ComponentObserver {
 public:
  class Observer : public base::CheckedObserver {
   public:
    virtual void OnExecutableReady(const base::FilePath& path) = 0;
    virtual void OnInstallationEvent(
        PresearchComponent::ComponentObserver::Events event) = 0;

   protected:
    ~Observer() override = default;
  };

  explicit PresearchIpfsClientUpdater(PresearchComponent::Delegate* delegate,
                                  const base::FilePath& user_data_dir);
  ~PresearchIpfsClientUpdater() override;
 ...
 protected:
  ...
  // PresearchComponent::ComponentObserver
  void OnEvent(Events event, const std::string& id) override;

 private:
  friend class ::PresearchIpfsClientUpdaterTest;
  ...
  base::ScopedObservation<PresearchComponent, PresearchComponent::ComponentObserver>
      updater_observer_{this};

  base::WeakPtrFactory<PresearchIpfsClientUpdater> weak_ptr_factory_;

  DISALLOW_COPY_AND_ASSIGN(PresearchIpfsClientUpdater);
};

// Creates the PresearchIpfsClientUpdater
...
#endif  // PRESEARCH_COMPONENTS_IPFS_PRESEARCH_IPFS_CLIENT_UPDATER_H_
