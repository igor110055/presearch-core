...
#include "presearch/components/ntp_background_images/browser/ntp_background_images_component_installer.h"
...
#include "presearch/components/ntp_background_images/browser/sponsored_images_component_data.h"
#include "presearch/components/ntp_background_images/browser/switches.h"
...
void NTPBackgroundImagesService::Init() {
  ...
  if (base::FeatureList::IsEnabled(features::kPresearchNTPSuperReferralWallpaper)) {
    // Flag override for testing or demo purposes
    base::FilePath forced_local_path(
        base::CommandLine::ForCurrentProcess()->GetSwitchValueNative(
...
void NTPBackgroundImagesService::RegisterSponsoredImagesComponent() {
  ...
  const auto& data = GetSponsoredImagesComponentData(
      presearch_l10n::GetCountryCode(locale));
  if (!data) {
...
void NTPBackgroundImagesService::CheckSuperReferralComponent() {
  ...
  if (local_pref_->FindPreference(
    ...
    if (presearch::PresearchReferralsService::IsDefaultReferralCode(code)) {
      MarkThisInstallIsNotSuperReferralForever();
    } else {
...
void NTPBackgroundImagesService::OnPreferenceChanged(
  ...
  if (presearch::PresearchReferralsService::IsDefaultReferralCode(new_referral_code)) {
...
NTPBackgroundImagesService::GetBackgroundImagesData(bool super_referral) const {
  const bool is_sr_enabled =
      base::FeatureList::IsEnabled(features::kPresearchNTPSuperReferralWallpaper);
  if (is_sr_enabled) {
    if (super_referral) {
