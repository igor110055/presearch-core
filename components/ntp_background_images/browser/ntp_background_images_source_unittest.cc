...
class NTPBackgroundImagesSourceTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    presearch::RegisterPrefsForPresearchReferralsService(registry);
...
TEST_F(NTPBackgroundImagesSourceTest, BasicTest) {
  ...
  EXPECT_FALSE(source_->IsValidPath("super-duper/presearch.png"));
...
TEST_F(NTPBackgroundImagesSourceTest, BasicSuperReferralDataTest) {
  ...
  EXPECT_TRUE(source_->IsValidPath("super-referral/presearch.png"));
  EXPECT_FALSE(source_->IsValidPath("super-duper/presearch.png"));
...
#endif  // ENABLE_PRESEARCH_REFERRALS
