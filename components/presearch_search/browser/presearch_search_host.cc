...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("presearch_search_host", R"(
      semantics {
        sender: "Presearch Search Host Controller"
        description:
          "This controller is used as a backup search "
          "provider for users that have opted into this feature."
        trigger:
          "Triggered by Presearch search if a user has opted in."
        data:
...
void PresearchSearchHost::SetBackupProviderForTest(const GURL& backup_provider) {
  backup_provider_for_test = backup_provider;
}

PresearchSearchHost::PresearchSearchHost(
...
// [static]
GURL PresearchSearchHost::GetBackupResultURL(const GURL& baseURL,
...
void PresearchSearchHost::FetchBackupResults(const std::string& query,
                                         const std::string& lang,
                                         const std::string& country,
                                         const std::string& geo,
                                         bool filter_explicit_results,
  ...
  iter->get()->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
      ...
      base::BindOnce(&PresearchSearchHost::OnURLLoaderComplete,
                     weak_factory_.GetWeakPtr(), iter, std::move(callback)));
}

void PresearchSearchHost::OnURLLoaderComplete(
