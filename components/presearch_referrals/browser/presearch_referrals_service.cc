...
#include "presearch_base/random.h"
...
PresearchReferralsService::ReferralInitializedCallback
...
std::string BuildReferralEndpoint(const std::string& path) {
  ...
  env->GetVar("PRESEARCH_REFERRALS_SERVER", &referral_server);
  if (referral_server.empty())
    referral_server = kPresearchReferralsServer;
  if (env->HasVar("PRESEARCH_REFERRALS_LOCAL"))
    proto = "http";
...
PresearchReferralsService::PresearchReferralsService(PrefService* pref_service,
...
PresearchReferralsService::~PresearchReferralsService() {
}

void PresearchReferralsService::OnProfileAdded(Profile* profile) {
...
void PresearchReferralsService::Start() {
  if (initialized_)
    return;
  ...
  finalization_checks_timer_->Start(
      ...
      base::TimeDelta::FromSeconds(
          presearch_base::random::Geometric(kFinalizationChecksFrequency)),
      this, &PresearchReferralsService::OnFinalizationChecksTimerFired);
  ...
  fetch_referral_headers_timer_->Start(
      FROM_HERE,
      base::TimeDelta::FromSeconds(
        presearch_base::random::Geometric(kFetchReferralHeadersFrequency)),
      this, &PresearchReferralsService::OnFetchReferralHeadersTimerFired);
  ...
  if (!checked_for_promo_code_file && !has_initialized && download_id.empty())
    task_runner_->PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&PresearchReferralsService::OnReadPromoCodeComplete,
                   weak_factory_.GetWeakPtr()));

  initialized_ = true;
}

void PresearchReferralsService::Stop() {
...
// static
void PresearchReferralsService::SetReferralInitializedCallbackForTesting(
...
// static
bool PresearchReferralsService::IsDefaultReferralCode(const std::string& code) {
  return code == kDefaultPromoCode;
}

// static
bool PresearchReferralsService::GetMatchingReferralHeaders(
...
void PresearchReferralsService::OnFinalizationChecksTimerFired() {
  PerformFinalizationChecks();
}

void PresearchReferralsService::OnFetchReferralHeadersTimerFired() {
  FetchReferralHeaders();
}

void PresearchReferralsService::OnReferralHeadersLoadComplete(
...
void PresearchReferralsService::OnReferralInitLoadComplete(
    std::unique_ptr<std::string> response_body) {
  int response_code = -1;
  ...
  if (referral_init_loader_->NetError() != net::OK || response_code < 200 ||
    ...
    initialization_timer_->Start(
                    FROM_HERE,
                    base::TimeDelta::FromSeconds(
                      presearch_base::random::Geometric(
                              kReportInitializationFrequency)),
                    this, &PresearchReferralsService::InitReferral);
...
void PresearchReferralsService::OnReferralFinalizationCheckLoadComplete(
    std::unique_ptr<std::string> response_body) {
  int response_code = -1;
...
void PresearchReferralsService::OnReadPromoCodeComplete(
...
void PresearchReferralsService::GetFirstRunTime() {
#if defined(OS_ANDROID)
  ...
  task_runner_->PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&PresearchReferralsService::SetFirstRunTime,
                     weak_factory_.GetWeakPtr()));
#endif
}

void PresearchReferralsService::SetFirstRunTime(
...
void PresearchReferralsService::PerformFinalizationChecks() {
  // Delete the promo code preference, if appropriate.
  base::PostTask(
      FROM_HERE, {content::BrowserThread::UI},
      base::BindOnce(&PresearchReferralsService::MaybeDeletePromoCodePref,
                     weak_factory_.GetWeakPtr()));

  // Check for referral finalization, if appropriate.
  base::PostTask(
      FROM_HERE, {content::BrowserThread::UI},
      base::BindOnce(&PresearchReferralsService::MaybeCheckForReferralFinalization,
                     weak_factory_.GetWeakPtr()));
}

// static
void PresearchReferralsService::SetPromoFilePathForTesting(
    const base::FilePath& path) {
  g_promo_file_path = path;
}

base::FilePath PresearchReferralsService::GetPromoCodeFileName() const {
...
void PresearchReferralsService::MaybeCheckForReferralFinalization() {
  ...
  env->GetVar("PRESEARCH_REFERRALS_CHECK_TIME", &check_time_str);
...
void PresearchReferralsService::MaybeDeletePromoCodePref() const {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);

  uint64_t delete_time = 90 * 24 * 60 * 60;
  std::unique_ptr<base::Environment> env(base::Environment::Create());
  std::string delete_time_str;
  env->GetVar("PRESEARCH_REFERRALS_DELETE_TIME", &delete_time_str);
...
std::string PresearchReferralsService::BuildReferralInitPayload() const {
...
std::string PresearchReferralsService::BuildReferralFinalizationCheckPayload()
    const {
  base::Value root(base::Value::Type::DICTIONARY);
  root.SetKey("api_key", base::Value(api_key_));
...
void PresearchReferralsService::FetchReferralHeaders() {
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation(
        "presearch_referral_headers_fetcher", R"(
        semantics {
          sender:
            "Presearch Referrals Service"
          description:
            "Fetches referral headers from Presearch."
          trigger:
            "An update timer indicates that it's time to fetch referral headers."
          data: "Presearch referral headers."
          destination: WEBSITE
        }
        policy {
          cookies_allowed: NO
  ...
  resource_request->url =
      GURL(BuildReferralEndpoint(kPresearchReferralsHeadersPath));
  ...
  referral_headers_loader_->DownloadToString(
      loader_factory,
      base::BindOnce(&PresearchReferralsService::OnReferralHeadersLoadComplete,
                     base::Unretained(this)),
      kMaxReferralServerResponseSizeBytes);
}

void PresearchReferralsService::InitReferral() {
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("presearch_referral_initializer", R"(
        semantics {
          sender:
            "Presearch Referrals Service"
          description:
            "Validates the current referral offer with Presearch, potentially "
            "unlocking special features and/or services."
          trigger:
            "On startup, sends the current referral code to Presearch."
          data: "Presearch referral metadata."
          destination: WEBSITE
        }
        policy {
          cookies_allowed: NO
  ...
  resource_request->url = GURL(BuildReferralEndpoint(kPresearchReferralsInitPath));
  resource_request->credentials_mode = network::mojom::CredentialsMode::kOmit;
  resource_request->load_flags = net::LOAD_DO_NOT_SAVE_COOKIES |
                                 net::LOAD_BYPASS_CACHE |
                                 net::LOAD_DISABLE_CACHE;
  ...
  referral_init_loader_->DownloadToString(
      loader_factory,
      base::BindOnce(&PresearchReferralsService::OnReferralInitLoadComplete,
                     base::Unretained(this)),
      kMaxReferralServerResponseSizeBytes);
}

#if defined(OS_ANDROID)
void PresearchReferralsService::GetSafetynetStatusResult(
...
void PresearchReferralsService::CheckForReferralFinalization() {
#if defined(OS_ANDROID)
  if (pref_service_->GetString(kSafetynetStatus).empty()) {
    // Get safetynet status before finalization
    safetynet_check::ClientAttestationCallback attest_callback =
        base::BindOnce(&PresearchReferralsService::GetSafetynetStatusResult,
                       weak_factory_.GetWeakPtr());
...
#endif
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("presearch_referral_finalization_checker",
        R"(
        semantics {
          sender:
            "Presearch Referrals Service"
          description:
            "Fetches referral finalization data from Presearch."
          trigger:
            ""
          data: "Presearch referral finalization status."
          destination: WEBSITE
        }
        policy {
          cookies_allowed: NO
  ...
  resource_request->method = "PUT";
  resource_request->url =
      GURL(BuildReferralEndpoint(kPresearchReferralsActivityPath));
  resource_request->credentials_mode = network::mojom::CredentialsMode::kOmit;
  resource_request->load_flags = net::LOAD_DO_NOT_SAVE_COOKIES |
                                 net::LOAD_BYPASS_CACHE |
                                 net::LOAD_DISABLE_CACHE;
  ...
  referral_finalization_check_loader_->DownloadToString(
      loader_factory,
      base::BindOnce(
          &PresearchReferralsService::OnReferralFinalizationCheckLoadComplete,
          base::Unretained(this)),
      kMaxReferralServerResponseSizeBytes);
}

std::string PresearchReferralsService::FormatExtraHeaders(
