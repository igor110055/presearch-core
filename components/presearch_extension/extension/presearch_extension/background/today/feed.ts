...
import { isPublisherContentAllowed } from '../../../../../common/presearchToday'
...
type RemoteData = PresearchToday.FeedItem[]
...
let memoryTodayData: PresearchToday.Feed | undefined
...
async function setStorageData (feed: PresearchToday.Feed) {
...
function performUpdateFeed () {
  ...
  readLock = new Promise(async function (resolve, reject) {
    try {
      ...
      console.error('Could not process Presearch Today feed contents')
      reject(e)
    } finally {
      readLock = null
    }
...
export async function checkForRemoteUpdate () {
  ...
  setLastUpdateCheckTime(Date.now())
  console.debug(`Presearch Today checked for new update. New Etag is "${newEtag}". Last Etag was "${existingEtag}".`)
