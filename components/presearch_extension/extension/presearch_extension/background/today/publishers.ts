...
let memoryData: PresearchToday.Publishers | undefined
...
async function setPublishersCache (publishers: PresearchToday.Publishers) {
...
async function convertToObject (publishers: PresearchToday.Publisher[]): Promise<PresearchToday.Publishers> {
  const prefs = await getPublisherPrefs()
  const data: PresearchToday.Publishers = {}
...
function performUpdate (notify: boolean = true) {
  ...
  readLock = new Promise(async function (resolve, reject) {
    try {
      ...
      if (feedResponse.ok) {
        const feedContents: PresearchToday.Publisher[] = await feedResponse.json()
        ...
        // Notify
        if (notify) {
          publishersEvents.dispatchEvent<PresearchToday.Publishers>(eventNameChanged, memoryData)
        }
      } else {
        throw new Error(`Not ok when fetching publishers. Status ${feedResponse.status} (${feedResponse.statusText})`)
      }
    } catch (e) {
      console.error('Could not process Presearch Today sources contents')
      reject(e)
    } finally {
      readLock = null
    }
...
type changeListener = (publishers: PresearchToday.Publishers) => any
export function addPublishersChangedListener (listener: changeListener) {
  publishersEvents.addEventListener<PresearchToday.Publishers>(eventNameChanged, listener)
}
