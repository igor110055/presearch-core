...
const SETTINGS_KEY_SHOW_TODAY = 'presearch.new_tab_page.show_presearch_today'
const ALARM_KEY_FEED_UPDATE = 'presearch-today-update-feed'
const ALARM_KEY_PUBLISHERS_UPDATE = 'presearch-today-update-publishers'

//
// This module is the orchestrator for the Presearch Today backend.
...
chrome.presearchToday.onClearHistory.addListener(async () => {
  ...
  console.debug('Clearing Presearch Today feed from cache due to clear history event.')
...
async function getHasPresearchTodayBeenUsed () {
...
import Messages = PresearchToday.Messages

async function fetchIsUpdateAvailableIfTime () {
  // When the user is interacting with Presearch Today, we check
  ...
  if (shouldCheck) {
    ...
    console.debug(`Presearch Today checked for update because it's been used for 10 mins. Update ${hasUpdate ? 'was' : 'was NOT'} available.`)
    return hasUpdate
  }
  return null
}
...
Background.setListener<void>(
  ...
  async function () {
    // This will only get called from a page that Presearch Today is enabled on.
    // Start to fetch data if that hasn't happened yet,
    // but don't use resources returning it when
    // most NTP opens won't result in reading Presearch Today
    // content (or changing publisher settings).
    // Only do this if we've previously interacted with presearch today
    if (await getHasPresearchTodayBeenUsed()) {
      await Promise.all([
        Feed.getOrFetchData(),
...
// Only do certain things when Presearch Today is enabled
...
async function conditionallyStartupOrShutdown (pref: chrome.settingsPrivate.PrefObject) {
  ...
  const isPresearchTodayEnabled = pref.value
  const hasPresearchTodayBeenUsed = await getHasPresearchTodayBeenUsed()
  if (isPresearchTodayEnabled && hasPresearchTodayBeenUsed && !isStartedUp) {
    isStartedUp = true
    ensureUpdateFrequency()
  } else if (!isPresearchTodayEnabled && isStartedUp) {
    stopUpdateFrequency()
    isStartedUp = false
  }
}
