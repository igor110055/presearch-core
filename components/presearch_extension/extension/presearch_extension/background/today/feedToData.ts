...
function convertFeedItem (item: PresearchToday.FeedItem): PresearchToday.FeedItem {
...
export default async function getPresearchTodayData (
  feedContent: PresearchToday.FeedItem[],
  enabledPublishers: PresearchToday.Publishers
): Promise<PresearchToday.Feed | undefined> {
  // No sponsor content yet, wait until we have content spec
  const promotedArticles: (PresearchToday.PromotedArticle)[] = []
  const deals: (PresearchToday.Deal)[] = []
  let articles: (PresearchToday.Article)[] = []
  ...
  const pages: PresearchToday.Page[] = []
...
function isItemWithImage (item: PresearchToday.FeedItem) {
  return !!item.padded_img
}

function isArticleTopNews (article: PresearchToday.Article) {
  return article.category === 'Top News'
}

function isArticleWithin48Hours (article: PresearchToday.Article) {
...
function generateNextPage (
  articles: PresearchToday.Article[],
  allDeals: PresearchToday.Deal[],
  promotedArticles: PresearchToday.PromotedArticle[],
  featuredCategory?: string,
  dealsCategory?: string): PresearchToday.Page | null {
...
function generateArticleSourceGroup (articles: PresearchToday.Article[]): [string, PresearchToday.Article[]] | undefined {
...
async function weightArticles (articles: PresearchToday.FeedItem[]): Promise<PresearchToday.FeedItem[]> {
