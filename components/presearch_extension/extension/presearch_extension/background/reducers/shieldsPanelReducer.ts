  ...
  switch (action.type) {
    ...
    case shieldsPanelTypes.SHIELDS_PANEL_DATA_UPDATED: {
      ...
      if (chrome.test && shieldsPanelState.getActiveTabData(state)) {
        chrome.test.sendMessage('presearch-extension-shields-data-ready')
      }
      break
    }
    case shieldsPanelTypes.SHIELDS_TOGGLED: {
      ...
      setAllowPresearchShields(tabData.origin, action.setting)
        .then(() => {
      ...
      state = shieldsPanelState
        .updateTabShieldsData(state, tabId, { presearchShields: action.setting })
      break
    }
    ...
    case shieldsPanelTypes.SHIELDS_READY: {
      onShieldsPanelShown().catch(() => {
        console.error('error calling `chrome.presearchShields.onShieldsPanelShown()`')
      })
      break
    }
    ...
    case shieldsPanelTypes.CONTENT_SCRIPTS_LOADED: {
      ...
      Promise.all([chrome.presearchShields.shouldDoCosmeticFilteringAsync(action.url), chrome.presearchShields.isFirstPartyCosmeticFilteringEnabledAsync(action.url)])
