...
void RewardsBrowserTestContribution::UpdateContributionBalance(
    ...
    const ledger::type::ContributionProcessor processor) {
  if (verified) {
    if (processor == ledger::type::ContributionProcessor::PRESEARCH_TOKENS ||
        processor == ledger::type::ContributionProcessor::PRESEARCH_USER_FUNDS) {
      balance_ -= amount;
      return;
    }
...
void RewardsBrowserTestContribution::SetUpUpholdWallet(
  ...
  // we need presearch wallet as well
  ...
  wallet.SetStringKey("user_name", "Presearch Test");
