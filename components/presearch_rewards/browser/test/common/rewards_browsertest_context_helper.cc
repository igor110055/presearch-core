...
void RewardsBrowserTestContextHelper::OpenPopup() {
  ...
  std::string error;
  bool popup_shown = extensions::PresearchActionAPI::ShowActionUI(
    browser_,
...
void RewardsBrowserTestContextHelper::OpenPopupFirstTime() {
  ...
  auto* presearch_actions = presearch_location_bar_view->GetPresearchActionsContainer();
  ASSERT_NE(presearch_actions, nullptr);

  presearch_actions->OnRewardsStubButtonClicked();
  loaded_ = true;
}
...
content::WebContents* RewardsBrowserTestContextHelper::OpenRewardsPopup() {
  ...
  auto check_load_is_rewards_panel =
      [&](const content::NotificationSource& source,
        ...
        std::string rewards_panel_url = std::string("chrome-extension://") +
            presearch_rewards_extension_id + "/presearch_rewards_panel.html";
  ...
  bool ac_enabled = browser_->profile()->GetPrefs()->
      GetBoolean(presearch_rewards::prefs::kAutoContributeEnabled);
