...
// npm run test -- presearch_browser_tests --filter=RewardsStateBrowserTest.*
...
class RewardsStateBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void GetTestFile(const std::string& file_name, base::FilePath* path) const {
    ...
    test_path = test_path.Append(FILE_PATH_LITERAL("presearch"));
...
IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, State_1) {
  profile_->GetPrefs()->SetInteger("presearch.rewards.version", -1);
  rewards_browsertest_util::StartProcess(rewards_service_);
  EXPECT_EQ(
      profile_->GetPrefs()->GetInteger("presearch.rewards.ac.min_visit_time"),
      5);
  EXPECT_EQ(
      profile_->GetPrefs()->GetInteger("presearch.rewards.ac.min_visits"),
      5);
  EXPECT_EQ(profile_->GetPrefs()->GetBoolean(
      "presearch.rewards.ac.allow_non_verified"),
          false);
  EXPECT_EQ(profile_->GetPrefs()->GetBoolean(
      "presearch.rewards.ac.allow_video_contributions"),
          false);
  EXPECT_EQ(profile_->GetPrefs()->GetDouble("presearch.rewards.ac.score.a"),
      14500.0);
  EXPECT_EQ(profile_->GetPrefs()->GetDouble("presearch.rewards.ac.score.b"),
      -14000.0);
...
IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, State_2) {
  ...
  EXPECT_EQ(
      rewards_service_->GetEncryptedStringState("wallets.presearch"),
      wallet);
  EXPECT_EQ(
      profile_->GetPrefs()->GetUint64("presearch.rewards.creation_stamp"),
      1590484778ul);
  EXPECT_EQ(
      profile_->GetPrefs()->GetUint64("presearch.rewards.ac.next_reconcile_stamp"),
      2593076778ul);
  EXPECT_EQ(
      profile_->GetPrefs()->GetDouble("presearch.rewards.ac.amount"),
      20.0);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("presearch.rewards.ac.enabled"),
      true);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("presearch.rewards.inline_tip.reddit"),
      true);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("presearch.rewards.inline_tip.twitter"),
      false);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("presearch.rewards.inline_tip.github"),
      false);
}

IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, V8RewardsEnabledACEnabled) {
  profile_->GetPrefs()->SetInteger("presearch.rewards.version", -1);
  profile_->GetPrefs()->SetBoolean("presearch.rewards.enabled", true);
  profile_->GetPrefs()->SetBoolean("presearch.rewards.ac.enabled", true);
  rewards_browsertest_util::StartProcess(rewards_service_);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("presearch.rewards.ac.enabled"),
      true);
...
IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, V8RewardsEnabledACDisabled) {
  profile_->GetPrefs()->SetInteger("presearch.rewards.version", -1);
  profile_->GetPrefs()->SetBoolean("presearch.rewards.enabled", true);
  profile_->GetPrefs()->SetBoolean("presearch.rewards.ac.enabled", false);
  rewards_browsertest_util::StartProcess(rewards_service_);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("presearch.rewards.ac.enabled"),
      false);
...
IN_PROC_BROWSER_TEST_F(RewardsStateBrowserTest, V8RewardsDisabledACEnabled) {
  profile_->GetPrefs()->SetInteger("presearch.rewards.version", -1);
  profile_->GetPrefs()->SetBoolean("presearch.rewards.enabled", false);
  profile_->GetPrefs()->SetBoolean("presearch.rewards.ac.enabled", true);
  rewards_browsertest_util::StartProcess(rewards_service_);
  EXPECT_EQ(
      profile_->GetPrefs()->GetBoolean("presearch.rewards.ac.enabled"),
