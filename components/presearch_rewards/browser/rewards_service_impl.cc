...
#include "presearch/components/presearch_rewards/browser/android_util.h"
#include "presearch/components/presearch_rewards/browser/diagnostic_log.h"
#include "presearch/components/presearch_rewards/browser/logging.h"
...
#include "presearch/components/presearch_rewards/browser/static_values.h"
#include "presearch/components/presearch_rewards/browser/switches.h"
...
#include "presearch/components/services/bat_ledger/public/cpp/ledger_client_mojo_bridge.h"
...
const char pref_prefix[] = "presearch.rewards";
...
GetNetworkTrafficAnnotationTagForFaviconFetch() {
  return net::DefineNetworkTrafficAnnotation(
      "presearch_rewards_favicon_fetcher", R"(
      semantics {
        sender:
          "Presearch Rewards Media Fetcher"
        description:
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTagForURLLoad() {
  return net::DefineNetworkTrafficAnnotation("rewards_service_impl", R"(
      semantics {
        sender: "Presearch Rewards service"
        description:
        ...
        trigger:
          "User-initiated for direct tipping or on a set interval while Presearch "
      ...
      policy {
        ...
        setting:
          ...
          "bar or by visiting presearch://rewards/."
        policy_exception_justification:
          "Not implemented."
      })");
}
...
RewardsServiceImpl::RewardsServiceImpl(Profile* profile)
  ...
  content::URLDataSource::Add(profile_,
                              std::make_unique<PresearchRewardsSource>(profile_));
...
void RewardsServiceImpl::OnAttestPromotion(
  ...
  if (promotion->type == ledger::type::PromotionType::ADS) {
    auto* ads_service = presearch_ads::AdsServiceFactory::GetForProfile(profile_);
    if (ads_service) {
...
void RewardsServiceImpl::PrepareLedgerEnvForTesting() {
  ...
  // this is needed because we are using presearchledger_request_util::buildURL
...
void RewardsServiceImpl::ProcessRewardsPageUrl(
  ...
  const auto url = GURL("presearch:/" + path + query);
  for (net::QueryIterator it(url); !it.IsAtEnd(); it.Advance()) {
...
void RewardsServiceImpl::GetPresearchWallet(GetPresearchWalletCallback callback) {
  if (!Connected()) {
    std::move(callback).Run(nullptr);
    return;
  }

  bat_ledger_->GetPresearchWallet(
    base::BindOnce(&RewardsServiceImpl::OnGetPresearchWallet,
        AsWeakPtr(),
        std::move(callback)));
}

void RewardsServiceImpl::OnGetPresearchWallet(
    GetPresearchWalletCallback callback,
    ledger::type::PresearchWalletPtr wallet) {
