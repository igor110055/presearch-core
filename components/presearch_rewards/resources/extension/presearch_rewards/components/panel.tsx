...
export class Panel extends React.Component<Props, State> {
  ...
  startRewards () {
    chrome.presearchRewards.getACEnabled((enabled: boolean) => {
      this.props.actions.onEnabledAC(enabled)
    })

    chrome.presearchRewards.shouldShowOnboarding((showOnboarding: boolean) => {
      this.props.actions.onShouldShowOnboarding(showOnboarding)
    })

    chrome.presearchRewards.getPrefs((prefs) => {
    ...
    chrome.presearchRewards.getRewardsParameters((parameters: RewardsExtension.RewardsParameters) => {
  ...
  onFetchCaptcha = () => {
    ...
    if (notification && notification.id) {
      ...
      if (promoId) {
        chrome.presearchRewards.claimPromotion(promoId, (properties: RewardsExtension.Captcha) => {
  ...
  onSolution = (promotionId: string, x: number, y: number) => {
    ...
    chrome.presearchRewards.attestPromotion(promotionId, data, (result: number, promotion?: RewardsExtension.Promotion) => {
      // if wrong position try again
      if (result === 6) {
        chrome.presearchRewards.claimPromotion(promotionId, (properties: RewardsExtension.Captcha) => {
          this.actions.onClaimPromotion(properties)
        })
      }
  ...
  showTipSiteDetail = (entryPoint: RewardsExtension.TipDialogEntryPoint) => {
    ...
    chrome.presearchRewards.tipSite(tabId, publisher.publisherKey, entryPoint)
    window.close()
  }
  ...
  refreshPublisher = () => {
    ...
    const publisherKey = publisher && publisher.publisherKey
    if (publisherKey) {
      chrome.presearchRewards.refreshPublisher(publisherKey, (status: number, publisherKey: string) => {
        if (publisherKey) {
  ...
  onDisconnectClick = () => {
    chrome.presearchRewards.disconnectWallet()
  }
