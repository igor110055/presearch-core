...
export class RewardsPanel extends React.Component<Props, State> {
  ...
  componentDidMount () {
    chrome.presearchRewards.isInitialized((initialized: boolean) => {
      if (initialized) {
    ...
    chrome.presearchRewards.onlyAnonWallet((only: boolean) => {
      this.setState({
        onlyAnonWallet: !!only
      })
    })
  ...
  getPublisherPanelInfoForCurrentTab () {
    chrome.tabs.query({
      ...
      const presearchExtensionId = 'mnojpmjdmbbfmejpflffifhffcmidifd'

      chrome.runtime.sendMessage(
        presearchExtensionId,
        {
  ...
  startRewards = () => {
    ...
    chrome.presearchRewards.getAllNotifications((list: RewardsExtension.Notification[]) => {
