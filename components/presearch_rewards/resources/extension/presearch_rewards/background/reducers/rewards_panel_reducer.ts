...
export const rewardsPanelReducer: Reducer<RewardsExtension.State | undefined> = (state: RewardsExtension.State, action: any) => {
  ...
  switch (action.type) {
    case types.ON_TAB_RETRIEVED: {
      ...
      if (!publisher || (publisher.tabUrl !== tab.url || !validKey)) {
        ...
        if (!handledByGreaselion(new URL(tab.url))) {
          chrome.presearchRewards.getPublisherData(
            tab.id,
            tab.url,
            tab.favIconUrl || '',
    ...
    case types.INCLUDE_IN_AUTO_CONTRIBUTION: {
      ...
      chrome.presearchRewards.includeInAutoContribution(publisherKey, exclude)
      break
    }
    ...
    case types.ON_SETTING_SAVE: {
      ...
      if (key) {
        state[key] = value
        chrome.presearchRewards.saveSetting(key, value)
      }
      break
    }
    case types.REMOVE_RECURRING_TIP: {
      ...
      chrome.presearchRewards.removeRecurringTip(publisherKey)
      break
    }
    case types.SAVE_RECURRING_TIP: {
      ...
      chrome.presearchRewards.saveRecurringTip(publisherKey, newAmount)
      break
    }
    ...
    case types.UPDATE_PREFS: {
      const { prefs } = action.payload
      chrome.presearchRewards.updatePrefs(prefs)
      state = { ...state }
