...
class PageWallet extends React.Component<Props, State> {
  ...
  onModalBackupOnCopy = async (backupKey: string) => {
    ...
    try {
      ...
      chrome.send('presearch_rewards.setBackupCompleted')
    } catch (e) {
  ...
  onModalBackupOnSaveFile = (backupKey: string) => {
    ...
    const backupFileText = 'presearch_wallet_recovery.txt'
  ...
  getProcessorString = (processor: Rewards.Processor) => {
    ...
    switch (processor) {
      ...
      case 1: { // Rewards.Processor.PRESEARCH_TOKENS
        text = getLocale('processorPresearchTokens')
        break
      }
      ...
      case 3: { // Rewards.Processor.PRESEARCH_USER_FUNDS
        text = getLocale('processorPresearchUserFunds')
        break
      }
