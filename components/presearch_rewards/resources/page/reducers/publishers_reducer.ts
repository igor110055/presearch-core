...
const publishersReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  ...
  switch (action.type) {
    ...
    case types.ON_EXCLUDE_PUBLISHER: {
      ...
      chrome.send('presearch_rewards.excludePublisher', [publisherKey])
      break
    }
    case types.ON_RESTORE_PUBLISHER: {
      ...
      chrome.send('presearch_rewards.restorePublisher', [publisherKey])
      break
    }
    case types.ON_RESTORE_PUBLISHERS:
      state = { ...state }
      chrome.send('presearch_rewards.restorePublishers', [])
      break
    ...
    case types.REMOVE_RECURRING_TIP:
      ...
      chrome.send('presearch_rewards.removeRecurringTip', [action.payload.publisherKey])
      break
    ...
    case types.GET_EXCLUDED_SITES:
      chrome.send('presearch_rewards.getExcludedSites')
      break
