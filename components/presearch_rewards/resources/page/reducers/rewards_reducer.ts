...
const rewardsReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  ...
  switch (action.type) {
    case types.IS_INITIALIZED: {
      chrome.send('presearch_rewards.isInitialized')
      break
    }
    case types.GET_AUTO_CONTRIBUTE_PROPERTIES: {
      chrome.send('presearch_rewards.getAutoContributeProperties')
      break
    }
    ...
    case types.ON_SETTING_SAVE: {
      ...
      if (persist) {
        chrome.send('presearch_rewards.saveSetting', [key, value.toString()])
      }
    ...
    case types.GET_TIP_TABLE: {
      chrome.send('presearch_rewards.getRecurringTips')
      chrome.send('presearch_rewards.getOneTimeTips')
      break
    }
    case types.GET_CONTRIBUTE_LIST: {
      chrome.send('presearch_rewards.getContributionList')
      break
    }
    case types.GET_ADS_DATA: {
      chrome.send('presearch_rewards.getAdsData')
      break
    }
    ...
    case types.GET_ADS_HISTORY: {
      chrome.send('presearch_rewards.getAdsHistory')
      break
    }
    ...
    case types.TOGGLE_AD_THUMB_UP: {
      chrome.send('presearch_rewards.toggleAdThumbUp',
    ...
    case types.TOGGLE_AD_THUMB_DOWN: {
      chrome.send('presearch_rewards.toggleAdThumbDown',
                  [action.payload.uuid, action.payload.creativeSetId, action.payload.likeAction])
      break
    }
    ...
    case types.TOGGLE_AD_OPT_IN_ACTION: {
      chrome.send('presearch_rewards.toggleAdOptInAction',
    ...
    case types.TOGGLE_AD_OPT_OUT_ACTION: {
      chrome.send('presearch_rewards.toggleAdOptOutAction',
                  [action.payload.category, action.payload.optAction])
      break
    }
    ...
    case types.TOGGLE_SAVE_AD: {
      chrome.send('presearch_rewards.toggleSaveAd',
    ...
    case types.TOGGLE_FLAG_AD: {
      chrome.send('presearch_rewards.toggleFlagAd',
    ...
    case types.ON_ADS_SETTING_SAVE: {
      ...
      const value = action.payload.value
      if (key) {
        chrome.send('presearch_rewards.saveAdsSetting', [key, value.toString()])
    ...
    case types.ON_STATEMENT_CHANGED: {
      chrome.send('presearch_rewards.getStatement', [])
      break
    }
    ...
    case types.ON_INLINE_TIP_SETTINGS_CHANGE: {
      ...
      inlineTip[key] = value
      chrome.send('presearch_rewards.setInlineTippingPlatformEnabled', [key, value.toString()])
    ...
    case types.PROCESS_REWARDS_PAGE_URL: {
      ...
      chrome.send('presearch_rewards.processRewardsPageUrl', [path, query])
    ...
    case types.ON_PROCESS_REWARDS_PAGE_URL: {
      ...
      chrome.send('presearch_rewards.getExternalWallet')
      ...
      if (data.walletType === 'uphold' || data.walletType === 'bitflyer') {
        chrome.send('presearch_rewards.fetchBalance')
    ...
    case types.DISCONNECT_WALLET: {
      chrome.send('presearch_rewards.disconnectWallet')
      break
    }
    case types.ONLY_ANON_WALLET: {
      chrome.send('presearch_rewards.onlyAnonWallet')
      break
    }
    ...
    case types.GET_COUNTRY_CODE: {
      chrome.send('presearch_rewards.getCountryCode')
      break
    }
    ...
    case types.ON_INITIALIZED: {
      chrome.send('presearch_rewards.getReconcileStamp')
      state = {
        ...state,
        initializing: false
      }
    ...
    case types.COMPLETE_RESET: {
      chrome.send('presearch_rewards.completeReset')
      break
    }
    ...
    case types.GET_PAYMENT_ID: {
      chrome.send('presearch_rewards.getPaymentId')
      break
    }
    ...
    case types.GET_ONBOARDING_STATUS: {
      chrome.send('presearch_rewards.getOnboardingStatus')
      break
    }
    ...
    case types.SAVE_ONBOARDING_RESULT: {
      chrome.send('presearch_rewards.saveOnboardingResult', [action.payload.result])
      chrome.send('presearch_rewards.getAutoContributeProperties')
      onboardingCompletedStore.save()
      state = {
        ...state,
