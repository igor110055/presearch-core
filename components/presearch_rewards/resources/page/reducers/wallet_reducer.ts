...
const walletReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  ...
  switch (action.type) {
    case types.GET_REWARDS_PARAMETERS:
      chrome.send('presearch_rewards.getRewardsParameters')
      break
    ...
    case types.RECOVER_WALLET: {
      ...
      chrome.send('presearch_rewards.recoverWallet', [key])
      break
    }
    case types.ON_RECOVER_WALLET_DATA: {
      ...
      if (result === 0) {
        chrome.send('presearch_rewards.fetchPromotions')
    ...
    case types.GET_BALANCE_REPORT: {
      chrome.send('presearch_rewards.getBalanceReport', [
    ...
    case types.GET_CONTRIBUTION_AMOUNT: {
      chrome.send('presearch_rewards.getContributionAmount')
      break
    }
    ...
    case types.GET_PENDING_CONTRIBUTIONS: {
      chrome.send('presearch_rewards.getPendingContributions')
      break
    }
    ...
    case types.REMOVE_PENDING_CONTRIBUTION: {
      chrome.send('presearch_rewards.removePendingContribution', [
        action.payload.id
      ])
      break
    }
    case types.REMOVE_ALL_PENDING_CONTRIBUTION: {
      chrome.send('presearch_rewards.removeAllPendingContribution')
      break
    }
    ...
    case types.GET_MONTHLY_REPORT: {
      ...
      chrome.send('presearch_rewards.getMonthlyReport', [month, year])
      break
    }
    ...
    case types.GET_MONTHLY_REPORT_IDS: {
      chrome.send('presearch_rewards.getMonthlyReportIds')
      break
    }
    ...
    case types.GET_WALLET_PASSPHRASE:	{
      chrome.send('presearch_rewards.getWalletPassphrase')
      break
    }
