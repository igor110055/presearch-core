...
#include "presearch/components/tor/tor_switches.h"
...
#if defined(OS_WIN)
const char kTorClientComponentName[] = "Presearch Tor Client Updater (Windows)";
...
const char kTorClientComponentBase64PublicKey[] =
    ...
    "rQIDAQAB";
#elif defined(OS_MAC)
const char kTorClientComponentName[] = "Presearch Tor Client Updater (Mac)";
...
const char kTorClientComponentName[] = "Presearch Tor Client Updater (Linux)";
...
std::string PresearchTorClientUpdater::g_tor_client_component_id_(
    kTorClientComponentId);
std::string PresearchTorClientUpdater::g_tor_client_component_base64_public_key_(
    kTorClientComponentBase64PublicKey);

PresearchTorClientUpdater::PresearchTorClientUpdater(
    PresearchComponent::Delegate* component_delegate,
    PrefService* local_state,
    const base::FilePath& user_data_dir)
    : PresearchComponent(component_delegate),
...
void PresearchTorClientUpdater::Register() {
  ...
  PresearchComponent::Register(kTorClientComponentName,
                           g_tor_client_component_id_,
                           g_tor_client_component_base64_public_key_);
  registered_ = true;
}

void PresearchTorClientUpdater::Unregister() {
  // We don't call PresearchComponent::Unregister here in order to prevent tor
  // executable component from getting deleted when last tor window closed
  registered_ = false;
}

void PresearchTorClientUpdater::Cleanup() {
...
void PresearchTorClientUpdater::SetExecutablePath(const base::FilePath& path) {
  executable_path_ = path;
  for (Observer& observer : observers_)
    observer.OnExecutableReady(path);
}

base::FilePath PresearchTorClientUpdater::GetExecutablePath() const {
  return executable_path_;
}

void PresearchTorClientUpdater::OnComponentReady(
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&PresearchTorClientUpdater::SetExecutablePath,
                     weak_ptr_factory_.GetWeakPtr()));
}

bool PresearchTorClientUpdater::IsTorDisabled() {
  if (local_state_)
    return local_state_->GetBoolean(tor::prefs::kTorDisabled);
  return false;
}

void PresearchTorClientUpdater::AddObserver(Observer* observer) {
  observers_.AddObserver(observer);
}

void PresearchTorClientUpdater::RemoveObserver(Observer* observer) {
  observers_.RemoveObserver(observer);
}

// static
void PresearchTorClientUpdater::SetComponentIdAndBase64PublicKeyForTest(
    const std::string& component_id,
