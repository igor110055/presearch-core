...
const StyledPage = styled('div')<PageProps>`
  ...
  /* Fix the main NTP content so, when Presearch Today is in-view,
  ...
  .${CLASSNAME_PAGE_STUCK} & {
    position: fixed;
    bottom: 0;
    /* Blur out the content when Presearch Today is interacted
...
export const Page: React.FunctionComponent<PageProps> = (props) => {
  ...
  // extra scroll area (Presearch Today) then we "sticky" the Page at
...
export const GridItemNavigationPresearchToday = styled('div')<{}>`
  position: absolute;
  bottom: 20px;
  left: 50%;
...
function getPageBackground (p: HasImageProps) {
  ...
  // and Page is the element with blur (when presearch today is active)
