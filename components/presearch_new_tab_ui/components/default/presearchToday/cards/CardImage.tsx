...
function useGetUnpaddedImage (paddedUrl: string, onLoaded?: () => any) {
  ...
  React.useEffect(() => {
    // Storybook method
    // @ts-ignore
    if (window.presearchStorybookUnpadUrl) {
      // @ts-ignore
      window.presearchStorybookUnpadUrl(paddedUrl)
      .then(onReceiveUnpaddedUrl)
      return
    }
    Background.send<PresearchToday.Messages.GetImageDataResponse,
        PresearchToday.Messages.GetImageDataPayload>(
