...
type Props = {
  publisher: PresearchToday.Publisher
...
export default function PublisherMetaComponent (props: Props) {
  ...
  const commandText = React.useMemo<string>(() => {
    const raw = getLocale('presearchTodayDisableSourceCommand')
    const publisherIndex = raw.indexOf('$1')
    if (publisherIndex === -1) {
      console.warn('Locale string for presearchTodayDisableSourceCommand did not have a $1 replacement area for publisher name.', raw)
