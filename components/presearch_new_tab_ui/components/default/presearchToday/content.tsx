...
function getFeedHashForCache (feed?: PresearchToday.Feed) {
  return feed ? feed.hash : ''
}

let pageRequestPending = false

export default function PresearchTodayContent (props: Props) {
  ...
  const setScrollTriggerRef = React.useCallback((element) => {
    ...
    const endOfCurrentArticlesListObserver = new IntersectionObserver((entries) => {
      console.debug('Presearch Today content Intersection Observer triggered')
      if (entries.some(entry => entry.intersectionRatio > 0)) {
        console.debug('Presearch Today content Intersection Observer determined need new page.')
  ...
  const intersectionObserver = React.useRef(new IntersectionObserver(entries => {
    ...
    console.debug(`Presearch Today: viewed ${count} cards.`)
  ...
  if (!feed || !publishers) {
    console.error('Presearch Today: should have shown error or loading state, but ran in to an unintended code path.')
    return null
  }
  ...
  return (
    ...
    {/* featured item */}
      ...
      <Customize onCustomizePresearchToday={props.onCustomizePresearchToday} show={showOptions} />
