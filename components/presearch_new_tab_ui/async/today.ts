...
handler.on<Actions.ReadFeedItemPayload>(Actions.readFeedItem.getType(), async (store, payload) => {
  ...
  if (payload.isPromoted) {
    backendArgs.push(
      ...
      (payload.item as PresearchToday.PromotedArticle).creative_instance_id,
      payload.isPromoted
    )
  }
...
handler.on<PresearchToday.PromotedArticle>(Actions.promotedItemViewed.getType(), async (store, item) => {
