...
export async function getInitialData (): Promise<InitialData> {
  try {
    ...
    const [
      ...
      new Promise((resolve) => {
        if (!('presearchTogether' in chrome)) {
          resolve(false)
          return
        }

        chrome.presearchTogether.isSupported((supported: boolean) => {
          resolve(supported)
        })
      }),
      new Promise((resolve) => {
...
export async function getRewardsPreInitialData (): Promise<PreInitialRewardsData> {
  try {
    const [
      ...
      new Promise(resolve => chrome.presearchRewards.getAdsEnabled((enabledAds: boolean) => {
        resolve(enabledAds)
      })),
      new Promise(resolve => chrome.presearchRewards.getAdsSupported((adsSupported: boolean) => {
        resolve(adsSupported)
      })),
      new Promise(resolve => chrome.presearchRewards.onlyAnonWallet((onlyAnonWallet: boolean) => {
...
export async function getRewardsInitialData (): Promise<InitialRewardsData> {
  try {
    const [
      ...
      new Promise(resolve => chrome.presearchRewards.getAdsAccountStatement((success: boolean, adsAccountStatement: NewTab.AdsAccountStatement) => {
        resolve(success ? adsAccountStatement : undefined)
      })),
      new Promise(resolve => chrome.presearchRewards.getBalanceReport(new Date().getMonth() + 1, new Date().getFullYear(),(report: NewTab.RewardsBalanceReport) => {
        resolve(report)
      })),
      new Promise(resolve => chrome.presearchRewards.fetchBalance((balance: NewTab.RewardsBalance) => {
        resolve(balance)
      })),
      new Promise(resolve => chrome.presearchRewards.getRewardsParameters((parameters: NewTab.RewardsParameters) => {
        resolve(parameters)
      })),
      new Promise(resolve => {
        chrome.presearchRewards.fetchPromotions()
        resolve(true)
      })
    ])
    return {
