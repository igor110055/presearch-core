...
export type PresearchTodayState = {
  ...
  articleScrollTo?: PresearchToday.FeedItem
}
...
const defaultState: PresearchTodayState = {
  isFetching: true,
...
if (history.state && history.state.todayArticle) {
  ...
  defaultState.articleScrollTo = history.state.todayArticle as PresearchToday.FeedItem
...
// if it gets removed and rendered again (e.g. if presearch today is toggled off and on).
// Reset to defaultState when Today is turned off or refreshed.

const reducer = createReducer<PresearchTodayState>({}, defaultState)
