...
function CategoryList (props: CategoryListProps) {
  ...
  return (
    <>
      <SettingsSectionTitle>{getLocale('presearchTodaySourcesTitle')}</SettingsSectionTitle>
...
type CategoryProps = {
  category: string
  publishers: PresearchToday.Publisher[]
  onBack: () => any
...
const categoryNameAll = getLocale('presearchTodayCategoryNameAll')
...
export default function Sources (props: SourcesProps) {
  const publishersByCategory = React.useMemo<Map<string, PresearchToday.Publisher[]>>(() => {
    const result = new Map<string, PresearchToday.Publisher[]>()
