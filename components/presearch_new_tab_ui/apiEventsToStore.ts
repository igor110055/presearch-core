...
chrome.presearchRewards.initialized.addListener((result: any | NewTab.RewardsResult) => {
  rewardsInitData()
})

chrome.presearchRewards.onAdsEnabled.addListener((enabled: boolean) => {
  getActions().onAdsEnabled(enabled)
})

chrome.presearchRewards.onPromotions.addListener((result: number, promotions: NewTab.Promotion[]) => {
  getActions().onPromotions(result, promotions)
})

chrome.presearchRewards.onPromotionFinish.addListener((result: number, promotion: NewTab.Promotion) => {
