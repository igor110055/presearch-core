...
#include "presearch/components/services/bat_ledger/bat_ledger_impl.h"
...
#include "presearch/components/services/bat_ledger/bat_ledger_client_mojo_bridge.h"
...
// static
void BatLedgerImpl::OnGetPresearchWallet(
    CallbackHolder<GetPresearchWalletCallback>* holder,
...
void BatLedgerImpl::GetPresearchWallet(GetPresearchWalletCallback callback) {
  auto* holder = new CallbackHolder<GetPresearchWalletCallback>(
      AsWeakPtr(), std::move(callback));

  ledger_->GetPresearchWallet(
      std::bind(BatLedgerImpl::OnGetPresearchWallet,
          holder,
          _1));
}
