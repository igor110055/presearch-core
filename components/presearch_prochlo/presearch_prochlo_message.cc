...
#include "presearch/components/presearch_prochlo/prochlo_data.h"
...
bool MakeProchlomation(uint64_t metric,
  ...
  // TODO(iefremov): - create patch for adding `presearch_p3a`
  // to src/base/trace_event/builtin_categories.h
  // TRACE_EVENT0("presearch_p3a", "MakeProchlomation");

  PresearchProchloCrypto crypto;
...
void InitProchloMessage(uint64_t metric_hash,
                        ...
                        presearch_pyxis::PyxisMessage* pyxis_message) {
  DCHECK(pyxis_message);
  presearch_pyxis::PyxisValue* value = pyxis_message->add_pyxis_values();
...
void GenerateProchloMessage(uint64_t metric_hash,
  ...
  // TODO(iefremov): - create patch for adding `presearch_p3a`
  // to src/base/trace_event/builtin_categories.h
  // TRACE_EVENT0("presearch_p3a", "GenerateProchloMessage");
  ShufflerItem item;
...
void GenerateP3AMessage(uint64_t metric_hash,
  ...
  // TODO(iefremov): - create patch for adding `presearch_p3a`
  // to src/base/trace_event/builtin_categories.h
  // TRACE_EVENT0("presearch_p3a", "GenerateP3AMessage");
  uint8_t data[kProchlomationDataLength] = {0};
