...
#include "presearch/components/presearch_ads/browser/ad_notification.h"
...
#include "presearch/components/presearch_ads/browser/frequency_capping_helper.h"
...
#include "presearch/components/presearch_ads/common/switches.h"
#include "presearch/components/presearch_rewards/browser/rewards_notification_service.h"
#include "presearch/components/presearch_rewards/browser/rewards_p3a.h"
...
#include "presearch/components/services/bat_ads/public/cpp/ads_client_mojo_bridge.h"
...
#if defined(OS_ANDROID)
#include "presearch/browser/notifications/presearch_notification_platform_bridge_helper_android.h"
...
using presearch_rewards::RewardsNotificationService;
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("ads_service_impl", R"(
      semantics {
        sender: "Presearch Ads Service"
        description:
          "This service is used to communicate with Presearch servers "
      ...
      policy {
        cookies_allowed: NO
        setting:
          "You can enable or disable this feature by visiting presearch://rewards."
...
AdsServiceImpl::AdsServiceImpl(Profile* profile,
  ...
  DCHECK(presearch::IsRegularProfile(profile_));
...
void AdsServiceImpl::OnTabUpdated(const SessionID& tab_id,
  ...
  const bool is_incognito = !presearch::IsRegularProfile(profile_);
...
void AdsServiceImpl::OnWalletUpdated() {
  ...
  rewards_service_->GetPresearchWallet(
      base::BindOnce(&AdsServiceImpl::OnGetPresearchWallet, AsWeakPtr()));
}

void AdsServiceImpl::OnGetPresearchWallet(ledger::type::PresearchWalletPtr wallet) {
  if (!wallet) {
...
void AdsServiceImpl::Shutdown() {
  ...
  g_presearch_browser_process->resource_component()->RemoveObserver(this);
...
void AdsServiceImpl::Initialize() {
  ...
  profile_pref_change_registrar_.Add(
      presearch_rewards::prefs::kWalletPresearch,
      base::Bind(&AdsServiceImpl::OnPrefsChanged, base::Unretained(this)));
...
void AdsServiceImpl::ResetState() {
  ...
  profile_->GetPrefs()->ClearPrefsWithPrefixSilently("presearch.presearch_ads");
...
void AdsServiceImpl::OnEnsureBaseDirectoryExists(const bool success) {
  ...
  g_presearch_browser_process->resource_component()->AddObserver(this);
...
#if defined(OS_ANDROID)
  if (GetBooleanPref(presearch_rewards::prefs::kUseRewardsStagingServer)) {
    environment = ads::Environment::STAGING;
  }
#else
...
void AdsServiceImpl::SetBuildChannel() {
  ...
  build_channel->name = presearch::GetChannelName();
...
void AdsServiceImpl::RegisterResourceComponentsForLocale(
  ...
  g_presearch_browser_process->resource_component()->RegisterComponentsForLocale(
      locale);
}
...
void AdsServiceImpl::MigratePrefs() {
  is_upgrading_from_pre_presearch_ads_build_ = IsUpgradingFromPrePresearchAdsBuild();
  if (is_upgrading_from_pre_presearch_ads_build_) {
    VLOG(1) << "Migrating ads preferences from pre Presearch Ads build";

    // Force migration of preferences from version 1 if
    // |is_upgrading_from_pre_presearch_ads_build_| is set to true to fix
    // "https://github.com/presearch/presearch-browser/issues/5434"
...
void AdsServiceImpl::MigratePrefsVersion2To3() {
  ...
  const auto country_code = presearch_l10n::GetCountryCode(locale);

  // Disable ads if upgrading from a pre presearch ads build due to a bug where ads
  // were always enabled
  DisableAdsIfUpgradingFromPrePresearchAdsBuild();
...
void AdsServiceImpl::MigratePrefsVersion6To7() {
  ...
  if (is_a_legacy_country_code) {
    // Do not disable Presearch Ads for legacy country codes introduced before
    // version 1.3.x
    return;
  }
  ...
  if (last_schema_version >= 4) {
    // Do not disable Presearch Ads if |kSupportedCountryCodesLastSchemaVersion|
...
void AdsServiceImpl::MigratePrefsVersion7To8() {
  const bool rewards_enabled = GetBooleanPref(presearch_rewards::prefs::kEnabled);
  if (!rewards_enabled) {
    SetEnabled(false);
  }
}
...
bool AdsServiceImpl::IsUpgradingFromPrePresearchAdsBuild() {
  // Presearch ads was hidden in 0.62.x however due to a bug |prefs::kEnabled| was
  // set to true causing "https://github.com/presearch/presearch-browser/issues/5434"

  // |prefs::kIdleTimeThreshold| was not serialized in 0.62.x

  // |prefs::kVersion| was introduced in 0.63.x

  // We can detect if we are upgrading from a pre Presearch ads build by checking
...
void AdsServiceImpl::DisableAdsIfUpgradingFromPrePresearchAdsBuild() {
  if (!is_upgrading_from_pre_presearch_ads_build_) {
    return;
  }
...
void AdsServiceImpl::OnPrefsChanged(const std::string& pref) {
  if (pref == ads::prefs::kEnabled) {
    ...
    // Record P3A.
    presearch_rewards::UpdateAdsP3AOnPreferenceChange(profile_->GetPrefs(), pref);
  } else if (pref == ads::prefs::kIdleTimeThreshold) {
    StartCheckIdleStateTimer();
  } else if (pref == presearch_rewards::prefs::kWalletPresearch) {
    OnWalletUpdated();
  }
}
...
std::string AdsServiceImpl::GetLocale() const {
  return presearch_l10n::LocaleHelper::GetInstance()->GetLocale();
}
...
void AdsServiceImpl::ShowNotification(
  ...
  if (presearch::IsNightlyOrDeveloperBuild()) {
    ...
    display_service_->Display(NotificationHandler::Type::PRESEARCH_ADS,
...
#if defined(OS_ANDROID)
    const std::string presearch_ads_url_prefix = kPresearchAdsUrlPrefix;
    const GURL service_worker_scope =
        GURL(presearch_ads_url_prefix.substr(0, presearch_ads_url_prefix.size() - 1));
    PresearchNotificationPlatformBridgeHelperAndroid::MaybeRegenerateNotification(
        uuid, service_worker_scope);
#endif
    display_service_->Close(NotificationHandler::Type::PRESEARCH_ADS, uuid);
  }
}
...
void AdsServiceImpl::LoadAdsResource(const std::string& id,
  ...
  const base::Optional<base::FilePath> path =
      g_presearch_browser_process->resource_component()->GetPath(id, version);
