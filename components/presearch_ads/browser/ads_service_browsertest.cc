...
#include "presearch/components/presearch_rewards/browser/rewards_notification_service_impl.h"
...
#include "presearch/components/l10n/browser/locale_helper_mock.h"
...
// npm run test -- presearch_browser_tests --filter=PresearchAdsBrowserTest.*
...
struct PresearchAdsUpgradePathParamInfo {
  // |preferences| should be set to the name of the preferences filename located
  // at "src/presearch/test/data/rewards-data/migration"
  ...
  // |rewards_enabled| should be set to true if Presearch rewards should be enabled
  // after upgrade; otherwise, should be set to false
  bool rewards_enabled;

  // |ads_enabled| should be set to true if Presearch ads should be enabled after
...
class PresearchAdsBrowserTest : public InProcessBrowserTest,
                            public base::SupportsWeakPtr<PresearchAdsBrowserTest> {
 public:
  PresearchAdsBrowserTest() {
    // You can do set-up work for each test here

    MaybeMockLocaleHelper();
  }

  ~PresearchAdsBrowserTest() override {
  ...
  void SetUpOnMainThread() override {
    ...
    rewards_service_ = static_cast<presearch_rewards::RewardsServiceImpl*>(
        presearch_rewards::RewardsServiceFactory::GetForProfile(browser_profile));
    rewards_service_->ForTestingSetTestResponseCallback(base::BindRepeating(
        &PresearchAdsBrowserTest::GetTestResponse, base::Unretained(this)));

    ads_service_ = static_cast<presearch_ads::AdsServiceImpl*>(
        presearch_ads::AdsServiceFactory::GetForProfile(browser_profile));
  ...
  void GetTestDataDir(base::FilePath* test_data_dir) {
    ...
    ASSERT_TRUE(base::PathService::Get(presearch::DIR_TEST_DATA, test_data_dir));
  ...
  void GetTestResponse(const std::string& url,
    ...
    if (url.find("/v3/wallet/presearch") != std::string::npos) {
      *response = wallet_;
  ...
  bool SetUpUserDataDirectory() override {
    MaybeMockUserProfilePreferencesForPresearchAdsUpgradePath();
  ...
  void MaybeMockLocaleHelper() {
    const std::map<std::string, std::string> locale_for_tests = {
        {"PresearchAdsLocaleIsSupported", "en_US"},
        {"PresearchAdsLocaleIsNotSupported", "en_XX"},
        {"PresearchAdsLocaleIsNewlySupported", "ja_JP"},
        {"PresearchAdsLocaleIsNewlySupportedForLatestSchemaVersion",
         newly_supported_locale_},
        {"PresearchAdsLocaleIsNotNewlySupported", "en_XX"},
    ...
    if (it == locale_for_tests.end()) {
      MaybeMockLocaleHelperForPresearchAdsUpgradePath();
      return;
    }
  ...
  void MaybeMockLocaleHelperForPresearchAdsUpgradePath() {
  ...
  void MockLocaleHelper(const std::string& locale) {
    locale_helper_mock_ =
        std::make_unique<NiceMock<presearch_l10n::LocaleHelperMock>>();

    presearch_l10n::LocaleHelper::GetInstance()->set_for_testing(
        locale_helper_mock_.get());

    ON_CALL(*locale_helper_mock_, GetLocale()).WillByDefault(Return(locale));
  }

  void MaybeMockUserProfilePreferencesForPresearchAdsUpgradePath() {
    std::vector<std::string> parameters;
    if (!GetUpgradePathParams(&parameters)) {
      return;
    }
  ...
  bool GetUpgradePathParams(std::vector<std::string>* parameters) {
    ...
    if (test_suite_name != "PresearchAdsBrowserTest/PresearchAdsUpgradeBrowserTest") {
      return false;
    }
  ...
  base::FilePath GetTestDataPath() const {
    ...
    path = path.Append(FILE_PATH_LITERAL("presearch"));
  ...
  presearch_rewards::RewardsServiceImpl* rewards_service_;

  presearch_ads::AdsServiceImpl* ads_service_;

  std::unique_ptr<presearch_l10n::LocaleHelperMock> locale_helper_mock_;
...
IN_PROC_BROWSER_TEST_F(PresearchAdsBrowserTest, PresearchAdsLocaleIsSupported) {
  EXPECT_TRUE(ads_service_->IsSupportedLocale());
}

IN_PROC_BROWSER_TEST_F(PresearchAdsBrowserTest, PresearchAdsLocaleIsNotSupported) {
  EXPECT_FALSE(ads_service_->IsSupportedLocale());
}

IN_PROC_BROWSER_TEST_F(PresearchAdsBrowserTest, PresearchAdsLocaleIsNewlySupported) {
  GetPrefs()->SetInteger(
      presearch_ads::prefs::kSupportedCountryCodesLastSchemaVersion, 3);

  GetPrefs()->SetInteger(
      presearch_ads::prefs::kSupportedCountryCodesSchemaVersion,
      presearch_ads::prefs::kSupportedCountryCodesSchemaVersionNumber);

  EXPECT_TRUE(ads_service_->IsNewlySupportedLocale());
}

IN_PROC_BROWSER_TEST_F(PresearchAdsBrowserTest,
                       PresearchAdsLocaleIsNewlySupportedForLatestSchemaVersion) {
  // IMPORTANT: When adding new schema versions |newly_supported_locale_| must
  // be updated in |PresearchAdsBrowserTest| to reflect a locale from the latest
  // "bat-native-ads/src/bat/ads/internal/locale/supported_country_codes.h"
  // schema

  GetPrefs()->SetInteger(
      presearch_ads::prefs::kSupportedCountryCodesLastSchemaVersion,
      presearch_ads::prefs::kSupportedCountryCodesSchemaVersionNumber);
...
IN_PROC_BROWSER_TEST_F(PresearchAdsBrowserTest, PresearchAdsLocaleIsNotNewlySupported) {
  GetPrefs()->SetInteger(
      presearch_ads::prefs::kSupportedCountryCodesLastSchemaVersion, 2);
...
class PresearchAdsUpgradeBrowserTest
    : public PresearchAdsBrowserTest,
      public ::testing::WithParamInterface<PresearchAdsUpgradePathParamInfo> {};

const PresearchAdsUpgradePathParamInfo kTests[] = {
    ...
    //
    // Upgrade from 0.63 to current version (Initial release of Presearch ads)
    {
...
IN_PROC_BROWSER_TEST_P(PresearchAdsUpgradeBrowserTest, PRE_UpgradePath) {
  // Handled in |MaybeMockLocaleHelperForPresearchAdsUpgradePath|
}

IN_PROC_BROWSER_TEST_P(PresearchAdsUpgradeBrowserTest, UpgradePath) {
  PresearchAdsUpgradePathParamInfo param(GetParam());

  EXPECT_EQ(IsAdsEnabled(), param.ads_enabled);
}

// Generate the test case name from the metadata included in
// |PresearchAdsUpgradePathParamInfo|
static std::string GetTestCaseName(
    ::testing::TestParamInfo<PresearchAdsUpgradePathParamInfo> param_info) {
...
INSTANTIATE_TEST_SUITE_P(PresearchAdsBrowserTest,
                         PresearchAdsUpgradeBrowserTest,
