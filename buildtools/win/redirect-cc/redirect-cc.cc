...
// variable PRESEARCH_BUILD_SHOW_REDIRECT_CC_CMD (set to 1 for verbose).
BOOL IsVerbose() {
  wchar_t buffer[2];
  DWORD dwRet =
      GetEnvironmentVariableW(L"PRESEARCH_BUILD_SHOW_REDIRECT_CC_CMD", buffer, 2);
  if (dwRet > 2) {
    wprintf(
        L"REDIRECT-CC: PRESEARCH_BUILD_SHOW_REDIRECT_CC_CMD env. var. is expected "
    ...
    if (dwRet != ERROR_ENVVAR_NOT_FOUND) {
      wprintf(
          ...
          L"PRESEARCH_BUILD_SHOW_REDIRECT_CC_CMD (%d).\n",
          dwRet);
    }
    return FALSE;
...
int wmain(int argc, wchar_t* argv[]) {
  ...
  std::wstring cmd_line_builder =
      L"python ..\\..\\presearch\\script\\redirect-cc.py";
